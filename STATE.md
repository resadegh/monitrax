...
- **Active task + stop-point:** Continuity-system PR #1100 OPEN (this `STATE.md` + `session-start.sh` continuity
  block). Chat-side `Section 0` boot gate already pasted into the Claude.ai project instructions.
- **Immediate next action:** RESUME CHECK confirms whether PR #1100 has merged. Then: (1) once merged, the
  read-rail is live; (2) grant GitHub `workflow` scope so `continuity-gate.yml` (write-rail) can be committed;
  (3) begin Phase 1 deep ingestion (Cowork range-reads the live plan + architecture + calc engines, fills PENDING).
- **Open decisions / blockers:**
  - PR #1100 awaiting Reza review/merge.
  - GitHub `workflow` scope NOT granted — blocks committing `.github/workflows/continuity-gate.yml` (and, later, the Phase 4 test-runner workflow).
