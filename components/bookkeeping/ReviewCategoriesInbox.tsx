...
import { Check, Loader2, Sparkles, X, CircleAlert, CheckCircle2 } from 'lucide-react';
...
export function ReviewCategoriesInbox() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      const [sumRes, medRes, lowRes] = await Promise.all([
      ...
      const [sumJson, medJson, lowJson] = await Promise.all([
        sumRes.json(),
        medRes.json(),
        lowRes.json(),
      ]);
  ...
  // Header progress — share of this month's pile already settled.
  const { reviewed, total, pct } = useMemo(() => {
    const high = summary?.high ?? 0;
    const toReview = items.length;
    const t = high + toReview;
    return {
      reviewed: high,
      total: t,
