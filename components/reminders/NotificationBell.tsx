 ...
 * popover dependency — §12.7 / §13.8). The "+ New reminder" action (R1 PR2b)
 * opens `<AddReminderDialog>` for user-created custom reminders.
 *
...
import { Bell, Car, ShieldCheck, Building2, Landmark, Link2, Pin, Download, Receipt, Clock, X, Plus } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuTrigger,
...
export function NotificationBell() {
  const { reminders, act, createCustom } = useReminders();
  ...
  return (
    <>
        ...
        <DropdownMenuContent align="end" className="w-80 p-0 sm:w-96">
          ...
          {count === 0 ? (
            ...
            <div className="max-h-96 overflow-y-auto py-1">
              {reminders!.map((r) => (
                <NotificationRow key={r.id} reminder={r} onNavigate={() => setOpen(false)} onAct={act} />
              ))}
            </div>
          )}
        </DropdownMenuContent>
      ...
      <AddReminderDialog open={dialogOpen} onOpenChange={setDialogOpen} onCreate={createCustom} />
    </>
  );
}
...
function NotificationRow({
  ...
  onNavigate: () => void;
  onAct: (reminder: RenewalReminder, action: 'snooze' | 'dismiss' | 'done', snoozeDays?: number) => void;
}) {
  ...
  return (
    <div className="flex items-center gap-3 px-2 py-2 transition-colors hover:bg-muted/50">
      {/* Custom reminders have no entity page (empty href) → non-navigable. */}
      {r.href ? (
        <Link
          href={r.href}
          onClick={onNavigate}
        ...
        <div className="flex min-w-0 flex-1 items-center gap-3 px-1">{inner}</div>
      )}
