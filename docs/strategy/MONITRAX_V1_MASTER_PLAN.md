...
| Field | Value |
|---|---|
| **Current milestone** | **M2: build half DONE, gate held OPEN by the Ring-3 FAIL on #1595 (the D-12 pack)** → **M3 kickoff running** per `BRIEF_M3_PACK_AND_SCOREBOARD.md` — the pack fix (M3 PR-1) is the M2.5 unblock AND the M3 priority |
| **Last session** | 2026-08-19 · Code (Fable 5) · **M3 PR-1 (#1601)**: the pack fix (MON-168/169/170 → FIXING; expected movements FIRST in `RING3_M3_PACK_FIX.md`) · **M3 PR-2**: the v1 scoreboard — D-19 tile registry + visibility law + suppress toggles, 5 kept-engine tiles + 9 dark stage tiles, admin Dashboard-tiles section, §D balances relabel ("Accounts net — excludes property") |
| **Next action** | 🧑 Reza merges #1601 (PR-1) then PR-2 → Matrix runs `RING3_M3_PACK_FIX.md` (backfill dry-run→apply→export; flips MON-168/169/170 VERIFIED, unblocks M2.5) → Reza flips MODULE_HOUSEKEEPING (§B) and, at scoreboard acceptance, MODULE_HOME (retires the redirect) · rules: portfolio-tile third figure (net worth as built, or add a canonical equity field), reports legacy tiles (hide — recommended), MON-145 dated rate · 🟦 Code next: the §18.2.1 Stitch backfill for the scoreboard + M3.1 pack restructure |
| **Blockers** | none |
| **Baseline of record** | `.audit/golden-baseline-12954ff.json` (VR-048, 1,756 leaves, treeHash `0d6753ef…`) |
| **PROD state** | v1 shape live — 13 MODULE_* keys HIDDEN, **zero overrides** (R0 teardown verified 2026-08-19 from override-holder, other-account and signed-out viewpoints). **Known live defect: MON-163** (kept property page links into hidden routes → 404s) — **fix built in M2 PR-1, awaiting merge.** |
...
- [x] M2.3 🟦(build)→🟩(Ring-3 pending) **Fixes BUILT on #1595** — MON-164 (pack rows canonical: actuals-first + one-off-once), MON-165 (ONE depreciation producer), MON-129 kept slice (entityBreakdown + propertyCashflow gates), MON-166 (100× dep-rate render), MON-145 M2 slice (honest basis label; real fix schema-bound → Reza decision), MON-001 re-cut (R1 = MON-164; income-chip check returns at R3), MON-146 HELD (hidden-only). **→ VERIFIED requires the Matrix's Ring-3 on live data (M2.2) — handout: `docs/verification/briefs/RING3_M2_PACK_CANONICAL.md`**
