...
| **Last session** | 2026-08-22 · Matrix HQ · catch-up #1603 merged (plan current) · **next Code brief CUT: `BRIEF_M3_PUNCHLIST_AND_CLOSEOUT.md`** — every punch-list item root-caused in source (intake tile: missing `band` param → 400 · EOFY tile: current-FY-only window · LVR: TWO live producers, snapshot vs page arithmetic · strip: bare `slice(0,4)`) · ATO zero-labelling diagnosed: seeds EXIST and run — exact-triple resolution mismatch (`summary.ts:327` vs seeds at subcategory-null) · **NEW: the MODULE_HOME flip resurfaced two R4-family surfaces** (legacy Financial Overview report tile + open `/api/money-flow`) — re-key in the brief §D |
| **Next action** | 🧑 Reza merges the brief PR → 🟦 Code executes `BRIEF_M3_PUNCHLIST_AND_CLOSEOUT.md` (§A registry + flips · §B ATO labelling fix, D-21 · §C punch list ×4 · §D MODULE_HOME re-key + legacy-tile hide · §E M2.5 close-out) → 🟩 Matrix runs the brief's Ring-3 handout, then M2.2 T2 Ring-3 + M2.7 gate review |
| **Blockers** | none |
...
| MON-168/169/170 | The D-12 pack trio (propertyId never stamped · transfers counted · silent label drops) | **FIXED #1601 · backfill applied on PROD 2026-08-22 (46 rows, idempotent) · Ring-3 PASS → VERIFIED** (registry flip lands with the next Code PR) |
| (to register) | Scoreboard punch list ×4 (EOFY empty state · intake "—" · LVR label pair · strip 4/6 rule) + duplicate 'Guildford' + stray 'Thornlands' + 2 orphaned link ids (data — register only) + pack ATO labelling zero-match (resolution fix, not a seed gap) + **MODULE_HOME re-key sweep (flip resurfaced Financial Overview tile + `/api/money-flow`)** | **BRIEFED — `BRIEF_M3_PUNCHLIST_AND_CLOSEOUT.md` §A–§D** (Code assigns MON numbers) |
