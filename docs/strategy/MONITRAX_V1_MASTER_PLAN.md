...
| **Last session** | 2026-08-19 · Code (Fable 5 + Opus ×2) · **M2 PR-3**: depth sweep (49 findings) → 11 fixed / MON-168…179 registered / census correction (the D-12 pack was unscoped — M2.5 blocked on its fix PR) · CI playwright hang diagnosed → #1597 hardening |
| **Next action** | 🧑 Reza merges #1597 (CI) → #1594 → #1595 → PR-3, and rules on the reports-page legacy tiles (hide now per D-4, or keep until M3.1?); 🟦 next fix PR = MON-168/169/170 (the pack) with expected movements; 🟩 Matrix: MON-161 flip re-check + `RING3_M2_PACK_CANONICAL.md` (incl. its scope-correction LIVE-CHECKs) |
| **Blockers** | none |
...
| MON-168 | **NEW (M2.6 sweep):** reconcile→link never stamps `propertyId` — the D-12 pack's per-property P&L can be EMPTY on a fully-linked year | OPEN → next fix PR (changesNumbers YES; LIVE-CHECK 1 baselines it) |
| MON-169 | **NEW (M2.6 sweep):** the pack counts transfers + loan repayments as income/expense | OPEN → next fix PR (changesNumbers YES) |
| MON-170 | **NEW (M2.6 sweep):** pack ATO-label totals silently drop unmapped rows | OPEN → next fix PR |
