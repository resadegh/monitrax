...
| `/api/tax/entity/[entityId]` | POST | 41e.1 slice D-1, D-2 | `tax_data.read` | Same response shape as GET, accepts a JSON body to drive slice-specific dispatch paths. Body fields: `trustDistribution` (D-1 — Div 6 allocation for TRUST entities), `cgtEvents` + `carryForwardCapitalLosses` (D-2 — CGT calc with entity-aware Div 115 discount + s100-50 loss netting; populates `entityPosition.cgtResult` independent of `result` so a COMPANY entity can surface real CGT figures even with income tax still UNCOMPUTED). Caller must own the entity. |
