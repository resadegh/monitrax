...
| `/api/tax/entity/[entityId]/smsf-return` | GET | 44.2 | `tax_data.read` | Returns the persisted `SmsfAnnualReturn` for the entity + `?fy=` (prefill). 404 unless the entity is a `LegalEntity(type=SMSF)` owned by the caller. |
| `/api/tax/entity/[entityId]/smsf-return` | PUT | 44.2 | `tax_data.write` | Upserts the SMSF's declared fund-tax figures (assessable investment income / deductions / contributions / NALI / `frankingCredits` / `isComplying` / `isInPensionPhase` / `ecpiExemptProportion`) keyed by entity+FY. Ownership-guarded; the exclusive writer of `smsf_annual_returns`. Feeds `assembleEntityTaxFacts` so the GET entity-tax path returns a real number. Audited (`SMSF_RETURN_SAVED`, no financial values in metadata). |
