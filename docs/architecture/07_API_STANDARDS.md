...
| `/api/tax/entity/[entityId]` | POST | 41e.1, 41e.2 | `tax_data.read` | Same response shape as GET, accepts a JSON body to drive slice-specific dispatch paths. Body fields: `trustDistribution` (Div 6 allocation for TRUST entities), `cgtEvents` + `carryForwardCapitalLosses` (CGT calc with entity-aware Div 115 discount + s100-50 loss netting), `smsfContributions` (41e.2 — concessional + non-concessional cap tracking with carry/bring-forward). Caller must own the entity. |
