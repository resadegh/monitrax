...
- **Slice D-2 (queued):** Wire `cgtDiscount` + `capitalLossNetting` into `entityTaxRouter` for any entity type with `cgtEvents`. Full Phase 41e.1 closure. After D-2, **41e.1 is COMPLETE** and **41e.2 (SMSF contribution caps)** starts.

After 41e.1, the audit's "never false numbers" principle relaxes for the v1-supported scope: TRUST entities with distribution data + COMPANY/individual entities with disposal events return computed figures, paired with the boundary footer citing exactly which sections were applied.
