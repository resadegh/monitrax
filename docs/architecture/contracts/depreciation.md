...
| lib/depreciation/div40.ts / div43.ts / schedule.ts | **DIFFERENT-QUANTITY (orphaned)** | effective-life + FY-aligned + days-held engines taking **fraction** rates (div43.ts:52-53 `0.025`); no importers outside `lib/depreciation/` (schedule.ts is their only consumer; nothing imports schedule.ts). A genuinely FY-based quantity definition — currently dead code |
...
4. `annualDeduction ≤ cost` and `totalDepreciationClaimed ≤ cost`, always.
