...
| `components/onboarding/wizard/steps/PropertiesStep.tsx:809` (:818–820) | **DUPLICATE** | wizard summary `totalEquity = totalValue − Σ p.loan.principal` re-derives (attached-loan basis, over unsaved draft rows) |
| `components/onboarding/wizard/types.ts:876` (:880–900) | **RE-TAGGED (§7 review): NOT property equity** | computes a wizard-summary **NET WORTH** (:893–900: properties + accounts + investments incl. cash + super + personal assets − loans − other debts). No equity computation exists anywhere in types.ts (grep `equity` = 0 hits). Belongs to the net-worth family — see `net-worth.md` §7 review |
...
- Duplicate collapses ((:154, :713, :767, financialAdvisor, wizard PropertiesStep — §7: wizard types.ts re-tagged out, it computes net worth not equity) → `calculateEquity`): **NO numeric movement** — all five compute value − attached-loans already; this is a pure producer-count drop (11 → target ≤ 4: two canonical homes + Decimal + the renamed different-quantity). Any move flags a hidden input difference (e.g. financialAdvisor's alias fallbacks) — investigate, don't absorb.
...
- Claims checked: 21 (anchors 14 · arithmetic 5 · negative-claims 2)
- REFUTED / CORRECTED:
  - **`components/onboarding/wizard/types.ts:876 (:880–891)` claimed "DUPLICATE — wizard summary totalValue − totalDebt re-derives [property equity]" → REFUTED.** The arithmetic at :880–900 computes a wizard-summary **net worth** (properties + accounts + investments-incl-cash + super + personal assets − loans − other debts, netWorth assignment :893–900); `grep -n equity types.ts` returns **zero** hits — there is no property-equity computation in the file. Row re-tagged in the body; the site is recorded in `net-worth.md` §7 as a census-missed net-worth-family producer. The PropertiesStep.tsx:818–820 half of the former "×2" IS a genuine property-equity re-derivation (verified: `totalEquity = totalValue − Σ p.loan.principal` at :818–820) and keeps its DUPLICATE tag. Consequence for the header verdict arithmetic: "5 inline duplicates" remains 5 (:154, :713, :767, financialAdvisor, PropertiesStep) only because types.ts leaves the family — the census's 11-site count for `propertyEquity` includes one mis-filed site.
- Verified intact (no drift): `lib/utils/calculations.ts:33` signed `calculateEquity` (MON-011) · `netWorthCalculator.ts:261/:452` aggregate + Decimal · `properties/page.tsx:462–464` canonical import, `:491–492` RENTAL filter + ":they're not owned" comment, `:494` sum, **`:812–813` RENTAL `purchasePrice: 0` / `currentValue: 0` forcing — EXACT** (form-level: fires on type-select; value inputs are hidden for RENTAL at :894/:963, so form-created RENTAL rows carry 0) · `[id]/page.tsx:154` inline duplicate EXACT · `snapshot/route.ts:713` inline equity EXACT · `portfolioEngine.ts:757/:767` EXACT · `financialAdvisor.ts:434/:447–448` alias-fallback equity EXACT · `metricAggregation.ts:390/:396` attached-loans aggregate EXACT.
- Could not verify: `lib/testing/exporter.ts:76` body (role-tagged, as the contract itself discloses). Design-record nuance: D10 is recorded "✅ RECOMMENDED (Matrix)" in `REFERENCE_NUMBERS_DESIGN.md:158`, not Reza-DECIDED; the brief §3.1(5) declares it a settled input, which this contract follows — noted for the gate pack.
- Verdict impact: **none** — MULTIPLE verdict, both canonical homes, the D10 finding and the RENTAL-zero-value expectedMove all stand. One callSite re-tagged out of the family (above).
