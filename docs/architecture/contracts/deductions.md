...
| `lib/tax-engine/divisions/negativeGearing.ts:152,346` | C | consumes property deductions vs rental income for regime gating — **DORMANT: zero production callers at HEAD** (adversarial note 2026-07-29, consistent with negative-gearing contract) |
| `app/api/portfolio/snapshot/route.ts:968-970` | **D (found by adversarial pass 2026-07-29)** | `deductibleExpenses = Σ isTaxDeductible toAnnual(amount)` feeding `estimatedTaxableIncome` (`:1090-1091`) in the snapshot TAX EXPOSURE block — no dep, no auto interest, no one-off gate. A live parallel deductions aggregate |
| `lib/reports/generators/incomeExpense.ts:21` | **D (found by adversarial pass 2026-07-29)** | second report generator: `deductibleExpenses = Σ isDeductible annualAmount`, rendered at `:65` — same class as the taxTime D |
| `app/dashboard/tax/page.tsx:749` | **D-shape surface (found by adversarial pass 2026-07-29)** | inline `deductions.total × (marginalRate/100)` rendered as "Tax savings at X% marginal rate" — linear re-derivation where canonical `calculateDeductionSavings` exists |
