...
3. Build command executes (`vercel-build` script in `package.json`):
   ...
   - `npm run lint:source-lock` (Calc-SSOT Wall A1 gate, added 2026-07-17 — CLAUDE.md §12.2.1 / `docs/architecture/MATRIX_FIX_DISCIPLINE.md` gates 2–3; fails the build when any `app/**/page.tsx` or API route reads RAW income/expense/loan rows instead of a canonical producer: inline `toMonthly/toAnnual(row.amount, row.frequency)`, raw `minRepayment` cost reads, or `.reduce`-sums over raw arrays. Exceptions are the tracked debt list `.audit/source-lock-exceptions.json` — line-free `{file, pattern, count}` entries, RATCHET-DOWN-ONLY: a count that is too high (new bypass) OR too low (paid-down debt not decremented) fails the build)
