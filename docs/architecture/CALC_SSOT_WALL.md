...
1. **Canonical upsert-by-signature at intake (kills Mechanism A).** ✅ **BUILT 2026-07-19 (Mechanism-A keystone PR — MON-084/085/074/076).** `classifyIntake` gained the `source-signature` stream policy (identity = kind + type + normalised name + ownerEntityId over USER-WIDE candidates) with the **scope-compatibility rule** — a candidate and a row converge only on the SAME scope or when at least one side is scopeless (General); two DIFFERENTLY-scoped rows (QBE on property A vs B; Lot-1 vs Lot-2 rent) never converge (the Reza distinct-sources correction, enforced structurally). Routed through it: the link-route income create branch (non-rental — reuse takes the `:831` update template), the link-route expense branch (cross-scope tier), `reconcileSuggestedAction` (income cross-amount; expense stays amount-bounded by doctrine), `POST /api/income` (409 `DUPLICATE_INCOME_SOURCE` on an exact manual duplicate; rental scope-singleton converges — the wizard's SSOT boundary), and onboarding complete (idempotent skip). Ratchet: `tests/golden/ring2.mechanismA.intakeDedup.test.ts` (real route: Ingeus update-not-mint · new source still mints · over-merge guard · battery cross-scope convergence). Neomatrix: `engine.intake.classifyIntake` + `law.intake.oneRowPerSource`. Existing already-minted duplicates are Part 2 (Reza-gated preview-and-confirm merge — §12.11; never auto-run). Extend the intake classifier (`lib/intake/classifyIntake.ts`, the MON-078 keystone) with a **source-signature reuse policy** = `(type, normalised name/employer/merchant, ownerEntityId)` — NOT scoped to `propertyId`. Route the link-route income `create` branch, the doc-import income branch, and the expense cross-scope case through it so reconciliation **updates the canonical row** (the `update` action template at `link/route.ts:831-943`, preserving `budgetedAmount`+`lastReconciled`) instead of inserting. One row ⇒ editing it propagates everywhere by construction.
