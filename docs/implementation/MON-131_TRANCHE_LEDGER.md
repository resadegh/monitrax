...
### Tranche 2 — loan cost (MON-130) — VERIFYING (G8 ✅ passed at VR-047 + VR-047B; G7 half — the whole-tree sweep is the one call left)
...
| G7 baseline diff | 🟡 **HALF** | **The 15 declared paths are verified live** (VR-047B A2, `c485b05` — every after-value landed, including both exactness traps). **The whole-tree `MOVED-UNDECLARED` sweep has NOT run.** These are two different instruments and the distinction is the gate: `GET /golden-baseline/t2-loan-cost` re-runs the T2 blocks only, so it can confirm the declared paths but is structurally blind to a move in the health / CFO / risk / reports subtrees; the three-outcome verdict G7 actually names comes from `POST /golden-baseline/diff` with `.audit/expected-moves-t2.json` as `expectedMoves` and the post-T1 reference (`347006b9…` at `3cdaa8c4`) as `baseline`. That call is still runnable and is the one action left on this gate. **Declaration amended to 15 paths BEFORE the migration** (opening commit): `debt.summary.totalRepayments` + `.byType.*.repayments` were an unswept sibling of a swept block, moving by construction via `quickMetrics.monthlyLoanRepayments`. The relay's sweep now diffs `snapshot.debt` whole |
...
| G11 issues closed on own evidence | 🟡 **HALF** | **MON-143 → VERIFIED** on its own numbers: its scope is the interest floor netting the offset, and Guildford's floor reads 384.45 against a 303,889.96 offset with no per-loan cost moved — the live evidence §23.2.3 requires and CI could never supply. **MON-130 stays `FIXING`, deliberately.** Its kept half is verified — `masterFinancialService` is the producer the property pages read, and all four of its expressions now agree — but MON-130's registered scope is *twelve* producers reading raw `minRepayment`, and **thirty sites remain**. Under Reza's Lever 2 those feed surfaces that are being hidden, which makes them out of **v1 scope** and does **not** make them **fixed**. Narrowing the issue to its kept surface is a scope call for Reza; closing it on this evidence would be the over-claim §22.2.4 exists to prevent |
| G1, G4 | — | |
