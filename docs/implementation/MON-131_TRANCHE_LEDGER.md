...
### Tranche 2 — loan cost (MON-130) — IN BUILD (migration merged for `masterFinancialService`; awaiting G7/G8)
...
| G7 baseline diff | ⏳ | Pending the post-merge relay run. **Declaration amended to 15 paths BEFORE the migration** (opening commit): `debt.summary.totalRepayments` + `.byType.*.repayments` were an unswept sibling of a swept block, moving by construction via `quickMetrics.monthlyLoanRepayments`. The relay's sweep now diffs `snapshot.debt` whole |
| G8 Ring-3 | 🟡 **HALF** | **VR-047 PASS — SCOPED** (`docs/verification/runs/VR-047.md`, `matrix:check` exit 0). The deciding row landed: Home's budget tile reads **$12,779**, matching `/dashboard/expenses` — two screens, one number, for the first time. Downstream figures reconcile to their derivations ($11,085 Saved · $358/day · 43.734% · 50.4183%); the regression cluster is byte-identical **including `healthScore` 53**, which the migration PR predicted by computing the clamp rather than hoping. **But §2, §2b and the build precondition were NOT run** — all three need the admin relay, and the account-first law forbids opening it in the same browser profile. So the four-expression identity (all four leaves reading 12,779.29 from different assembly paths) is **untested**, and a correct tile does not establish it. **MON-130 / MON-143 stay `FIXING`.** The admin half is `docs/verification/briefs/MATRIX_T2_ADMIN_RELAY.md`. Original handout:  `docs/verification/briefs/RING3_T2_LOAN_COST.md`** (§3.0b — a build section is done when the instrument for checking it exists). Build precondition, identity assertion (`loanCount === 5`, net worth $3,401,782), 15 falsifiable predictions, the `mustNotMove` cluster, the pre-declared non-findings, and a `matrix-result/v1` return block |
