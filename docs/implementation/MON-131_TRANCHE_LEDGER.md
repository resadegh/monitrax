...
### Tranche 2 — loan cost (MON-130) — DONE-WITH-A-NAMED-GAP (G8/G10/G11 ✅; G7 permanently HALF — the pre-T2 reference tree never existed, MON-157. Settled by Reza 2026-08-04; not to be re-litigated)
...
| G7 baseline diff | 🟡 **HALF — permanently. Do not re-litigate.** | **Reza, 2026-08-04:** *"T2's G7 stays HALF so it isn't re-litigated: this capture establishes the reference for T3, not a close for T2."* **The 15 declared paths ARE verified live** (VR-047B A2). **The whole-tree `MOVED-UNDECLARED` sweep can never be run for T2**, and the reason is structural: G7's verdict comes from diffing the PRE-change tree against a fresh one, T2's pre-change tree never existed as a file (only its hash), and it cannot be re-captured — the code has changed and the live data has moved on. **No artefact captured today can answer a question about a state that is gone.** Registered as **MON-157** (§21.2.2 rule-4 — the instrument's reference lived in a session, not the repo; it survived four tranches because a hash suffices for the CLEAN case and nothing hit the localisation case until a tranche moved numbers on purpose). **FIXED FORWARD, and only forward:** VR-048 captured the tree at `12954fff` and it is committed at `.audit/golden-baseline-12954ff.json` — the first golden-baseline tree ever committed to this repo — with a Ratchet (`tests/matrix/goldenBaselineReference.test.ts`) that fails the build if it goes missing again. **T3's G7 runs as designed.** |
...
| 08-03 | #1580 | `e3a3715` | **VR-047B consumed — G8 CLOSED, and two findings acted on.** The producer half of the T2 Ring-3 came back from the second Chrome profile and PART A passed in full: build precondition `paths: []`, all 15 declared paths landed with both exactness traps (`annualLoanRepayments` **153,351.51** not …48; `annualCashflow` **133,020.78** not .79), and **the four-expression identity HOLDS** (three leaves byte-equal at `12,779.292814353912`, the fourth that same value rounded at the producer). `byType` keys unchanged and summing bit-identical to the total. **G8 ✅** on VR-047 + VR-047B together — neither half was sufficient alone, which is why both ran. **MON-143 → VERIFIED** (Guildford floor 384.45 against a 303,889.96 offset; no per-loan cost moved — the live evidence §23.2.3 requires and CI could never supply). **Finding 2 fixed, after verifying its mechanism in source rather than trusting the report:** the relay compared `cf.monthlyLoanRepayments` (rounded by the orchestrator) against the unrounded canonical sum — not, as reported, "annualised the rounded monthly". Pre-migration the 0.0028 residue was invisible against a $3,962.64 delta; post-migration both arms are the same producer, so it *is* the signal (`0.00` monthly, `0.03` annually). Now reads `debt.summary.totalRepayments`, comparing like with like. **MON-154** raised (one quantity at two precisions in one snapshot) and **MON-155** (`lib/admin/auth.ts:501` reads an `admin_session` cookie nothing writes — tagged `@deprecated`). **An earlier VR-047B is withdrawn in full:** it returned FAIL on `INSTRUMENT_UNREACHABLE` and recommended an auth change, having called the relay by address-bar navigation instead of a page-context fetch. Reza refused the diagnosis (*"you never had this issue in the past month"*) and the transcript settled it — all 39 successful relay calls used the fetch form. No code change was made. All three T2 handouts now state the fetch form explicitly. **MON-130 deliberately stays `FIXING`** — its kept half is verified, but its scope is twelve producers and thirty sites remain; Lever 2 hides those surfaces, which is out of v1 scope, not fixed | **No — record + relay repair only.** `loanCost` still 30 |
| 08-04 | #1581 | `e5017ba` | **D50 answered (option A) — MON-130 narrowed and VERIFIED, its residue carried as MON-156 — and G7 turns out to be unclosable for T2 (MON-157).** **D50-A:** MON-130's title and `rootCause` now name the ONE producer #1575 migrated (`masterFinancialService`); the other **11 producers / ~30 raw-read sites** become **MON-156** with the producer list copied verbatim and a re-verify-before-fixing caveat on the line numbers. The reasoning is recorded on both entries so it cannot be re-litigated: Lever 2 hides the surfaces those eleven feed, which is an EXPOSURE control, not a DEFECT control — folding them into a VERIFIED issue would have said they were fixed. **G11 ✅**. **MON-157 — the one that matters beyond T2.** Writing the G7 handout Reza asked for exposed that it cannot be run. `diffBaselines()` needs a TREE on the old side; what T1 left is a HASH (`347006b9…`, a string in VR-045's prose), and `git log --all` over `.audit/golden-baseline*` returns NOTHING — no tree has ever been committed, on any branch, ever, despite the CLI writing one and printing 'COMMIT IT'. The pre-T2 tree cannot be re-captured (code changed, live data moved on), so **T2's G7 stands at HALF permanently** and every prior tranche's G7 rested on either a CLEAN hash match or the declared-path check alone. §21.2.2 rule-4 failure: the reference lived in a session, not the repo. **The handout shipped is therefore a different one** — `MATRIX_G7_REFERENCE_CAPTURE.md` commits the full ~282 KB tree at the current commit (hash-verified on reassembly) and self-diffs it to prove the capture→serialise→POST→flatten→verdict chain end to end before T3 depends on it. **T3's G7 becomes runnable as designed.** Also: **MON-155** `changesNumbers` corrected true → false (auto-raise default; a dead auth branch produces no number, and leaving it true would have demanded a propagation test before it could ever be verified) | **No — records + one handout.** `loanCost` still 30 |
| 08-04 | #1582 | `e90a9195` | **VR-048 consumed — the FIRST golden-baseline tree ever committed, and T2's G7 settled as permanently HALF.** The Matrix captured at `12954fff`: 8 trees, `captureErrors: []`, **1,755 hashed leaves + 1 volatile = 1,756**, `treeHash 0d6753ef…`. Committed at `.audit/golden-baseline-12954ff.json` in the CLI's own document shape so `--diff` reads it unadapted. **Integrity verified independently rather than accepted:** the reassembled tree was re-hashed with the REAL `hashBaseline()` and matches exactly — which proves more than 'the paste survived', because Call 1 and Call 2 were SEPARATE server-side captures, so the match also proves they were numerically identical. The committed file then ran back through the real `diffBaselines` locally: **CLEAN, unchanged=1756**, matching the Matrix's server-side self-diff figure. Call 3 (`POST /diff`, `expectedMoves: []`) returned **CLEAN** — the capture→serialise→POST→flatten→verdict chain proven end to end BEFORE T3 depends on it. **The 1,755/1,756 pair is not a discrepancy** — `hashBaseline` excludes volatile leaves, `diffBaselines` counts them; it reconciles exactly. Checked, because two counts differing by one is also what a silently-dropped leaf looks like. **RATCHET (§23.2.2):** a `describe` block in `tests/matrix/goldenBaselineRelay.test.ts` (the suite that already covers this module — §22.2 rule 2, never a parallel silo) — fails the build with no committed reference, without the CLI's shape, with the `captureErrors` tripwire lit, on a treeHash mismatch, or if it fails to diff CLEAN. **Negative control run:** reference moved aside → fails on *'No `.audit/golden-baseline-<sha>.json` is committed'*; restored → passes. A ratchet not seen to fail is not a ratchet. **MON-159 raised, and it is the session's own near-miss:** the full suite aborts intermittently with a Prisma query-engine panic (`engine.rs:74`, exit 134). It was blamed on the new ratchet TWICE and restructured twice before six runs with the block ABSENT measured 2 failures in 6 — pre-existing, and the change was innocent. Filed with the signature, the measurement, an empty `rootCause` (MON-147 precedent — no verified line, so no invented anchor), and the reason it matters: a red build landing somewhere different each time is how sessions learn to stop reading failures. **MON-157 → VERIFIED** (fixed FORWARD; T2's G7 explicitly NOT reopened). **MON-158** raised — `riskRadar.ts:605` mints `crypto.randomUUID()` + `new Date()` per scan, so risk identities are unstable across runs; verified in source, 19 differing leaves ALL non-numeric, zero numeric, no verdict impact, nil impact today (nothing persists a risk by id) but it forecloses any future 'dismiss this risk' feature | **No — reference artefact + ratchet.** `loanCost` still 30 |

| 08-19 | #1590 | `ba10e451` | **PROD-Simplification P2-gate PR touches two MON-131 instruments — record only.** (1) **Producer-census reseed:** `deductions` 105 → 104. NOT a producer deletion — a phantom TEXT match removed: the P2.3 reports narrowing deleted the page's hardcoded duplicate help list (§12.2.1) and one prose line had been matching the text-based deductions pattern — the exact phantom class the 2026-08-03 seed entry records. Honest note written into the seed history entry; `REFERENCE_NUMBERS_SCOREBOARD.md` regenerated (it renders the census). (2) **Registry movement:** MON-160 registered → FIXING (module gates baked at BUILD time — statically pre-rendered gated layouts froze the flag verdict; fix = `await connection()` in `moduleRouteGuard` before the flag read, order-locked by test; `changesNumbers: NO`). Also backfilled two overdue SHAs above (#1566 `b733c82`, #1582 `e90a9195`) per the §6 convention. **No §3 gate cell moves** — instrument reseed + registry row only; no engine, contract, or tranche state touched | **No — instrument reseed + registry rows.** `loanCost` still 30 |
| 08-19 | #1594 | *(this PR)* | **M2 PR-1 (V1 master plan §4) — registry movement only.** MON-160 FIXING → **VERIFIED** (live M1.5 flip test on PROD, #1591 comment — request-time gating held with no redeploy). MON-161 raised → FIXING on #1594 (gated-route 404s were cacheable; fix = registry-derived `Cache-Control: no-store` at the middleware chokepoint, locked by test). MON-162 raised → OPEN (admin/app shared browser session; fix DEFERRED by Reza P-3/P-5). MON-163 raised → FIXING on #1594 (kept surfaces linked into hidden modules — 8 files/14 sites gated + the permanent kept-reachable dead-link guard in CI). All `changesNumbers: NO`. **No §3 gate cell moves** — registry rows only; no engine, contract, census or tranche state touched. Also backfilled #1590's SHA above (`ba10e451`) per the §6 convention | **No — registry rows only.** `loanCost` still 30 |
| 08-19 | #1595 | *(this PR, 2nd row)* | **M2 PR-2 §B main slice — census reseed #2 + registry movement.** Producer deletions: `incomeRunRate` 125→123 · `expenseRunRate` 78→76 · `depreciation` 15→12 · `deductions` 104→103 (pack fetchers → canonical producers; dep page + both pack fetchers → `calculateDepreciationAnnual`; entityBreakdown + propertyCashflow one-off gates). Source-lock `RAW_FREQ_RUN_RATE` exceptions ratcheted: entityBreakdown entry DELETED, propertyCashflow 3→2, contextBuilder 2→1. Registry: MON-164/165/166/167 raised → FIXING on #1595; MON-129 kept-slice note; MON-145 → DIAGNOSED (schema-bound, Reza decision); MON-001 re-cut note; MON-146 HELD note. `changesNumbers: YES` — expected movements in #1595 + `RING3_M2_PACK_CANONICAL.md`; VERIFIED gates on the Matrix Ring-3 (M2.2). **No §3 gate cell moves** | **No §3 moves — M2 slice, not a tranche.** `loanCost` still 30 |
| 08-19 | #1595 | *(this PR)* | **M2 PR-2 B-3 census reseed — three counts FELL, zero rose.** The properties-dialog SSOT fix (both inline budget/actual blocks deleted, dialog now reads `computePropertyCashflow`) removed inline producers: `incomeRunRate` 126→125 · `expenseRunRate` 79→78 · `cashflow` 57→55. `loanCost` stays 30 — a transient 30→31 during the build was a PHANTOM (the census splits function units on `const X = () =>`; a new display helper had swallowed the loans tab's pre-existing raw-`minRepayment` displays into a new unit) — fixed by inlining the helper, NOT by reseeding the rise. Source-lock `RAW_MIN_REPAYMENT_COST` exception ratcheted 4→3 in the same fix. `REFERENCE_NUMBERS_SCOREBOARD.md` regenerated. **No §3 gate cell moves** — instrument reseed only | **No — reseed locks three falls.** `loanCost` still 30 |
| 08-19 | #1598 | *(this PR)* | **M2 PR-3 §E depth sweep — registry movement + handout amendment, no census/engine change.** Registry: MON-168/169/170 raised → OPEN (the D-12 pack trio: propertyId never stamped by link routes · isTransfer counted · unmapped rows dropped — queued as the next fix PR with expected movements); MON-171…179 raised → OPEN per the sweep catalogue (`docs/strategy/M2_DEPTH_SWEEP_CATALOGUE.md`). Handout `RING3_M2_PACK_CANONICAL.md` amended with the SCOPE CORRECTION (the real D-12 pack `buildTaxPackSummary` was OUT of the M2.1 census — M2.5 must not close on that handout alone). PR-3 fixes are navigation/gating/error-states/idempotency only, `changesNumbers: NO`; no producer added or deleted, census counts untouched. **No §3 gate cell moves** — registry rows + brief scope-correction only | **No — registry rows only.** `loanCost` still 30 |
| 08-19 | #1601 | *(this PR)* | **M3 PR-1 — the D-12 pack fix (MON-168/169/170), registry movement + ONE new engine family modelled.** Forward fix: 17 link-write sites (16 from the census + `lib/intake/duplicateMerge.ts` caught by the NEW Ring-1 guard test) now stamp `UnifiedTransaction.propertyId` via the ONE resolver `lib/bookkeeping/propertyLink.ts`; 5 clear-sites null it; the PATCH editor derives it. Admin backfill route (dry-run default, §12.11, idempotent). `buildTaxPackSummary` rewritten property-scoped: transfers + loan repayments + non-property rows into COUNTED exclusion buckets; identity `included + Σexcluded = total` HARD-ASSERTED + printed in XLSX/PDF; label loop de-N+1'd. Census note: a `bucket` arrow helper minted a phantom `insuranceAdequacy` 15→16 (the PR-2 function-unit class) — fixed by INLINING + renaming `labelCoverage`→`atoLabelling` (the word 'coverage' matched the insurance signature); ratchet green, no reseed. Neomatrix: D-12 pack modelled (engine + 2 semanticKey'd numbers + identity law + export surface + resolver engine, 6 nodes/edges) — closing the census-correction blind spot. `changesNumbers: YES` — expected movements written FIRST in `RING3_M3_PACK_FIX.md` (D-21); VERIFIED gates on its Ring-3 run. **No §3 gate cell moves** | **No — pack scope, not a tranche.** `loanCost` still 30 |
| 08-22 | #1605 | *(this PR)* | **M3 punch-list PR-1 (brief `BRIEF_M3_PUNCHLIST_AND_CLOSEOUT.md`) — registry movement + scoreboard/reports fixes, no census/engine change.** Registry: **MON-168/169/170 → VERIFIED** citing the Ring-3 PASS verdict on #1601 (2026-08-22: backfill 293/46/idempotent-0; identity 35+39+0+313=387 exact to the cent; perProperty 4 entries; property tiles byte-identical). **MON-180…186 registered** per brief §A: 180 EOFY-tile misleading empty state (§C-1, FIXING #1605) · 181 intake-queue dead fetch (§C-2, fixed here) · 182 two portfolio-LVR producers 41.3% vs 40.8% (§C-3, DIAGNOSED — fix is punch-list PR-2, D-21) · 183 strip slice(0,4) no rule (§C-4, fixed here) · 184 pack ATO labelling zero rows (§B, DIAGNOSED — fix is punch-list PR-2, D-21) · 185 DATA duplicates/stray/orphans (REGISTER-ONLY, Reza runbook in the issue body, NEVER auto-fix) · 186 MODULE_HOME re-key semantics (§D, fixed here: Financial Overview→MODULE_CFO, /api/money-flow→MODULE_HOUSEHOLD, Tax-Time tile→MODULE_TAX, expected-map guard test). All PR-1 fixes `changesNumbers: NO` (display windows, fetch params, render set/order, gate keys — no producer touched). **No §3 gate cell moves** — registry rows + kept-surface display fixes only | **No — registry rows only.** `loanCost` still 30 |
| 08-22 | #1606 | *(this PR)* | **M3 punch-list PR-2 — M2.5 CLOSED + one producer-family consolidation, D-21 honoured.** **M2.5 closed with per-condition evidence** (plan M2.5 box): census AT seed · source-lock/surface-linter/A3 green · expectedMoves landed (VR-045 · VR-047/047B · #1601 to the cent) · Ring-3 PASS 2026-08-22; condition 5 (the complete Matrix sweep) explicitly NOT claimed — it closes MON-131 the programme. **MON-182 → FIXING #1606:** `lib/calculations/portfolioLvr.ts` becomes THE owned-portfolio-LVR producer; the snapshot's all-liabilities inline basis + the properties page's screen arithmetic DELETED; `lvrGearing` census back AT seed (+1 lib producer offset by removing the `const lvr = calculateLVR(...)` text-match phantom — renamed, never reseeded). **MON-184 → FIXING #1606:** hierarchy fallback at the ONE pack lookup + enum seed vocabulary; totals byte-identical by contract (`RING3_M3_PUNCH_FIXES.md`, committed BEFORE the fix code). Neomatrix: engine + semanticKey'd number + hero surface modelled; 4 anchored hashes re-pinned. **No §3 gate cell moves** — M2/M3 punch scope, not a tranche | **No — census stays at seed.** `loanCost` still 30 |
| 08-25 | m2-depth PR-1 | *(this PR)* | **M2 kept-depth PR-1 (BRIEF_M2_KEPT_DEPTH §PR-1) — the intake pipeline made real + F1–F10 registered, no census/engine change.** Registry: **MON-180…184/186 → VERIFIED** citing the RING3_M3_PUNCH_FIXES PASS (#1606 comments — the outstanding Code duty from the cursor); **MON-187…196 registered** from the P-9 live-sweep findings (envelope on #1613); 187/188/189/190/191/193/194 fixed in this PR (→ FIXING with the PR number in the follow-up commit); 192/195 OPEN and 196 DIAGNOSED for PR-3. Root causes all verified in source before code: F1 = the component read suggestedAction `.type` while the analyzers store `.action` (every checkbox born disabled) + a second latent leg (wrapped extractedData spread into a flat-reading confirm route); F2 = Done was a local close; F4 = the tree total summed the category AND tax buckets (2× by construction); F5 = first-plausible-line vendor fallback let a garbled doc-type word beat the merchant; F7 = five independent 10MB constants over a ~4.5MB platform cap; F8 = raw res.json() on non-JSON error bodies. Fixes: pure analyzer-typed `inboxModel` + flattened approve payload · PATCH persist on the existing analyze route (§12.11 answers in the PR) · ONE document total · ONE 4MB limit constant · ONE guarded error reader · doc-type-word rejection + prominence scoring in extractVendor. Also restores the #1579 row's dropped final clause above (the #1613 transcription delta the Matrix flagged). `changesNumbers: NO` — no producer touched; Ring-3 = the brief's PR-1 predictions (the stranded Bunnings item edited/selected/approved into FY2026-27). **No §3 gate cell moves** | **No — registry rows + intake plumbing.** `loanCost` still 30 |
| 08-25 | #1612 | *(merged)* | **BACKFILLED ROW (recorded via #1613 — #1612 shipped without its ledger row; the docs-hygiene gate caught the class on the next PR).** **The re-scoped T2 kept-path Ring-3** (plan M2.2 / gate-review P-6): handout `docs/verification/briefs/RING3_T2_KEPT_LOAN_COST.md` committed BEFORE the run, run account-first on Reza's data. **8/8 checks PASS, verdict PARTIAL.** B1 CLEARED for the rendered cashflow number (−$2,790 ≡ canonical $2,789.71 — the ACTUALS path binds) and **reclassified LATENT** (`offsetBalance` FIELD_ABSENT in `/api/properties`; the HECS loan already shows `flooredToInterest: true`). **MON-151 CONFIRMED LIVE + QUANTIFIED:** property-detail Tax position card charges $23,576.09/yr vs canonical offset-netted $4,613.35/yr on Guildford — $18,962.73/yr overstated; implied offset works back to $303,889.96, matching MON-143's figure to the cent; Broadbeach no-offset control exact. The T2 gate table's G8 row is UNCHANGED — VR-047/047B's evidence stands for the leg it covered; this run covers the KEPT leg those runs never read | **No — verification record only.** `loanCost` still 30 |
| 08-25 | #1613 | *(this PR)* | **P-9 live sweep + P-10 brief (docs only).** Handout `docs/verification/briefs/RING3_M2_6_LIVE_SWEEP.md` committed BEFORE the run (D-21); 6 of 7 outstanding M2.6 LIVE-CHECK items executed account-first, verdict **PARTIAL** (envelope on #1613; S7 fresh-account deferred by Reza until the intake fix). **Ten findings F1–F10** — headline: the Smart Inbox intake pipeline is display-only (select/approve dead both viewports, edits revert on reload); plus upload 413@6.2MB vs the 10MB promise, raw JSON-parse error surface, vault tree count exactly 2×, OCR vendor mis-pick, $0-as-truth during real 503s, undismissable help drawer, basiq 403. **K6 correction:** the activity money-flow widget RENDERS on both viewports (no `/api/money-flow` call — a different producer feeds it); #1612's "no kept consumer" inference withdrawn. `BRIEF_M2_KEPT_DEPTH.md` cut: PR-1 intake pipeline first (Reza's blocker; MON ids for F1–F10 assigned there) · PR-2 MON-151 + offset thread · PR-3 display + sweep highs | **No — verification + brief records only.** `loanCost` still 30 |
---
