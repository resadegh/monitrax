...
**Last updated:** 2026-05-01 (end of day) — Reza + Claude (**EOD documentation pass.** Phase 37 (My Budget IA + premium redesign) and Phase 38 (My Vault — document management uplift) BOTH SHIPPED today across 8 merged PRs (#574, #575, #576, #577, #578) plus the supporting Session-expiry UX hardening, Home TRAIL banner v3 premium redesign, WIF Phases 9 + 10 closure, and dead-code audit pass. Both phases moved from Active → Recently Completed. Continuation tomorrow: My Guide simplification + Tax → Actions consolidation (Up Next #8), Onboarding wizard PR 3c (#7), Phase 11 WIF fallback removal (queued for ≥ 2026-05-31). All PRs merged; main is the truth.)
...
### ~~3. Phase 37 — My Budget IA simplification + premium redesign~~ ✅ COMPLETE 2026-05-01

Shipped via PR #574. 6 sub-tabs collapsed to 3 (Cashflow / My Plan / Debt Freedom). Default landing flipped from `/dashboard/budget-analysis` → `/cashflow`. Tax relocated to My Guide. New `/dashboard/plan` hub built. Cashflow + Debt Freedom heroes uplifted to Apple typography. **Full details below in `✅ Recently Completed`.**

PR 7 (telemetry + soft-retire of legacy routes) deferred to Up Next #9 — `/dashboard/income`, `/dashboard/expenses`, `/dashboard/budget-analysis` are still actively reached via "Manage all →" deep links from `/dashboard/plan`, so soft-retire isn't appropriate yet.

---

### ~~4. Phase 38 — My Vault (document management uplift)~~ ✅ COMPLETE 2026-05-01

Shipped via PRs #575, #576, #577, #578. Documents elevated from a sub-tab of Reports to its own top-level "My Vault" sidebar item. Apple-typography hero (FY counter), Smart Inbox surface for AI-analysed-but-unverified docs (one-tap confirm), accountant Share Pass (secure shareable link, time-limited, audit-logged, watermarked), folder view + toolbar redesign, universal upload migration to canonical Phase 25 DME, tax-status lens (Deductible / Non-deductible / Untagged) with FolderTree visual redesign. **Full details below in `✅ Recently Completed`.**

PR 4 (tax-status filter) was the originally-deferred lens; shipped in the final wrap-up PR (#578) along with PR 2.5 (upload migration) once the share architecture stabilised.
...
| 11 | `lib/cfo/trailStage.ts` (Phase 17 spec placeholder) | `lib/cfo/trailStage.ts` (96 lines) | Defines `getTrailStageInfo()`, `determineTrailStage()`, `actionCategoryToTrailStage()`, `isActionRelevantToStage()`. Zero callers. NOT re-exported from `lib/cfo/index.ts`. Found during dead-code audit 2026-05-01. | When Phase 17 (Personal CFO Engine) implementation begins — wire it up; OR delete after confirming Phase 17 is deprioritized. Low priority — only 96 lines. |
...
### 2026-05-01
- **Phase 38 — My Vault (document management uplift) — COMPLETE.** 4 merged PRs (#575, #576, #577, #578) shipped over the course of the evening. Reza brief: *"Documents is one of the most important pages of the app … these documents can be shared with the accountant for tax return activities and they should be very well structured and organised. Also, when a document is attached anywhere in the app, the document should be stored in My Vault, organised correctly, tagged with correct meta data."* Audit confirmed Phase 25 (Document Management Engine) and Phase 26 (Document Intelligence Engine) already implemented the canonical-upload-cascade architecture; Phase 38's job was the user-facing realisation, not new engines. **Hard constraints honoured: zero new calc engines, zero data duplication.**
  - **PR #575 (PR 1 — Vault IA + visual uplift + Smart Inbox count).** Sidebar split: "Reports" unbundled into two top-level items, "My Vault" (`Archive` icon) elevated between My Guide and Reports. Friendly `/dashboard/vault` alias redirects to canonical `/dashboard/documents`. Apple-typography hero (FY counter as primary number, glassmorphic 28px card, sequenced framer-motion entrance, full `prefers-reduced-motion`). Empowering supporting copy that morphs by state (empty / awaiting-review / all-caught-up). Smart Inbox glassmorphic card surfacing the count of AI-analysed-but-unverified docs.
  - **PR #576 (PR 2 — Smart Inbox interactive + universal upload audit).** Smart Inbox card became expandable (Apple-style chevron toggle, `AnimatePresence` height + opacity reveal). Per pending doc, one row shows: confidence dot (emerald ≥0.9 / amber ≥0.7 / rose else), document-type badge, AI-extracted summary line (e.g. *"Officeworks · $42.50 · 2025-03-12"*, formatted via new `summariseExtractedData` helper), original filename, and two actions: **Open** (existing signed-URL `/api/documents/[id]` endpoint) and **Confirm** (existing `/api/documents/analyze/confirm` — picks the highest-confidence suggested action). Universal upload audit completed: 2 of 3 codepaths route through Phase 25 DME canonically; ONE legacy bypass identified (`hooks/useDocumentUpload.ts` → `POST /api/documents` → legacy `documentService.uploadDocument()`); deferred refactor to PR 2.5 to keep PR 2 reviewable.
  - **PR #577 (PR 3 — Send to accountant Share Pass + folder view restyle).** New `SharePackage` Prisma model with polymorphic `contentRefs` (Json) + `contentType` enum + `deliveryMethod` enum so the engine extends to non-document content (Reports, property summaries, net-worth snapshots) and non-link delivery (future Xero/MYOB push) without future migrations. Migration `prisma/migrations/20260501112526_add_share_package/migration.sql` (CLAUDE.md §12.12). Six new API routes: `POST /api/share` (create — validates ownership of every documentId, generates 32-byte URL-safe token, builds watermark), `GET /api/share` (list owner's shares), `GET /api/share/:token` (PUBLIC read — token IS the credential, no auth, audit-logged under owner's userId), `GET /api/share/:token/download` (PUBLIC ZIP — reuses JSZip layout from existing `/api/documents/export`), `GET /api/share/:token/file/:docId` (PUBLIC single-file streaming), `DELETE /api/share/:token` (auth-required soft-revoke; sets `revokedAt`). Owner UI: `SendToAccountantDialog` modal on `/dashboard/documents` (purpose / recipient / expiry / structure → Download ZIP via existing endpoint OR Generate secure link). Recipient UI: minimal-aesthetic `/share/[token]` page (no app shell — logo + hero number + Download all + per-document list with per-file Open). Owner management: `/dashboard/settings/shares` page with copy-link, view counter, revoke (alert confirmation). Default 30-day expiry; picker for 7 / 30 / 90. CDR audit-logged via `createAuditLog()` with `entityType='SharePackage'` on every CREATE / READ / EXPORT / DELETE. Watermark auto-generated server-side: *"Generated from Monitrax · {FY} · {owner name}"*. Token = 32 random bytes → 43-char base64url (≈256 bits entropy) — enumeration computationally infeasible. **Plus** — folder view + toolbar redesign (Reza note: *"just to confirm you are redesigning the document folder view as well?"* The hero + Smart Inbox shipped earlier in PR 3 looked Apple-modern but the toolbar / breadcrumb / FolderTree sidebar / DocumentFolderView grid below were still old Card chrome. Toolbar Card → glass surface; folder header Card → glass; upload section Card → glass + framer-motion entrance; FolderTree sidebar wrapper → glass; DocumentFolderView list → `<motion.li>` with staggered entrance, refined typography, file icons in glass tiles; DocumentFolderView grid → `<motion.div>` glass cards with hover lift. Hit a Vercel build error mid-deploy: *"You cannot use different slug names for the same dynamic path ('id' !== 'token')."* — collapsed `[id]` route into `[token]` with same path/different HTTP method.).
  - **PR #578 (PR 2.5 + PR 4 + FolderTree restyle — final wrap-up).** **PR 2.5 universal upload migration** — `hooks/useDocumentUpload.ts` migrated from legacy `POST /api/documents` (which bypassed the RuleEngine) to canonical `POST /api/documents/upload` (Phase 25 DME). New `LINK_FIELD_BY_ENTITY` translation map + `buildDmeFormData()` helper convert the hook's `links: { entityType, entityId }[]` shape into DME's individual entity form fields (`propertyId`, `expenseId`, `loanId`, etc.). Hook-level fix only — no edits to the 2,031-LOC `ExpenseDialog` or expenses page. Result: every document uploaded anywhere in the app now flows through the canonical engine and lands in My Vault correctly tagged. Tech-debt row #12 closed. **PR 4 tax-status lens** — new "By Tax Status" root in `FolderTree` with three children: Deductible / Non-deductible / Untagged. Counts derived client-side from a new `expenseTaxMap: Map<expenseId, isTaxDeductible>` populated on page mount via `/api/expenses` (existing endpoint — no new API). Helper `getDocumentTaxStatus(doc)` buckets: at least one EXPENSE link with `isTaxDeductible === true` → DEDUCTIBLE; falls back to NON_DEDUCTIBLE if any false; UNTAGGED otherwise. Filter wires via existing `pathParts[0] === 'tax-status'` check in `filteredDocuments`. **FolderTree visual redesign** — addresses Reza note: *"I still don't see any changes in the folder structure design — was there any visual design changes?"* PR 3 only restyled the wrapping `<aside>`, leaving the tree internals untouched. PR #578 fixed every row: Apple typography (`tracking-[-0.01em]`), glass active-row (`bg-primary/8 border-primary/15`), tabular-nums count badges, top-level lenses rendered as uppercase tracked-out section headers (small caps with `tracking-[0.16em]`). Hit one Vercel build error mid-flight: TypeScript TDZ error because `getDocumentTaxStatus` referenced `expenseTaxMap` before declaration; fixed by hoisting both Phase 38 state declarations (`expenseTaxMap` + `showSendDialog`) up next to the other top-of-component `useState` calls.
- **Phase 37 — My Budget IA simplification + premium redesign — COMPLETE.** Shipped as mega-PR #574 on `claude/phase-37-final` (Reza requested a single PR URL after PR #571's smoke test). Bundled all of: PR 1 Sidebar IA (My Budget collapsed from 6 sub-tabs → 3: Cashflow / My Plan / Debt Freedom; default landing flipped from `/dashboard/budget-analysis` → `/cashflow`; Tax relocated to My Guide as 3rd sibling tab alongside Actions + Health), PR 2 Cashflow uplift (Apple-typography `CashflowHero` glassmorphic banner with warm-sentence headline that morphs by surplus/shortfall sign + atmospheric mesh-gradient that shifts colour with sign — *"Money working for you this month — build on it."* / *"Spending edged ahead this month — trim a leak below to close it."*), PR 3 New `/dashboard/plan` hub (Apple-style segmented control with sliding selection pill via shared `layoutId`, `AnimatePresence` cross-fades blur-in/blur-out 0.45s, hero glass card with morphing "$X in · $Y out · $Z" sentence, condensed summaries per section with "Manage all →" deep-links to existing detail pages — all data via `Promise.allSettled` from existing APIs), PR 4 Standalone routes preserved (pragmatic delivery: condensed-summary + deep-link instead of 5,000-LOC panel extraction; same IA outcome, materially less risk), PR 5 Debt Freedom uplift (`DebtFreedomHero` with aspirational *"Free by Oct 2031"* headline + 6s shimmer sweep over the date when computed, fallback warm sentences for budget-not-ready and plan-not-computed states, two stat pills for interest-saved + months-saved), PR 6 Tax tab under My Guide (sidebar move shipped as part of PR 1's `DashboardLayout.tsx` edit — page itself unchanged; future My Guide simplification session will fold non-duplicated tax data into Actions). PR 7 (telemetry + soft-retire) deferred to Up Next #9. Single Apple-typography refinement landed mid-flight after Reza's review: hero numbers became the primary visual (font-light, tracking-[-0.04em], color isolated to digits — Apple Wallet pattern), sentences demoted to muted supporting copy.
- **Session-expiry UX hardening.** Added `SessionExpiryHandler` (global `window.fetch` wrapper) that catches 401s from `/api/*` after the cached Firebase token has expired, force-refreshes the token + retries once, and falls back to `logout()` + redirect to `/signin?reason=session_expired&next=<path>` if the session is genuinely gone. Companion to the existing 30-min `IdleTimeoutGuard` (CLAUDE.md §13.5). Closes the gap where users idle <30 min, return to a stale token, and see broken pages instead of a clean re-auth prompt.
