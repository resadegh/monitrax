...
**Last updated:** 2026-05-04 — Reza + Claude (**Phase 36 Phase 2a (this PR) — inline `LoanDetailDialog` on `/dashboard/balances`.** New `components/loans/LoanDetailDialog.tsx` (~620 lines) extracted from `app/dashboard/loans/page.tsx` mirroring the `AccountDetailDialog` pattern: 6-tab structure (Overview / Property / Offset / Expenses / Strategy / Linked), canonical-SSOT calculations (`calculateEffectivePrincipal`, `calculateLVR`, `toAnnual`), AlertDialog confirm-delete sub-flow. Wired on the legacy loans page (parity replacement of the inline dialog) AND on Balances (new — loan rows now open the dialog instead of navigating away). `LoanFormDialog` also opens in edit mode from the Balances detail dialog via a new `editingLoan` state hook. Phase 2c + 2e were no-ops (already done in Phase 1c / sidebar `matchRoutes`); Phase 2b + 2d remain. Up Next #4 reflowed with sub-phase status. Build green. See CHANGELOG_2026_05_04.md.)
