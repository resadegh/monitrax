...
| 13 | Legacy GCP key **"Maps Platform API Key"** awaiting hard-delete | GCP Console → Credentials (project `monitrax-479700`), key id ends `…1428c87`, auto-created 10 Dec 2025 with no restrictions across 32 APIs | Soft-disabled 2026-05-02 by setting API restrictions to **only Cloud Resource Manager API** (a service Maps doesn't use → effectively kills all Maps calls without deleting the key, instant rollback if anything breaks). Audit confirmed: no Vercel env var references it; codebase grep returns no matches; Maps Platform metrics show no Maps traffic attributable to it. Reason for soft-disable rather than direct delete: 24-hour observation window in case a non-obvious caller surfaces. See `docs/operational/runbooks/04_GOOGLE_MAPS_SETUP.md` §8 Troubleshooting (rollback procedure) + §11 Review history (full incident record). | **Trigger to permanently delete:** ≥ 2026-05-03 if no Maps regressions observed during the observation window, OR ≥ 2026-05-15 if extra caution preferred. **If anything Maps-related breaks during the wait:** instant rollback per runbook §8 (Edit key → API restrictions → "Don't restrict key" or restore previous). |
| 14 | Legacy GCP key **`MonitraxGemini`** unrestricted (warning icon in Console) | GCP Console → Credentials (project `monitrax-479700`), key labelled `MonitraxGemini`, created 12 Dec 2025 | Found during the 2026-05-02 Maps audit on a separate row. Warning icon = no application restrictions, no API restrictions. Same audit pattern needed as Tech Debt #13 above. | Run §5-style audit on the Gemini API metrics dashboard — check for traffic, soft-disable if unused, narrow API restrictions to **Generative Language API** only if used. Owner: Reza + Claude in next housekeeping pass. |
