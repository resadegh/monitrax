...
**Last updated:** 2026-05-03 — Reza + Claude (**Snapshot SSOT cleanup (PR #598)** — Tech-Debt #1b closed by migrating `/api/dashboard/insights` from legacy `getFinancialSnapshot()` → canonical `getMasterFinancialSnapshot()` with field-path remap; deleted `/api/financial-snapshot/route.ts` + `lib/services/financialSnapshot.ts` + the legacy barrel re-exports. Tech-Debt #1a RECLASSIFIED — pre-flight audit found `/api/portfolio/snapshot` is NOT a duplicate of master; it returns `SnapshotV2` (GRDCS / relational scope) which is a distinct canonical SSOT. CLAUDE.md §12.2 updated with two-snapshot-SSOTs explanation so future sessions don't re-attempt the wrong cleanup. Up Next #5 removed (work landed); items 6→13 renumbered to 5→12. See `✅ Recently Completed` for the 2026-05-02 stack and CHANGELOG_2026_05_03.md for this PR's session detail.)
...
| 6 | **Incident Response Plan WIF section** | `docs/policy/INCIDENT_RESPONSE_PLAN.md` | Next housekeeping pass — capture Phase 9 cutover lessons formally |
