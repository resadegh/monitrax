...
- [ ] 3 — **Accountant bundle modal + secure share-link.** "Send to accountant" modal wrapping the **existing** `/api/documents/export` endpoint (which already produces FY-first / entity-first / category-first ZIPs — discovered in audit). Designed as a **generic Share Pass** from day one (per Reza's directive 2026-05-01 evening: "this sharing the data to accountant should be for other relevant data as well in future"). New `SharePackage` Prisma model with polymorphic `contentRefs` (Json) + `contentType` enum so the same engine extends to non-document content later (Reports, property summaries, net-worth statements). Three actions in the modal: download ZIP · generate secure shareable link · email recipient. Share-link is time-limited (default 30 days), audit-logged via `createAuditLog()` per CLAUDE.md §13 CDR compliance, watermarked. Recipient lands on read-only `/share/[token]` Monitrax-branded page (no login required); every view audit-logged. New endpoints: `POST /api/share`, `GET /api/share/[token]` (public), `DELETE /api/share/[id]`. Manage-Shares page at `/dashboard/settings/shares`.
