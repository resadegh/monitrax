...
**Last updated:** 2026-06-16 (Phase 4 Layer 4 — **Playwright UAT via the Firebase Auth emulator**: the `playwright (UAT)` job now runs (gate removed) under `firebase emulators:exec --only auth` — synthetic users seeded into the emulator + linked to `seed:lighthouse` archetypes via `OAuthAccount`, specs sign in through the real `/signin` UI. **No secret, no real Firebase project, no manual login.** Client connect (`lib/firebase/config.ts`) + server decode-only verify path (`lib/auth/gcpTokenVerifier.ts`) are strictly gated on `*_FIREBASE_AUTH_EMULATOR_HOST` (set only in the job; prod/preview unchanged; server path unit-tested). Verified here: 6/6 emulator-verify unit tests + full vitest suite green; specs discovered via `--list`. The end-to-end CI run is what the first PR run proves; job is non-required until green, then add `playwright (UAT)` as a required check. Prior: Phase 4 L1–L4 shipped (#1115–#1118) + cursor refresh #1119; Phase 3 fix PRs #1113/#1114; #1111 FY-review.) _Keep this date current: it must never be older than the newest entry in [`04_RECENTLY_COMPLETED.md`](implementation/04_RECENTLY_COMPLETED.md) (CI-checked — finding F-1)._
