...
**Blocking:** Reza decision on **Q-PRA-1** below (D2C vs org-attached AI voice context). Decision shapes how PR2/PR3 wire the AI Guide. Build can proceed without it for PR1 (no AI surfaces touched).
...
| Q-PRA-1 | **D2C vs org-attached AI voice context** — when a user is linked to an org (adviser / broker / accountant), how does the AI Guide / Gemini advisor differentiate from the D2C voice? | The B2B2C model only works if the AI yields to the professional. AFSL/credit/TPB boundary: D2C must stay on the *general information* side; org-attached can be richer because the licensed professional is the gatekeeper. Wrong call here either (a) puts the unlicensed AI on the wrong side of ASIC RG 244 / RG 36, or (b) undermines the professional's relationship with their own client. | **Claude recommendation 2026-05-04:** ONE engine, TWO voices. Same `getMasterFinancialSnapshot()`, same Gemini orchestrator, same scenarios — flip via `voiceContext: 'self' \| 'professional-attached'`. (1) Tone shifts: "I'd suggest…" → "Worth a conversation with [Adviser]…". (2) Action affordance shifts: Run-Scenario / Mark-as-planned → Send-to-adviser-for-review (creates structured task in Practice queue). (3) Disclaimer shifts: heavy "general information only" → confidence chip "Reviewed by [Firm Name] — last review [date]". (4) Refusal behaviour: D2C refuses product-specific questions, org-attached defers them to the professional. NOTHING else changes — Health, TRAIL, scenarios, tiles, AI input context, cost shape all identical. Edge case (D2C → org-attached transition): 24h cache invalidates on next advice generation; explicit transition notification; existing data preserved. **Awaiting Reza decision** on (a) accept the two-voice model, (b) "Reviewed by [Firm]" wording vs alternatives, (c) bake voiceContext plumbing into PR2 vs defer to PR3 (when AI Guide actually lands inside the Practice surface). |

**Open as of 2026-05-04:** Q-PRA-1 (B2B2C AI voice context).
