...
**Last updated:** 2026-05-04 (late) — Reza + Claude (**Phase 32B PR1 SHIPPED + Phase 32C strategic decisions LOCKED.** Q-PRA-1 (D2C vs org-attached AI voice context) DECIDED — REJECTED in favour of Reza's single-voice + marketplace + Ask-a-Professional + in-app comms model. Two-voice approach logged in ↩️ Reversed Decisions to prevent re-litigation. New workstream Phase 32C queued (Up Next #15–20): marketplace listings + verification → Ask button → request lifecycle → conversations + email-through-app → admin platform surface → Stripe billing. Two independent revenue streams confirmed: (A) Org subscription tiered Solo/Practice/Enterprise, bundles all professional seats + all client Wealth-tier features (no per-client charge — incentive aligned for orgs to push the platform to their entire client book); (B) D2C marketplace lead fee tiered by user profile (AU$80/$150/$250 by net worth bracket); (C) D2C subscription unchanged. Org-attached users see ONLY their org's professionals (Reza decision: org pays for Monitrax to be their CRM + comms channel; can't be a leaky funnel to competitors). Earlier today: **Phase 32B B2B2C Practice surface STARTED — PR1 of ~3 shipped.** Reza brief: monetisation strategy session evolved into B2B2C architecture for advisers/brokers/accountants. Existing `/portal/*` is consent+invitation harness only (audit 2026-05-04: ~30% real / 70% stub); the value surface — cross-client Practice dashboard with alert stream, drill-in shared with the consumer dashboard, role-flavoured columns — is greenfield. Decision: redesign + extend the existing portal (keep RBAC + invite + consent plumbing), don't fork. PR1 ships: schema migration `20260504120000_add_organisation_profession` adding `Organization.profession` (forced at registration, no MULTI; reuses existing `OrganizationType` enum), Practice surface design primitives (`PracticeGlassCard`, `TrailStageChip`, `PracticeKpiStrip`, `PracticeAlertStream`, `PracticeClientBookTable`, `PracticeHeader`) under `components/portal/practice/`, profession-aware config registry (`lib/portal/practice/professionConfig.ts` — adviser/broker/accountant defaults for alert library + book columns + AFSL/credit/TPB compliance footer), lighthouse demo dataset (`lib/portal/practice/lighthouseDataset.ts` — 5 fictitious AU property-investor clients spanning all 5 v1 alert triggers), dead-code removal of `lib/portal/auth.ts` (closes tech-debt #8). Practice dashboard page wiring, sidebar repaint, IMPLEMENTATION_PLAN/CHANGELOG/03_DATA_MODEL/06_UI_UX_FOUNDATION sync, and AFSL voice-context decision (D2C vs org-attached AI behaviour) deferred to PR2 pending Reza response. Earlier commits: previously **Open Questions resolved + Phase 39 propagation parked.** Cloud SQL upgraded to Enterprise Plus (Q1 closed); Vercel Pro confirmed (Q3 closed); legacy `/api/portfolio/snapshot` + `/api/financial-snapshot` deletion path documented (Q4 closed); auth-route hard-delete window confirmed for ≥ 2026-05-15 (Q5 closed); Prisma Accelerate not adopted (Q2 closed). **Phase 39 propagation declined for now** — v4 tile pattern stays scoped to My Wealth; not extended to My Accounts / My Budget / My Safety Net. Decision documented in `PHASE_39_MY_WEALTH_REDESIGN.md` §7 with revisit triggers. Cloud SQL ops doc updated with Enterprise Plus edition. **Phase 39.3 — Assets redesign SHIPPED — Phase 39 COMPLETE.** `/dashboard/assets` now uses the v4 tile pattern with a **warm sub-palette family** (amber/rose/peach hero atmosphere; six per-type tile palettes: VEHICLE amber/orange, ELECTRONICS rose/pink, FURNITURE stone/amber, EQUIPMENT slate/zinc, COLLECTIBLE fuchsia/violet, OTHER orange/amber). New `AssetsHero` + `AssetTile`. 6 new filled silhouette glyphs added to `wealthGlyphs.tsx` (car / laptop / sofa / wrench / diamond / box) + `AssetGlyph` resolver. Status pill (Sold / Written off) + 75% opacity wrapper for inactive assets. 4-card summary block replaced with single hero. Build green, no new deps. **Phase 39.2 — Investments redesign SHIPPED.** `/dashboard/investments/accounts` and `/dashboard/investments/holdings` now use the v4 tile pattern. New `InvestmentsHero` (shared) + `InvestmentAccountTile` (5 Stage I sub-palettes: BROKERAGE/SUPERS/FUND/TRUST/ETF_CRYPTO) + `HoldingTile` (4 palettes: SHARE/ETF/MANAGED_FUND/CRYPTO — CRYPTO uses warm amber). 9 new filled silhouette glyphs added to `wealthGlyphs.tsx`. Build green, no new deps. Mobile sticky-stack PARKED via PR #591 with comprehensive PHASE_39 §4 doc. **Phase 40 — My Guide AI Financial Advice STARTED.** Replaces the rule-based "Prioritised Actions" surface on `/dashboard/cfo` with a comprehensive Gemini-powered advice section grounded in the canonical `getMasterFinancialSnapshot()` output. New deterministic scenario engine (`lib/cfo/scenarios/`) for what-if calculations (sellProperty, payDownLoan, refinanceLoan, redirectToOffset, cutSpendCategory, addInvestment). New `lib/cfo/aiAdvisor.ts` orchestrator: feeds a sanitised, CDR-safe snapshot + rule-engine findings into Gemini Pro (`gemini-2.5-pro-preview-05-06`) with strict instruction to never invent numbers; AI calls scenarios via tool calling so projected impacts stay deterministic. New endpoints `/api/cfo/advice` (24h-cached headline advice), `/api/cfo/advice/chat` (follow-up Q&A scoped to the same snapshot, persisted per advice doc), `/api/cfo/scenarios/run`. Page redesigned to match Phase 39 Wealth / Phase 37 Budget design language (glassmorphic 28px cards, sky/indigo Stage I atmosphere, framer-motion v12 appleEase, full prefers-reduced-motion). Tax tab folded into Actions per Up Next #8 — `/dashboard/tax` redirects in. Tech-debt #11 (`lib/cfo/trailStage.ts`) closed — now wired up. **Phase 39 — My Wealth premium redesign STARTED 2026-05-01.** Properties page redesigned with new `PropertiesHero` (glassmorphic Stage I hero, equity-vs-debt bar, type allocation segments) + `PropertyTile` (sky/indigo Apple-glass tile with stagger entry, animated LVR bar, type-aware body, hero gradient CTA). New phase doc `PHASE_39_MY_WEALTH_REDESIGN.md`. Investments + Assets queued. Build green, no new deps. **Phase 38 COMPLETE.** Final PR closes the workstream: PR 2.5 (universal upload migration — `hooks/useDocumentUpload` migrated to canonical DME endpoint with hook-level link translation, no 2,031-LOC form edits) + PR 4 (tax-status lens — fourth FolderTree filter with Deductible/Non-deductible/Untagged buckets derived from `/api/expenses isTaxDeductible`) + FolderTree visual redesign (Apple typography, glass active-row, tabular-nums badges, top-level lenses as uppercase section headers — addresses Reza's "still don't see folder structure changes" note after PR 3 merge). Tech-debt #12 closed. Earlier today: PR #574 (Phase 37 — My Budget IA), PR #575 (My Vault IA + visual uplift), PR #576 (Smart Inbox interactive + upload audit), PR #577 (Send to accountant Share Pass + folder view restyle), Session-expiry UX hardening, WIF Phases 9 + 10 closed.)
...
- [ ] 7 — **DEFERRED.** Tax fold-in (closes Up Next #8). Defer to a follow-up PR per Phase 37 precedent ("Standalone routes preserved" — `/dashboard/income`, `/dashboard/expenses`, `/dashboard/budget-analysis` are unchanged, full CRUD intact). The cfo page already surfaces the headline tax data via the existing `TaxInsights` tile; the AI advisor naturally references tax data and recommends tax actions. The deeper `/dashboard/tax` route stays alive as a deep-link destination. Re-queued under Up Next #8 with a sharper trigger: "after Phase 40 stabilises, evaluate whether the AI advice surface has made the standalone tax route redundant."
...
**Supersedes:** Up Next #8 (My Guide simplification + Tax → Actions consolidation). The Tax fold-in is delivered as Phase 40 Phase 7 instead of as a separate session — bundling avoids re-touching the same surface twice. Closes the workstream queued there.
...
| 8 | **Tax fold-in into My Guide AI Advice surface.** Re-queued from Phase 40 — Phase 7 was deferred per the Phase 37 precedent (standalone routes preserved as deep-link destinations). The AI advice surface now surfaces tax-related guidance natively via the advisor; the `/dashboard/tax` page is reachable from the existing TaxInsights tile. **Trigger:** after Phase 40 stabilises (~2 weeks of usage), evaluate whether the AI advice surface has made the standalone tax route redundant; if yes, redirect /dashboard/tax → /dashboard/cfo and embed the deeper tax breakdown inside the cfo page. | UX / TRAIL Live stage | After Phase 40 ships and stabilises (~2 weeks observation) |
...
| 10 | **Phase 39.2** — Investments redesign (`/dashboard/investments/accounts` + `/dashboard/investments/holdings`). New `InvestmentsHero` + per-type `InvestmentAccountTile` and `HoldingTile` with Stage I tonal hue + filled-silhouette glyphs (Brokerage / Super / Fund / Trust / ETF / Crypto for accounts; Share / ETF / Managed Fund / Crypto for holdings). Includes mobile sticky-stack scroll pattern from Phase 39.1d. | UX / Phase 39 | After Reza approves Phase 39.1d sticky-stack mock |
| 11 | **Phase 39.3** — Assets redesign (`/dashboard/assets`). New `AssetsHero` + `AssetTile` with per-type hue + filled-silhouette glyphs (Vehicle / Electronics / Furniture / Tools / Collectable / Other). Includes mobile sticky-stack scroll pattern. | UX / Phase 39 | After 39.2 ships and is approved |
| 12 | **Phase 40 — App-wide mobile sticky-stack replication.** Pattern documented in `PHASE_39_MY_WEALTH_REDESIGN.md` §4. Apply the `sticky top-3 + useScroll/useTransform` wrapper to every tile-list page that reads as a vertical list on mobile (Balances, Budget, Safety Net, Reports/Vault, etc.). Reusable hook `hooks/useIsMobile.ts` already in place. | UX / mobile polish | After Reza approves the pattern across all 3 My Wealth pages (Phase 39.1d → 39.2 → 39.3) |
| 13 | **Phase 32B PR2 — Practice dashboard wiring.** Replace `/portal/dashboard` "Coming Soon" stub with the assembled Practice view (Header + KpiStrip + AlertStream + ClientBookTable). Repaint `PortalSidebar.tsx` + `PortalLayoutClient.tsx` from slate-900 admin look to brand tokens (warm-ivory background, brand navy + emerald accents). Demo-mode profession switcher in chrome (production hides the switcher; production reads `org.profession`). All primitives shipped in PR1 — this is pure assembly + paint. ~3 days. **Trigger to start:** next session. Unblocks the lighthouse adviser pitch demo. | Phase 32B | Next session |
| 14 | **Phase 32B PR3 — Drill-in render pattern.** Render canonical consumer dashboard at `/portal/clients/[id]/view` with adviser overlay docked right (desktop) / bottom-sheet (mobile). Wire `viewerContext` through `masterFinancialService.ts` — service rejects calls without `seatId` + `clientUserId` + `accessScopes`. Scope filter at the service layer (not UI). Retire `components/portal/clients/ClientDetail.tsx`. ~5 days. **Trigger:** after PR2 ships and Reza confirms the Practice view is on-brand. | Phase 32B | After PR2 |
| 15 | **Phase 32C PR4a — Marketplace listings + verification.** New Prisma models `ProfessionalListing` (per-Org public profile), `ProfessionalRating`. Verification flow against ASIC + TPB public registers (manual at v1; automated cross-check post-launch). Admin approval queue at `/admin/marketplace/listings` (manual gate before any listing goes live publicly). Invite-only at v1 — Monitrax-curated 10–20 launch professionals. ~4 days. **Trigger:** after Phase 32B PR3 lands. | Phase 32C | After Phase 32B PR3 |
| 16 | **Phase 32C PR4b — "Ask a Professional" button.** New primitive `<AskAProfessionalButton context={...} />` wired into AI Guide recommendation cards + Practice alerts. State A (D2C, no professional linked) → marketplace picker showing 3 best-fit + "see more". State B (org-attached) → org's roster grouped by discipline (adviser / broker / accountant). Same component anatomy, different destination. ~3 days. | Phase 32C | After PR4a |
| 17 | **Phase 32C PR4c — `ProfessionalRequest` lifecycle.** Pick → compose question (free text, AI-suggested starters) → request → adviser inbox → ACCEPT (lead fee billed) / DECLINE → consent invite → `ClientLink` materialises → engagement begins. Tiered lead fee billing wired (AU$80/$150/$250 by user net-worth bracket). Org-attached path skips the consent loop (already linked). ~5 days. | Phase 32C | After PR4b |
| 18 | **Phase 32C PR4d — In-app conversations + email-through-app.** New Prisma models `ProfessionalConversation` + `ConversationMessage` + `ConversationParticipant`. In-app chat thread (polling for v1; WebSockets later). Email-out via SendGrid; email-in via SendGrid Inbound Parse webhook → `/api/conversations/inbound` → routed to conversation by reply-to address. Compliance archive for AFSL holders. Hardening: DKIM/SPF, attachment scanning, rate limiting. ~7 days. **Open question for Reza before this PR ships:** conversation transcript retention vs CDR consent revocation — soft-delete from user view + 7yr retention for the professional's compliance archive (recommended) vs hard-delete on revoke. | Phase 32C | After PR4c |
| 19 | **Phase 32C PR5 — Monitrax platform admin (`/admin/orgs/*`).** Reza's surface for managing all Orgs across the platform: list with KPIs (active seats / clients / MRR / marketplace participation / lead-fee revenue MTD); drill-in for plan changes, seat allocation overrides, billing adjustments, marketplace listing approval, audit trail. ~5 days. **Trigger:** after first 5–10 paying orgs sign on (manual support workflow is fine until then — Reza in the DB). | Phase 32C | After ~5 paying orgs |
| 20 | **Phase 32C PR6 — Stripe Billing wiring.** Org subscription tiers (Solo / Practice / Enterprise) with seat + client caps enforced. Lead-fee billing (per-accepted-request charge). Dunning + failed-payment grace. Usage tracking surfaced in `/admin/orgs/{orgId}`. ~4 days. **Trigger:** ready to start charging — coordinated with Basiq accreditation milestone. | Phase 32C | Coordinated with Basiq go-live |
...
**Open as of 2026-05-04:** none in flight today. Q-PRA-1 closed by single-voice decision.
...
|---|---|---|---|---|
| 1a | `app/api/portfolio/snapshot/route.ts` | `app/api/portfolio/snapshot/route.ts` | Duplicate of `/api/master-snapshot` per CLAUDE.md §12.4. **Dead-code audit 2026-05-01 finding: STILL ALIVE** — 3 callers (`app/dashboard/page.tsx:348`, `hooks/useUISyncEngine.ts:282`, `app/api/linkage/health/route.ts:19`). Cannot delete until callers are migrated. | After migrating the 3 callers to `/api/master-snapshot` (~2-3h work). Then delete the route. |
| 1b | `app/api/financial-snapshot/route.ts` (route) + `lib/services/financialSnapshot.ts` (service) | Both files | Route handler has zero callers. The underlying `getFinancialSnapshot()` service is still used by `app/api/dashboard/insights/route.ts:101` (and thus is itself a duplicate of canonical `getMasterFinancialSnapshot()`). Both are dead at the route level but the service has 1 indirect caller. | (1) Migrate `dashboard/insights` to `getMasterFinancialSnapshot()`, (2) delete the route file, (3) delete the service file. Bundle as one PR. |
