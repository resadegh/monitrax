...
**Last updated:** 2026-06-15 (Phase 4 Layer 3 — **invariant / property checks**: `tests/regression/invariants/` asserts engine laws over the 6 archetypes + a seeded 40-portfolio random sweep — net worth == assets−liabilities across all read-paths (Float, Decimal, `buildEntityBreakdown`, #1113 valuation helpers); per-entity legal-title value reconciles to the household total; ownership shares sum to 100% (`attributeAsset`); Float/Decimal siblings agree at the boundary; D6 CGT owner discount+share matches `calculateCgtDiscountDecimal` + `attributeAsset`. 246 tests green; no correctness bug surfaced. Branches off main `2ca4043`; detailed `01`/`04` spoke entries staged in the PR body. Prior same-day: Phase 4 Layer 2 golden-master (PR #1116) + Layer 1 vitest CI (PR #1115); Phase 3 audit + fix PRs #1113/#1114; #1111 FY-review; Phase 47 D6; F-8 plan split.) _Keep this date current: it must never be older than the newest entry in [`04_RECENTLY_COMPLETED.md`](implementation/04_RECENTLY_COMPLETED.md) (CI-checked — finding F-1)._
