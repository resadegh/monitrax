...
**Last updated:** 2026-05-01 (evening) — Reza + Claude (**Phase 38 PR 2 in flight: Smart Inbox interactive + universal upload audit.** Smart Inbox now expandable (chevron toggle, AnimatePresence height + opacity reveal); per-row inline confirm using existing `/api/documents/analyze/confirm` endpoint; per-row Open button using existing signed-URL `/api/documents/[id]` endpoint. Confidence dot, doc-type badge, AI-extracted summary (vendor + amount + date) per row. Zero new APIs, zero new components — surfaces existing Phase 26 work that was previously buried per-document. **Upload-path audit complete**: 2 of 3 codepaths route through Phase 25 DME canonically; ONE legacy bypass identified (`hooks/useDocumentUpload.ts` → `POST /api/documents` → legacy `documentService.uploadDocument()`). Affected: `ExpenseDialog`, expenses page. Captured as PR 2.5 + tech-debt row #12; preferred fix is to refactor `documentService.uploadDocument()` to internally route through DME (fix-once-fix-everywhere). Earlier today: PR 1 (My Vault IA + visual uplift + Smart Inbox count) shipped as PR #575; **Phase 37 SHIPPED via PR #574** (My Budget IA 6→3 tabs, `/dashboard/plan` hub, Cashflow + Debt Freedom Apple-typography heroes); **Session-expiry UX hardening** shipped; **WIF Phases 9 + 10 closed**.)
...
- [ ] 3 — **Accountant bundle modal + secure share-link (Option C).** Reza decision 2026-05-01 evening: ship Option C (secure shareable link) now; Option E (direct push to Xero / MYOB) parked as future expansion (see "Future expansion" note below). "Send to accountant" modal wraps the **existing** `/api/documents/export` endpoint (already produces FY-first / entity-first / category-first ZIPs — discovered in audit). Designed as a **generic Share Pass** from day one (per Reza's directive: "this sharing the data to accountant should be for other relevant data as well in future"). New `SharePackage` Prisma model with polymorphic `contentRefs` (Json) + `contentType` enum + `deliveryMethod` enum so the same engine extends to non-document content (Reports, property summaries, net-worth statements) AND to non-link delivery (Xero/MYOB push) later. Three actions in the v1 modal: download ZIP · generate secure shareable link · email recipient. Share-link is time-limited (default 30 days), audit-logged via `createAuditLog()` per CLAUDE.md §13 CDR compliance, watermarked. Recipient lands on read-only `/share/[token]` Monitrax-branded page (no login required); every view audit-logged. New endpoints: `POST /api/share`, `GET /api/share/[token]` (public), `DELETE /api/share/[id]`. Manage-Shares page at `/dashboard/settings/shares`.

  **Future expansion (Option E — direct accountant-software push):** Reza flagged 2026-05-01 evening that direct Xero / MYOB integration is on the future roadmap. The PR 3 schema design accommodates this without a future migration: `SharePackage.deliveryMethod` enum (v1 values: `LINK`, `EMAIL_LINK`, `DOWNLOAD_ONLY`) extends in v2+ to add `XERO_PUSH`, `MYOB_PUSH`, etc. The polymorphic `contentRefs` + `contentType` shape means accountant-software integrations reuse the same audit-log + revocation + recipient-tracking machinery — only the transport adapter changes. Tracked as a Phase 38 future (post-launch) item.
...
| 6 | When to schedule Phase 38 "Option E" — direct push to Xero / MYOB / accountant software? Reza decision 2026-05-01: not v1; PR 3 schema is built to accommodate it. | Reduces accountant-side workflow friction (no manual ZIP import). Each integration is its own CDR sub-processor compliance project. | After Phase 38 PRs 1-3 ship and we have telemetry on share-link usage; pick the platform with the highest user-share volume first. |
...
| 12 | Legacy upload codepath bypassing Phase 25 DME | `hooks/useDocumentUpload.ts` → `POST /api/documents` → `lib/documents/documentService.ts:uploadDocument()`. Consumers: `components/ExpenseDialog.tsx`, `app/dashboard/expenses/page.tsx`. | Per Phase 38 PR 2 audit (2026-05-01): the legacy hook bypasses the Phase 25 `DocumentManagementEngine.processUpload()` cascade, so uploads don't get RuleEngine-driven storage selection, category inference, auto-linking, or path generation. Doc rows still get created (visible in the Vault) but without the full canonical metadata. | Phase 38 PR 2.5 — preferred fix is to refactor `lib/documents/documentService.ts:uploadDocument()` to internally route through `getDocumentManagementEngine().processUpload()`, so every existing caller benefits without touching the 2,031-LOC expenses page. Fix-once-fix-everywhere. |
