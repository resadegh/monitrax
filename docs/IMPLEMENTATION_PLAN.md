...
**Last updated:** 2026-06-15 (Phase 4 Layer 2 — **golden-master regression**: `tests/regression/golden-master/` captures canonical-engine outputs (net worth + per-entity, cashflow, per-entity breakdown, per-entity CGT split) for six archetypes — sole owner / couple 50-50 / discretionary trust / SMSF / company / mixed portfolio — into committed vitest snapshots taken FROM the engine (`toMatchSnapshot`, no hand-authored numbers; §2/§12.3). 24 tests, green + stable; drift fails the suite. No correctness bug surfaced. Branches off main `2ca4043`; the detailed `01`/`04` spoke entries are staged in the PR body (spokes exceed the connector rewrite ceiling). Prior same-day: Phase 4 Layer 1 vitest CI (PR #1115); Phase 3 audit + fix PRs #1113/#1114; #1111 FY-review; Phase 47 D6 feature-complete; F-8 plan split.) _Keep this date current: it must never be older than the newest entry in [`04_RECENTLY_COMPLETED.md`](implementation/04_RECENTLY_COMPLETED.md) (CI-checked — finding F-1)._
