...
**Last updated:** 2026-06-15 (Phase 3 calc-engine + data-relationship correctness audit — STATIC, record-don't-fix — landed `docs/audits/PHASE3_ENGINE_CORRECTNESS_2026-06-15.md`: 7 findings, 4×P2 / 3×P3, recorded with recommended fix PRs in that audit doc. The matching one-line backlog index row for `03_OPEN_QUESTIONS_AND_BACKLOG.md` (#35 — #34 was taken by PR #1111's FY2026-27 tax-config item mid-session) is **staged for the next git-capable session** — a verbatim connector round-trip of the 63 KB spoke was withheld to avoid corrupting the SSOT (finding F-8 in practice). Prior same-day: PR #1111 extended the FY-review checkpoint + hardened tax-config fallback (advanced main c10333d2 → 3e5881f1); Phase 47 Stage D · D6 — the `sellProperty` What-If lever now computes CGT through the canonical per-entity path; Phase 47 is feature-complete. Earlier same-day: Phase 2 PR-C / finding F-8 — the plan was split from one 884 KB file into this hub + the `docs/implementation/` spokes so it stays connector-writable and under the §15.5 size budget; the historical preamble narrative was relocated to `docs/changelog/IMPLEMENTATION_CHANGELOG.md`). _Keep this date current: it must never be older than the newest entry in [`04_RECENTLY_COMPLETED.md`](implementation/04_RECENTLY_COMPLETED.md) (CI-checked — finding F-1)._
