...
- 💻 **CODE** — desktop Claude Code session *(or Matrix-via-PR if you enable full-auto — see the decision I've put to you)*.
...
| 3.1 | **Data model** | ✋ YOU → 💻 | Add `isRecurring Boolean @default(true)` to `model Income` (`prisma/schema.prisma:1865`), mirroring `Expense:1950`, **+ a migration.** *Schema/DB change = your approval before it's applied (§12.11).* | migration written |
...
| 3.3 | **Producers P1–P5** | 💻 CODE | Thread `isRecurring` through: P1 `app/api/income/route.ts:240` (main CRUD), P2 `transactions/[id]/link/route.ts:417`, P3 `documents/analyze/confirm/route.ts:459`, P4 `onboarding/complete/route.ts:230`, P5 `lib/db/tenant.ts:168`. | all 5 pass the flag |
| 3.4 | **Gate** | 💻 CODE | `TransactionLinkDialog.tsx:373-377`: change `>= 2` to `>= 1` so a single transaction is classified not defaulted; send `isRecurring` on the income payload (mirror expense `:610-611`). **Also satisfies MON-037 RC-C.** | single-txn classified |
| 3.5 | **Read path** | 💻 CODE | `userTaxPosition.ts:94-105`: carry `isRecurring` through the map (`:99`) or the engine never receives it. | flag reaches engine |
| 4 | Model | 🤖 MATRIX | Re-pin Neomatrix income-guard anchors in the SAME PR (§21.2.1). | graph delta in PR |
| 5 | Ratchet | 💻 CODE | Ring-0 (one-off income counts once, Float **and** Decimal); **Ring-1 SOURCE-LOCK** (any Income/Expense annualisation site must be one-off-guarded — the ancestor-killer); Ring-2 Float/Decimal parity on the Golden Household. Set the registry `test` field (null today; gate requires it). | 3 rings added |
| 6 | PR | 🤖 MATRIX | Open the fix PR, §8 template, 10/10 self-score, STATE.md + plan spoke same PR. | PR open |
| 7 | Merge | ✋ YOU | Money-language + schema merge = your click. Matrix shepherds CI green first. | merged |
| 8 | Deploy verify | 🤖 MATRIX | Vercel prod deploy READY. | READY |
| 9 | Ring-3 (Chrome) | 🤖 MATRIX → ✋ | Claude-in-Chrome on your real data (targets below). **Number-changing → you sign off the verdict.** | symptom gone, sign-off |
...
**What YOU actually touch for MON-053:** three points — approve the schema migration (3.1), click the merge (7), sign off the Ring-3 verdict (9). Everything else is automated or in the Code session.
...
- **Opus 4.8** → 3.3 producers, 3.4 gate, 3.5 read path, 3.1 migration *application* once specified, and all tests (mechanical, fully specified above).

## Gates & pause points
1. **Schema migration** — your approval before applying (§12.11).
2. **changesNumbers → Ring-3** — you sign off the verdict before VERIFIED.
3. Engine/producer/gate/tests code proceeds through the PR without further intervention.
...
## Definition of done
Model + engine (Float+Decimal) + producers + gate + read-path shipped with Ring-0/1/2 ratchets and the `test` field set; STATE.md + plan updated same PR; Chrome Ring-3 PASS on the four targets with your sign-off → **MON-053 VERIFIED**; **VR-008 re-baseline** run; MON-045 stage 2 unblocked.
