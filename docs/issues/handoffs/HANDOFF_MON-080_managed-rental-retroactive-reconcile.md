...
4. **Gross-integrity prompt (D2).** On transition to MANAGED, if the stream has no gross distinct from its linked disbursements — i.e. no `grossAmount` set, or `amount` ≈ the mean net deposit (within materiality) — require the real gross before saving: a prompt "It looks like you haven't entered the actual rent for <property> — what's the full rent before your agent's fees?" (weekly/period aware). Also expose an amount/gross field in the **Edit** modal (fixes the income-side MON-008 gap for this path). Never let a stream sit MANAGED with gross = net (silent understatement).

## Ratchets (Part 5 style)
- **Ring-0 (D2):** marking a stream MANAGED with `amount === net deposit` must block/prompt (not silently save gross = net).
...
- **Opus 4.8** → the nudge-chip → card UI wiring, the D2 gross prompt + Edit-modal amount field (Stitch §8, reuse `ManagedRentalReconcileCard`) + the ratchets.
