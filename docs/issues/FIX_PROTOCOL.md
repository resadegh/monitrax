...
0. **Holistic SSOT audit — the MATRIX_FIX_DISCIPLINE clause-4 gate (Reza directive 2026-07-17).** Before ANYTHING else, bind this issue to the four clauses of `docs/architecture/MATRIX_FIX_DISCIPLINE.md`: the fix must be **non-regressing** (net issue count + lint exception counts only go down — `lint:source-lock` enforces the ratchet mechanically), **SSOT-preserving** (no second producer created or left alive; the fix lands at the canonical producer, never one surface), **cumulative** (never trades one bug for another), and **holistic before fixing** (steps 1–5 below ARE that audit — no fix code exists until they're complete and the whole blast radius, not the reported symptom, is the fix scope). The live counter-example to internalise: MON-080's Ring-3 (VR-011) verified TAX only while cashflow double-counted the agent fee — a per-surface verification is how drift ships.
...
2. The PR carries, non-negotiably: the §20.6 tri-axis gate line (`Gate (§20.6): Document X/10 · Requirements X/10 · Logic X/10`) · the §19.2 evidence (units, formula+authority, worked example, verification) · the census tables (§3) · the Ratchet test (in the same PR, failing-before/passing-after where demonstrable) · the **same-PR Neomatrix update** (§21.2.1: repointed edges, deleted rogue nodes, new models for blind spots; `neomatrix:check` green) · the **same-PR Neobrain update** when intake/reconciliation/categorisation moved (§21.2.2 THIRD RULE; N/A stated otherwise) · **nothing sandbox-only** — every artefact this fix produced is in this PR · the plain-English trio (§19.5) · the §16 doc-sync block · destructive-write checklist if applicable (§12.11).
...
| 2026-07-19 | **MON-031/064 VR-017 Stage-4 FAIL — the #1452 netting was silently inert on live data** | Safety Net "Liquid savings" + emergency months stayed GROSS ($304,304 / 11.7) with the deploy confirmed live. The netting `gross − netWorth.liabilities.creditCards` subtracts a LOANS-only figure — the live Qantas card is a CREDIT_CARD-typed ACCOUNT at −$2,496, so it subtracted 0. Balances' net number was an accident of the buckets' `min(gross, accounts)`, which masked the inertness on the surface everyone compared against. | **Ring-2 fixture topology (a second F2, one level down).** The golden modelled the card as a LOAN — the representation the fix happened to handle — so the parity suite proved the fix on a topology the live data doesn't have. Same-engine-different-inputs at the FIXTURE level: VR-005's lesson ("a green repro on an unfaithful mock is not proof") recurred as "a green golden on an unfaithful TOPOLOGY is not proof". | ✅ FIXED (PR #1455): (1) ONE producer `computeLiquidCash` nets BOTH card representations; inline netting deleted; CFO buffer repointed. (2) NEW golden `ring2.liquidCashParity.accountCard.test.ts` locks the LIVE topology (asserting the topology *signature* — `liabilities.creditCards === 0` — so the fixture can't silently drift back to the convenient shape) + the I9 months identity on master AND the real route. (3) CENSUS RULE TIGHTENED: when a defect involves a data-modelled entity that has MULTIPLE storable representations (card = loan OR account; rent = declared OR managed-statement; income = recurring OR one-off), Stage-1's Input-Feed Census MUST name which representation the LIVE data uses (from a VR capture or Reza), and the Ring-2 fixture MUST mirror that representation — plus a variant per other representation where cheap. LESSON: **fixtures inherit the auditor's assumptions; lock the live topology's signature into the test so the assumption is checked, not embedded.** |
| 2026-07-21 | **MON-094 VR-021 Stage-4 FAIL — the engine fix was correct but UNREACHABLE: the review surface shipped on the staff admin portal** | The #1475 non-assessable override worked, but the review page (`/admin/tax-review`) authenticates as the STAFF principal (`admin@monitrax.com.au`) — its `where: { userId }` query ran against the staff account, showed "Nothing to review", and Reza's two ATO rows stayed fully taxable (Taxable Income $185,481 unchanged). A correct engine behind an unreachable door is a failed fix. | **Stage 2 (design).** The design placed a user-data WRITE surface on the staff portal without checking the AUTHENTICATING PRINCIPAL; the censuses verified the query shape and the producer chain, but never asked *"whose session runs this query?"*. | ✅ FIXED (this PR): both personal-data tools (`tax-review`, `intake-duplicates`) RELOCATED to the app (`/dashboard/housekeeping/{tax,duplicates}`, user session, Stitch-designed #1477); both admin pages DELETED; topology Ratchet (`tests/housekeeping/relocation.test.ts`) locks the class — the two APIs are unreachable from `app/admin/**` and the pages authenticate via the user's own session. GATE CHANGE (Reza ruling 2026-07-21, standing): **personal-data review/write surfaces live in the Monitrax app, never the staff admin portal — Stage-2 design MUST name the session principal that will run each new surface's queries and assert it owns the data.** LESSON: a census that proves WHAT runs is incomplete without WHO runs it. |
