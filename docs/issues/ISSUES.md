...
| MON-130 | 🔵 OPEN | 🔴 | yes | CLASS SWEEP 2: 12 producers under lib/ read raw loan.minRepayment as a monthly cost — interest-only loans silently cost $0, and moneyFlowService SKIPS them entirely (root cause of the Activity 'Loans $106K/yr' vs canonical $12,779) | — | — |
...
> **What was wrong:** Your two interest-only investment loans cost $3,709 a month in real interest. Twelve different parts of the app treat them as costing nothing, because they read the contractual minimum repayment — which is zero for an interest-only loan. The year-to-date money chart leaves them out altogether.
>
- **Root cause:** `lib/services/moneyFlowService.ts:385`, `lib/planning/debtPlanner.ts:164`, `lib/calculations/cashflowOrchestrator.ts:219`, `lib/calculations/loanAggregator.ts:90`, `lib/cfo/scoreCalculator.ts:132`, `lib/cfo/riskRadar.ts:202`, `lib/cashflow/buildCFEInput.ts:115`, `lib/reports/contextBuilder.ts:392`
- **Holistic test (§19.4):** ⚠ required before VERIFIED/CLOSED
- **Detail:** `neoaudit-run:VR-041`
