...
**142 total** · 81 open · 🔵 38 · 🟡 6 · 🟠 28 · 🟢 9 · ✅ 59
...
| MON-130 | 🟠 FIXING | 🔴 | yes | CLASS SWEEP 2: 12 producers under lib/ read raw loan.minRepayment as a monthly cost — interest-only loans silently cost $0, and moneyFlowService SKIPS them entirely (root cause of the Activity 'Loans $106K/yr' vs canonical $12,779) | ##1575 | ✅ |
...
| MON-155 | 🟡 DIAGNOSED | 🟢 | yes | extractAdminToken reads an admin_session cookie that NOTHING writes — a reader with no writer that cost an evening of misdiagnosis | — | — |
...
### MON-130 — CLASS SWEEP 2: 12 producers under lib/ read raw loan.minRepayment as a monthly cost — interest-only loans silently cost $0, and moneyFlowService SKIPS them entirely (root cause of the Activity 'Loans $106K/yr' vs canonical $12,779)

**🟠 FIXING** · 🔴 critical · changes numbers: **yes** · area: cashflow · opened 2026-07-29
...
>
> **What changed:** Home's numbers now come from the same one place the Spending page already used: the engine that works out what each loan actually costs, using your real repayments where they exist and the interest charged where they do not. The rule that skipped loans with no minimum repayment is deleted, not just widened, so an empty field can no longer make a loan invisible. A test now fails the build if an interest-only loan ever disappears from that total again.
>
> **What you should see:** On Home, the THIS MONTH'S BUDGET tile should now show Loans $12,779 instead of $8,817 — matching the Spending page exactly. Two figures that follow from it also move, and that is correct: 'Saved' falls from about $15,048 to $11,085, and your saving rate from 59.4% to 43.7%. Nothing else should change — net worth stays $3,401,782 and the health score stays 53. One thing deliberately does NOT change yet: the entity money-flow chart still leaves those three loans out, so it will disagree with Home until the next piece of this work lands.

- **Root cause:** `lib/services/masterFinancialService.ts:1889`, `lib/services/moneyFlowService.ts:382`, `lib/planning/debtPlanner.ts:164`, `lib/calculations/loanAggregator.ts:90`, `lib/cfo/scoreCalculator.ts:132`, `lib/cfo/riskRadar.ts:202`, `lib/cashflow/buildCFEInput.ts:115`, `lib/reports/contextBuilder.ts:392`
...
**🟡 DIAGNOSED** · 🟢 low · changes numbers: **yes** · area: auth · opened 2026-08-03
