...
| MON-053 | 🟡 DIAGNOSED | 🔴 | yes | One-off income annualised x12 into the tax base — two single ATO deposits become ~$120.6K phantom recurring income | — | — |
...
| MON-060 | 🔵 OPEN | 🟡 | yes | Estimated annual tax differs between /cashflow ($194,218) and /dashboard/activity ($175K) | — | — |
...
**🟡 DIAGNOSED** · 🟠 high · changes numbers: **yes** · area: tax · opened 2026-07-14
...
>
> **What changed:** (planned, Reza-approved Option 1) Property loan interest will become an automatic tax deduction derived from your loan — so your total deductions rise to a realistic figure and the negative-gearing benefit becomes correct and consistent everywhere (no longer 4× your deductions).
>
> **What you should see:** (after the fix ships) On My Guide, the negative-gearing benefit will be less than your total deductions and match a hand-check of loan interest × your marginal rate; your total deductions and estimated refund will also reflect the loan interest.
...
**🟡 DIAGNOSED** · 🔴 critical · changes numbers: **yes** · area: tax · opened 2026-07-15
...
>
> **What changed:** (planned) REMOVE THE CULPRIT (§4.3) — do NOT add a compensating adjustment. A one-off income is currently INEXPRESSIBLE in the data model, which is the real defect:
(1) DATA MODEL — `prisma/schema.prisma`: `model Expense`:1933 has `isRecurring Boolean @default(true)`:1950; `model Income`:1865 has NO such column, and `enum Frequency`:183-190 has only WEEKLY/FORTNIGHTLY/MONTHLY/QUARTERLY/ANNUAL/HALF_YEARLY — NO ONE_OFF member. Add `isRecurring Boolean @default(true)` to `model Income` + a migration (mirror the Expense column exactly; default true keeps every existing row behaving as today).
(2) ENGINE — `lib/tax-engine/position/taxPositionCalculator.ts`: `ExpenseItem`:44-63 carries `isRecurring?`:62 and the MON-037 guard at :194-197 counts a one-off ONCE. `IncomeItem`:30-42 has NO such field and :125-128 annualises unconditionally (`annualize(income.amount, income.frequency)`). Add `isRecurring?: boolean` to IncomeItem and apply the IDENTICAL MON-037 guard at :125-128. THEN THE DECIMAL TWIN: :636 annualises income unguarded while :694-697 already guards the expense — a §12.2.1 duplicate that will silently keep the bug alive if only the Float path is fixed.
(3) PRODUCERS (P1–P5, thread isRecurring through; every one defaults frequency to MONTHLY, which is the mechanism): P1 `app/api/income/route.ts:240` — the MAIN income CRUD create (frequency required at :196, passed at :247); P2 `app/api/transactions/[id]/link/route.ts` — `const frequency = body.frequency || 'MONTHLY'`:346 → create at :417; P3 `app/api/documents/analyze/confirm/route.ts` — frequency default :456-457 → create at :459; P4 `app/api/onboarding/complete/route.ts:230`; P5 `lib/db/tenant.ts:168` (generic tenant wrapper — passes data straight through).
(4) GATE — `components/transactions/TransactionLinkDialog.tsx:373-377`: `if (tp?.detectedFrequency && (tp.count ?? 0) >= 2)` pre-ticks recurring. It is STRUCTURALLY BLIND to the single-transaction case: with count===1 it neither sets a frequency nor marks non-recurring, so the MONTHLY default silently wins — exactly the ATO rows. The expense payload already sends `isRecurring: isRecurringExpense`:610-611; the income path has no equivalent to send. Gate `>= 1` so a single transaction is classified rather than defaulted.
(5) READ PATH — `lib/tax-engine/position/userTaxPosition.ts:94-105` maps IncomeItem and passes `frequency: income.frequency`:99 straight through to `toAnnual` (`lib/utils/frequencies.ts:7`, MONTHLY ×12 at :13-14). Carry isRecurring through this map too, or the engine fix never receives it.
>
> **What you should see:** On Reza's real data: the two ATO rows — "Ato Ato001100022493651" ($9,098) and "Ato Ato002000023189359" ($952), each a SINGLE deposit dated 27 May — must each count ONCE, not ×12. /dashboard/income must stop showing them as "$9,098 /mo — $109,176/yr" and "$952 /mo — $11,424/yr"; the declared gross on /dashboard/tax must fall from $524,831/yr by ~$120,600 (the ~$109,176 + ~$11,424 that was never received); the tax estimate ($194,218 on /cashflow and CFO) must fall accordingly. Every other income row must be unchanged.
...
**🔵 OPEN** · 🟡 medium · changes numbers: **yes** · area: tax · opened 2026-07-15

> **What was wrong:** Your estimated tax reads $194,218 on Cashflow and CFO but $175K on Activity — a ~$19K gap with no explanation of why the two differ.
>
- **Neomatrix:** `number.cashflowIntelligence.estimatedTax`, `number.taxPayable`
- **Holistic test (§19.4):** ⚠ required before VERIFIED/CLOSED
- **Detail:** `neoaudit-run:VR-007`

Auto-raised by issues:raise (NeoAudit finding bus, §3.1). Surface: /dashboard/cashflow vs /dashboard/activity. Expected: One tax figure. Actual: /cashflow 'TAX - FY ESTIMATE' = $194,218 (matches CFO -$194,218 exactly). /dashboard/activity 'Year to date' flow shows Tax = $175K, ~$19K lower, with no basis label.. Evidence/run: VR-007.
