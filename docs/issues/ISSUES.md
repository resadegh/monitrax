...
**144 total** · 83 open · 🔵 38 · 🟡 8 · 🟠 27 · 🟢 10 · ✅ 59
...
| MON-157 | 🟡 DIAGNOSED | 🟠 | no | The golden-baseline REFERENCE was persisted as a hash, never as a tree — so no tranche can run the whole-tree declared-vs-undeclared diff retrospectively (T2 G7 is unclosable because of it) | — | n/a |
...
**🟡 DIAGNOSED** · 🟠 high · changes numbers: **no** · area: verification · opened 2026-08-04
...
>
> **What changed:** Not fixed yet. The capture tool already writes the full snapshot and its own output says 'COMMIT IT' — it simply was never committed, because the tool needs a database connection that the build container does not have, and the browser-based relay that CAN reach the database returned the snapshot into a chat session that ended. The fix is to commit the full tree, from the relay, as the reference artefact.
>
> **What you should see:** Nothing on any screen. What changes is that the next tranche can answer 'did anything move that we did not declare?' with a verdict instead of a shrug.
