...
| **n8n** | Orchestration | The automation spine — all GTM workflows (lead enrichment, personalisation, reply routing, daily digest, lifecycle, reporting) | Self-hosted (Community) | $0 (software) | $0 | 🟡 Server provisioned — install pending | Server `n8n-1` created on Hetzner 2026-05-13 (Nuremberg, CPX22, Ubuntu 24.04). DNS record `n8n.try-monitrax.com` + Docker install pending. Step 1.1. No-VPS alternative: n8n Cloud ~US$20/mo. |
| **VPS — Hetzner Cloud `n8n-1`** | Infra | Hosts self-hosted n8n + Postgres (n8n data) + Caddy (HTTPS) + future small GTM services (Documenso e-sign, etc.) | CPX22 (3 vCPU / 4 GB / 80 GB / 20 TB traffic) | ~€7.05/mo + ~€0.50/mo IPv4 + ~20% backups (if enabled) | ~$14–15/mo all-in | 🟢 Active (provisioned 2026-05-13) | Location: Nuremberg (eu-central). DNS-accessed via `n8n.try-monitrax.com`. Hetzner-managed daily backups; SSH key-only access; `unattended-upgrades` for security patches (configured during install). Raw IP intentionally NOT in repo — DNS hostname is the canonical reference. |
