...
| 5.3a | Right to erasure (consumer-initiated) | **DONE** | Settings overhaul 2026-05-08. `POST/DELETE /api/account/delete-request` implements a 30-day soft-delete grace period (Privacy Act APP 11.2 + CDR §3.2). `User.deletionRequestedAt` + `User.deletionScheduledFor` track the timer; partial index supports the future Cloud Scheduler hard-delete + anonymise job. Audited via `USER_DELETION_REQUESTED` / `USER_DELETION_CANCELLED` / `USER_DELETED`. | **Pending:** Cloud Scheduler job to actually run the purge on the scheduled date (queued post-Phase 35). |
