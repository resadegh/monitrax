...
**Decision (Reza 2026-06-27): migrate the AI gateway to Vertex AI** — contractual no-training + **AU data-residency** + DPA/CMEK, GCP-first (§12.7), reusing the existing WIF identity. This restores row 2.3 to true for the AI path.

**↩️ Decision REVISED (Reza 2026-06-28): DEFER the Vertex cutover to Basiq/CDR go-live; keep the most-capable models on the paid (no-train) tier for now.** Empirical reason (verified by live probe against `australia-southeast1`, Cloud Shell, 2026-06-28): Vertex's Sydney **regional** endpoint currently serves **only `gemini-2.5-flash`** of our models — the app's live `gemini-3.5-flash` (primary) and all pro models (`gemini-2.5-pro`, `gemini-3.1-pro-preview`) return **404 "not found in region"** there. Cutting over to AU-resident Vertex today would force a one-tier flash downgrade **and** drop the pro tier. Since Monitrax is **not live with Basiq and has no current plan to pursue accreditation**, that capability cost isn't justified yet.

**Interim posture (accepted pre-Basiq):** stay on the **paid, no-training** consumer Gemini API with the current capable models. The only residual gap is *cache* residency ("any country") — tolerable now because **no CDR data is flowing and no accreditation is being claimed**, and the Neobrain grounding layer (not the model) is the financial-correctness guarantee. **This gap is NOT closed; it is documented and deferred.**

**Closure path (a hard pre-condition of Basiq go-live):** the full Vertex-AU cutover — including the known `gemini-2.5-flash` model downgrade and a re-probe for newer Sydney-resident models *at that time* — is written up as a go-live playbook: **`docs/compliance/CDR_BASIQ_GOLIVE_CUTOVER.md`**. Pre-staged and inert today (no code reads them): Vertex AI API enabled, `roles/aiplatform.user` granted to the runtime SA `vercel-monitrax-db`, and `VERTEX_PROJECT`/`VERTEX_LOCATION=australia-southeast1` set on Vercel. `USE_VERTEX` is unset (current Gemini path active). Tracked as `PHASE_54_NEOBRAIN.md §15` Phase 0.5 (**PARKED**).

Sources: [Gemini API Additional Terms](https://ai.google.dev/gemini-api/terms) · [Vertex AI data governance](https://cloud.google.com/vertex-ai/generative-ai/docs/data-governance) · [GCP gen-AI data residency](https://cloud.google.com/blog/products/ai-machine-learning/google-cloud-generative-ai-data-residency-guarantees-for-data-stored-at-rest) · [Vertex AI locations](https://docs.cloud.google.com/vertex-ai/generative-ai/docs/learn/locations).
...
| **Status** | BUILT + **LIVE** (Phase 52). Schema + PII-scrubber + k-anon graduation write-back + lookup + housekeeping merged behind `KB_WRITE_ENABLED`/`KB_READ_ENABLED`; both flags **set `true` on Vercel Prod+Preview since 2026-06-22** (Reza-confirmed 2026-06-25). `KB_GEMINI_ENABLED` remains unset (`false`). Written de-identification procedure: `docs/policy/CDR_KB_DEIDENTIFICATION_PROCEDURE.md`. ⚠️ Confirm procedure sign-off + privacy-PDF regen were completed at/before write-enablement (the de-id procedure tracker still shows those two rows pending). Enablement runbook: `PHASE_52_SHARED_CATEGORISATION_KB.md` §10. |
