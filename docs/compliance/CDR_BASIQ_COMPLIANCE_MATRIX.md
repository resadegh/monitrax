...
| **Status** | BUILT — GATED OFF (Phase 52, 2026-06-22). Schema + PII-scrubber + k-anon graduation write-back + lookup + housekeeping all merged behind `KB_WRITE_ENABLED`/`KB_READ_ENABLED` (default false). Written de-identification procedure: `docs/policy/CDR_KB_DEIDENTIFICATION_PROCEDURE.md`. **Awaiting: procedure sign-off + privacy-PDF regen before flags are flipped on** (enablement runbook: `PHASE_52_SHARED_CATEGORISATION_KB.md` §10). |
