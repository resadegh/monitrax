...
| 3.2 | Network rules are enforced to limit external access | **DONE (DB) / PARTIAL (HTTP edge)** | **Database:** Workload Identity Federation + Cloud SQL Connector (Phase 9 of WIF workstream, **CUTOVER COMPLETE 2026-05-01** — Production traffic now serving via WIF). The Vercel runtime reads its OIDC token from the per-request `x-vercel-oidc-token` header, exchanges it via STS for an impersonated SA access token, then uses the Cloud SQL Connector to open a TLS 1.3 tunnel to the instance and authenticates to Postgres in IAM-database-auth mode using the SA access token as the per-connection password (no static password in any env var; rotation automatic per connection). Code: `lib/db.ts`. Evidence: `docs/compliance/CDR_WIF_AUTHENTICATION_EVIDENCE.md`. The `0.0.0.0/0` authorized-network entry is retained only as a fallback through the 30-day stabilisation window and will be removed in Phase 10 (24h after IAM auth is stable). **HTTP edge:** Rate limiting middleware exists (`lib/middleware/apiSecurity.ts`). Cloud Armor WAF still pending. | **TODO:** Enable Cloud Armor WAF (HTTP edge protection). **Phase 10 (queued, +24h):** Remove `0.0.0.0/0` from Cloud SQL authorized networks. **Phase 11 (queued, +30d):** Remove the legacy `DATABASE_URL` runtime path from `lib/db.ts` and rotate / disable `monitrax_user`. |
