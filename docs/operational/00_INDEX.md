...
| 12 | [Claude Code MCP Setup](runbooks/12_CLAUDE_CODE_MCP_SETUP.md) | How the repo-root `.mcp.json` file wires Claude Code sessions to external MCP servers — added 2026-05-20 to give the agent direct Vercel log access during live debugging. Covers: token provisioning + storage in the cloud env var, rotation procedure (every 90 days), adding/revoking servers, security considerations. Currently configures `vercel` MCP at `https://mcp.vercel.com` with `${VERCEL_TOKEN}` expansion. |
