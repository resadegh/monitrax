...
| **Anthropic Claude API** (`@anthropic-ai/sdk`) ✅ **WIRED 2026-05-10** | Live AI chat triage in consumer feedback drawer (Phase 33g.2). `claude-haiku-4-5` default; `claude-opus-4-7` reserved for synthesis-only | Per-token. Haiku 4.5 cheap; Opus 4.7 expensive. With prompt caching expect $0.10–0.30 per feedback thread | ~AU$5–25 | _fill_ | **Hard cap set at console.anthropic.com**: $50/mo budget with 50/90/100% email alerts. Set `ANTHROPIC_API_KEY` in Vercel env to enable. When key absent, drawer falls back to form-only — no AI calls fire. Service path: `lib/services/feedbackService.ts:respondToFeedbackThread()` |
