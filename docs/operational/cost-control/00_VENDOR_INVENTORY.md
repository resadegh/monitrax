...
| **Anthropic Claude API** (`@anthropic-ai/sdk`) ✅ **WIRED 2026-05-10** (extended 2026-05-17) ✅ **API key live in Vercel 2026-05-17** ✅ **Spend cap set 2026-05-17** | **Two consumers sharing one key:** (1) Phase 33g.2 — live AI chat triage in consumer feedback drawer; (2) Phase 12 Track E — conversational onboarding extraction (8-topic chat chain on `/onboarding`, structured-output tool calls only, NO advice surface). Both use `claude-haiku-4-5` default; `claude-opus-4-7` reserved for synthesis-only | Per-token. Haiku 4.5 cheap; Opus 4.7 expensive. With prompt caching: $0.10–0.30 per feedback thread + ~AU$0.003–0.006 per onboarding chat walkthrough (~30-60 extractions × ~$0.0001 each = ~AU$50/mo covers 10,000–25,000 walkthroughs). | ~AU$5–25 | _fill_ | **Hard cap set at console.anthropic.com 2026-05-17**: $50/mo budget with 50/90/100% email alerts (covers BOTH consumers — single key, shared workspace). Key live as `reza-onboarding-api-key` (Default workspace) — name is cosmetically misleading (covers both consumers, not just onboarding); optional rename to `monitrax-prod` later. **Pending verification**: confirm the $50/mo cap is on the SAME workspace as the key (Anthropic caps are per-workspace — if cap was set on a separate `monitrax-prod` workspace, it doesn't apply to the Default-workspace key). `ANTHROPIC_API_KEY` set in Vercel Production env. When key absent, feedback drawer falls back to form-only AND onboarding chat returns 503 `AGENT_NOT_CONFIGURED` (calm error banner; UI suggests switching to form). Service paths: `lib/services/feedbackService.ts:respondToFeedbackThread()` + `lib/ai/onboarding-agent/gateway.ts:extractWizardStepDelta()`. Per-user daily cap of 200 extractions enforced at the onboarding extract endpoint via audit-log count over rolling 24h. |
