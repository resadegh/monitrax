...
### 2026-07-30 — MON-131 Tranche 1 START GATE cleared: §1.1 leaf-jump ruled, `?format=hash` persistence, expectedMoves committed (moves NO numbers)
- **Type:** fix/docs (T1 start gate) | **Scope:** `lib/matrix/goldenBaseline.ts` (`hashBaseline` — THE canonical hash, one construction for route/CLI/tests), `app/api/admin/matrix/golden-baseline/route.ts` (`?format=hash` — ~400-byte committable summary), `tests/matrix/goldenBaselineRelay.test.ts` (+3 ratchets incl. the partial-capture tripwire), `.audit/expected-moves-t1.json` (NEW), ledger + build-brief amendments, registry MON-137 raised + MON-135 VERIFIED.
- **§1.1 ruling (the gate that blocked everything):** `goldenBaseline.ts` is **byte-identical** between `4e6cdd5c` and `d3d7e147` — the 1,767 → 10,254 leaf jump is NOT instrumentation. Explanation (drift log **D8**): a failed capture writes a string `__captureError` stub in-tree (0 numeric leaves) while still counting as a tree — today's tree minus `getUserTaxPosition` (7,924) and `generateHealthReport` (566) is **1,764 ≈ 1,767**; the old operand was never persisted so exact attribution is impossible (the D5 lesson restated). The `d3d7e147` capture is the reference — internally verified 48/48 rendered + 9/9 producer↔render + per-tree decomposition. **Hardening:** the hash summary carries `captureErrors`; non-empty = not a valid baseline — this class can't recur silently.
- **§1.2 persistence:** `?format=hash` returns `{sha, capturedAt, trees, leafCount, treeHash, perTree, captureErrors, renderedPartC}`. Canonical construction defined in code (sorted `path=value` lines, SHA-256); VR-042's in-session hash predates the definition and is not comparable — **the first hash-mode capture post-deploy is the committed reference of record.**
- **G3 closed:** `.audit/expected-moves-t1.json` — 14 per-path declarations anchored to VR-042 measured values (the V2 gross-carries-net relabels computed exactly; engine-dependent afters marked `PENDING_RELAY`, to be filled via relay old-vs-new during the T1 build, before merge — a PENDING entry at merge fails G3). **PAYG withheld $11,129 removed from the regression cluster** (may be the wrong of two candidates); the only removal, itself declared.
- **Build brief amended (start-gate §2):** §1 carries the real MON-128 mechanism (a different producer's net total mislabelled gross — not a net-exceeds-gross paradox) + **MON-137** (the cashflowOrchestrator's second withholding producer: three PAYG values 9,932.00/11,128.70/36,197.69, identity broken by $1,196.70/yr, PAYG deducted twice); §3 gains the identity invariants (`cashflow.annualGrossIncome ≡ income.annual.all.grossTotal`, `grossTotal − netTotal ≡ paygWithholding`); §2.1 rental gains the third value (154,443.11 gross≡net).
- **Registry:** **MON-137 raised** (critical, changesNumbers, rootCause verified at `cashflowOrchestrator.ts:131/:319/:400/:403/:491`) · **MON-135 → VERIFIED** (VR-042 §1 Ring-3 PASS — 48 figures zero movement). **Ledger:** baseline DONE (+persistence caveat) · MON-135 DONE (G8 filled; prod sha corrected to `d3d7e147`) · **T1 → READY** (G1/G4/G5 evidenced, G3 committed) · drift **D7** (G5 row contradicted the build brief's own §0) + **D8**.
- **Coverage (§22.2.4):** the hash ratchets verify determinism, single-leaf sensitivity and the error-stub tripwire on synthetic trees; they do NOT verify the endpoint against production data (that is the Matrix's first hash-mode capture); the §1.1 explanation is structural + arithmetic, not an exact attribution (the old operand no longer exists — stated in D8).

### 2026-07-30 — MON-131 handover: Code owns the repo record; sandbox-only artefacts recovered; ledger corrected (docs only)
- **Type:** docs (governance handover) | **Scope:** the Matrix now does live verification ONLY and no longer pushes — it reports findings, Code records them. This PR executes the handover brief's pre-merge items.
- **Recovered:** `docs/verification/runs/VR-040.md` — committed verbatim from the unmerged `matrix/vr-038` branch (`728b95a8`); it was referenced by three docs but reachable on no mainline ref.
- **Ledger corrections** (`MON-131_TRANCHE_LEDGER.md`): the "Baseline captured — DONE" cell cited a relay call, not a committed artefact — flipped to **BLOCKED** (nothing exists under `.audit/golden-baseline-*.json`; the 1,767-leaf capture at `4e6cdd5c` was sandbox-only) and recorded as drift-log **D5**; contract count corrected everywhere from 49 → **48** (45 quantity + 3 register/index) as drift-log **D6**; new **§4b** documents the MON-112…124 register gap as deliberate (112–114 reserved for the unexecuted raise portion of `vr038-039-advance`; 115–124 = VR-040's findings, seven demoted to observations under the tracing rule, re-filed only after re-tracing; ids never reused); MON-135 row updated with the PR #1538 build evidence.
- **Discoverability:** `STATE.md` §B gains the MON-131 doc-set pointer block (build specification · tranche ledger · design record · decisions register · D42 corrections) + a fresh §C cursor at `c07669e5` (the old cursor was pinned 2026-07-15 — fifteen days stale); `01_ACTIVE_WORKSTREAMS.md` §0·REF spec-of-record now points at the Build Specification and Ledger first, with the brief-is-a-slice-of-the-spec and acceptance-criteria-pre-committed rules stated.
- **Blocker 1 CLEARED in this PR:** `.audit/rendered-baseline-8700b1d7.json` handed off via `matrix/rendered-baseline-handoff` (`bddc3de6`) and committed here — the VR-041-class rendered Part-C reference (live PROD at `8700b1d7`, Reza's account, read-only). **Still blocked:** the golden-baseline capture (confirmed absent from every clone — the 1,767-leaf capture existed only in the Matrix's browser session); requires one fresh relay capture at current main, BEFORE any code-touching merge (ledger cell + D5 hold the gate).

### 2026-07-30 — MON-135: the AI categoriser stops asserting "one-off" (the T3 precondition — moves NOTHING on screen)
- **Type:** fix (critical precondition) | **Scope:** `lib/bank/aiCategorisation.ts` (tri-state prediction + evidence-based recurrence overlay), `lib/bank/aiLearning.ts` + `lib/bank/reviewQueue.ts` + import/basiq write sites (null handling), `prisma/schema.prisma` + migration `20260730000000` (two `aiIsRecurring` columns → nullable, constraint loosening only), fixtures in `ring2.calcSsotWall` + `cascadeReconcile`.
- **What was wrong (plain):** the AI categoriser stamped every suggestion with "this is a one-off" (`isRecurring: false`) — not because it judged the payment, but as a default. Harmless today; the moment Tranche 3 routes every run-rate through the one-off gate, that default would have silently removed every AI-categorised recurring cost from every monthly figure at once — and the golden baseline would have absorbed the loss inside T3's legitimate "spending goes down" moves (brief §2).
- **The fix (at the producer):** predictions now carry `null` = "no determination"; `false` may only ever mean a REAL one-off finding. Where evidence exists, recurrence IS determined: the previously fetched-but-unused learned recurring patterns (user-confirmed merchant + cadence, amount within the ONE ≤10% tolerance) now set `true` + the learned frequency — reuse, not a new heuristic. The canonical gate (`monthlyRunRate`, strictly `=== false`) was verified at source: null flows through at the full rate; all seven `=== false` gates in `lib/` are strict (no falsy-treating producer exists).
- **Remediation census (§3.3):** the feared "AI-stamped Expense rows" cohort is structurally EMPTY — AI paths never mint declared rows (classifyIntake wall, CI-enforced) and the link dialog prefills from transaction-cadence evidence, not the AI stamp. The only ambiguity is the pre-MON-053 dialog-default cohort, indistinguishable from user choice in the schema — the provenance gap stated for Reza with the census query, no data overwritten.
- **Permanent guards:** Wall-B2 tri-state fixtures (Float + Decimal: null/unset → full rate, explicit false → 0) + categoriser never-emits-false on the REAL prediction paths + the evidence-overlay + transfer-branch tests. Gate evidence: `docs/implementation/MON-131_TRANCHE_LEDGER.md` §3.0 (NEW — the per-tranche evidence record).
- **Coverage (§22.2.4):** verifies the gate tri-state + the emission contract on real pure paths; does NOT verify the import route against a live DB or the review UI's rendering; the moves-NOTHING claim is proven by the Matrix relay diff on real data post-merge (required verdict: CLEAN), not by these tests.

### 2026-07-29 — MON-131 Phase A: the Number Inventory — 68 named quantities, one verdict each (READ-ONLY, moves NO numbers)
- **Type:** docs (Phase A gate deliverable) | **Scope:** `docs/architecture/NUMBER_INVENTORY.md` (the decision artefact), `docs/architecture/contracts/` (48 contract files — 45 quantity + 3 register/index; header corrected 2026-07-30, drift D6), `scripts/census/producers-census.mjs` (Phase A0 census extension — committed earlier today), NO production code.
- **What this is (plain):** the full report Reza asked for — every number the app computes, what it means, how many places compute it, and a verdict (CLEAN / MULTIPLE / WRONG / UNNAMED / UNVERIFIABLE), before anything is fixed. 45 MON-131 rows + 16 MON-136 families + 23 new quantities proposed from the 424-function unattributed sweep.
- **Method:** Phase A0 complete census first (40 quantities · 1,307 sites · 424-function sweep bucket; D16 — nothing out of scope), then 13 parallel read-only contract agents (one per quantity cluster, D14), then **5 adversarial refutation agents re-verified every load-bearing claim** (~200 anchors re-checked at HEAD, worked examples recomputed, negative claims independently re-grepped). 8 contracts materially corrected; 2 verdicts changed by the adversarial pass (property-cashflow CLEAN→MULTIPLE; loan-monthly-interest strengthened to MULTIPLE+WRONG).
- **Headline findings:** a **live 100× bug on Home** (EntityCashflowSummary:693 treats the schema's decimal rate as a percent — interest estimate 100× too low); runway shows **11.6 months** where the D3/D4 definition derives **72.57**; stale super constants ($27,500 cap, SG 11.5%) live INCLUDING a writer stamping 11.5% onto Income FACT rows; an abolished 32.5% tax band still computing; the GlassBudgetTile's budget side built from storage-blob metadata keys; a **fourth** multi-year-projection producer that has the LLM invent the numbers; two dead forecast stacks (CFE routes: 0 fetchers; next-month forecast: computed then discarded); `insuranceGapsScore = 70` hardcoded and rendered as a scored metric.
- **For Reza:** Part 3 of the inventory — **28 decisions required · 12 preconditions (fixes that harm if mis-ordered, MON-135 class) · 14 wrong-input findings.** Phase B writes nothing until the §5.1 calls are made.
- **Coverage boundary (§22.2.4):** static-only; rendered values quoted only from the golden baseline/VR-041; census v1 heuristic blind spots itemised (top-level consts, 5 missed loan-cost sites); MON-136 register + sweep NOT adversarially re-verified (register-entry depth per brief); ~25 forecast units and ~95 sweep functions classified by name only — all named in Part 4.
- **Gate (§20.6): Document 10/10 (brief + NUMBER_INVENTORY_SPEC conformed — census → contracts → inventory, stop there) · Requirements 10/10 · Logic 10/10** (the adversarial pass IS the recorded critique; corrections listed per contract in each `## Adversarial review (§7)` section).
