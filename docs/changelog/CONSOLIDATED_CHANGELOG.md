...
- **Still blocked, stated honestly:** (1) `.audit/rendered-baseline-8700b1d7.json` exists on NO ref anywhere — it cannot be recreated by Code and must be handed over by the Matrix/Reza; (2) the golden-baseline capture requires the relay (admin browser session) or CLI+`DATABASE_URL`, neither of which the Code sandbox has — the Matrix captures, Reza/Code commits, BEFORE any code-touching merge (ledger cell + D5 hold the gate).
