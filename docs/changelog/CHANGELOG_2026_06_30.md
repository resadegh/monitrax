...
- **Visual-only changes to the desktop row:**
  - Direction icon → a **luminous 40px gradient gem** (`rounded-[12px]`, the same `gemGradient` the mobile row uses) with an **unreviewed sky dot**, replacing the flat `bg-rose-50` tinted square.
  - **Gem colour (Reza decision 2026-06-30, 4-lens):** outgoing → **neutral SLATE** (was indigo); income → emerald; transfer → a lighter slate (the ⇄ icon differentiates). Reza asked "why purple — wouldn't red make sense?"; the 4-lens review recommended a calm neutral over BOTH indigo and red — red is reserved for true loss/attention (§18.7.2), and a wall of red debit rows reads as constant alarm (behaviour-psychology). The ↑/↓ arrow + "−" already carry direction. Applied to mobile + desktop (shared `gemGradient`).
