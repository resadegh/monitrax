...
### Update 7 (2026-05-13) — GTM Step 1.1 COMPLETE — n8n live at https://n8n.monitrax.com.au
- **Full install done on the Hetzner CPX22:** Docker + Docker Compose plugin, `/opt/n8n` directory, `docker-compose.yml` (n8n + Postgres + Caddy services), `Caddyfile` for the public reverse-proxy, `.env` with a 32-byte n8n encryption key + 24-byte Postgres password (file mode 600). UFW (allow 22/80/443 only), `unattended-upgrades` enabled, fail2ban running, SSH hardened (PasswordAuthentication no, PermitRootLogin prohibit-password). All 3 containers Up, postgres healthy.
- **Owner account** created at the n8n UI; password stored in Reza's password manager. n8n free-license-key registered to `admin@monitrax.com.au` (unlocks Folders, Execution search/tagging, Advanced debugging — paste the key in Settings → Usage when it arrives).
- **Mid-session URL migration: `n8n.try-monitrax.com` → `n8n.monitrax.com.au`.** Reason: Chrome Safe Browsing flagged the `try-monitrax.com` subdomain ("Dangerous site" — new-domain heuristic). Architectural correction baked into the docs going forward: `try-monitrax.com` is **email-only** (Smartlead cold-outbound + the `reza@try-monitrax.com` mailbox); `monitrax.com.au` is **brand + internal tools** (the production app + `n8n.monitrax.com.au` + future Documenso, landing pages, etc.). `sed` swap of Caddyfile + docker-compose hostname references + container restart; Caddy auto-fetched fresh Let's Encrypt cert. Cold-email setup on `try-monitrax.com` untouched. Old `n8n.try-monitrax.com` DNS A record at GoDaddy can stay or be deleted — Caddy isn't configured to serve it any more (harmless either way).
- Docs updated: `GTM_EXECUTION_PLAN.md` (Step 1.1 → ✅ DONE with the full record + the migration note; status tracker 1.1 ticked). `GTM_TOOL_STACK.md` (n8n row → 🟢 Active at the new URL; VPS row → access via `n8n.monitrax.com.au`). `IMPLEMENTATION_PLAN.md` workstream 0d updated to reflect n8n is live.
- **Next:** Step 1.6 (Founder Daily Digest — first real workflow). Optional preceding step: install the n8n MCP connector so Claude can build/inspect workflows directly via API instead of click-by-click UI walkthroughs (recommendation in the chat).
