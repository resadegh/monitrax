...
### Stage B — ONE PR: super attachment + universe coverage + derived-ownership helper
- **B1 migration** `20260610090000_backfill_super_owner_entity` — retail/industry super backfilled to the member's PERSONAL_NAME entity (Q-EOF-2). §12.11: UPDATE confined to `ownerEntityId IS NULL` rows (can never overwrite an SMSF link); only the attribution pointer touched. Fix-forward: the super create route now defaults non-SMSF accounts to the personal entity. Schema comment updated to the new semantics (no structural change).
- **B1 universe coverage** — wealth graph fetches super accounts; new asset kind `'super'` (subtype SMSF member / Retail / Industry, fund name as context); layout maps it to the investment vocabulary + a "Super" cluster label; detail panel + mobile list gained the PiggyBank glyph, `/dashboard/investments/super` href, and "Super" label cases.
...
### Picker discoverability hint (Reza feedback: "doesn't have a trust or company ownership")
- Root cause: progressive disclosure hid the "An entity" card entirely when no structure entity exists — correct restraint, zero signposting. The picker now shows a quiet dashed hint card in that state: "A trust or company? Add it in My Structure first — then pick it as the owner here" (links to `/dashboard/entities`). Layered structures (you → trustee of trust → trust SHAREHOLDER_OF company → company holds the property) are already fully capturable via Phase 44 relationships + the entity picker; this closes the discoverability gap.

### Stage C — ONE PR: entity-aware numbers (byEntity breakdown + dashboard entity lens v1)
- **C1**: NEW canonical calculator `lib/calculations/entityBreakdown.ts` — partitions the SAME raw rows by `ownerEntityId` and computes per-entity positions via the canonical engines (`calculateNetWorth` per partition, `toMonthly` streams). `masterFinancialService` fetch selects `ownerEntityId` everywhere (holdings derive via their account, §4B D3) + entity refs; snapshot gains the ADDITIVE `byEntity` field — flat household numbers untouched. **Additivity invariant pinned by 5 new tests** (Σ per-entity == household engine result; null owners → Unattributed bucket that still reconciles). Stake-splitting deliberately deferred to Stage D (legal-title attribution at C — splitting display values before tax splits would make surfaces disagree; documented in the calculator header).
- **C2 (scope decision, delegated)**: entity lens v1 = `EntityBreakdownWidget` ("MY STRUCTURE · By entity": per-entity net position + monthly cashflow + holdings count, honest title-attribution footnote) in the documented empty column beside the universe widget; self-hides below 2 entities. A GLOBAL dashboard filter pill is deferred until Stage D gives it per-entity tax payoff — re-evaluate then.
- **C3**: universe ↔ dashboard parity is by construction at Stage C — both group by the same `ownerEntityId` column; per-stake display parity arrives with Stage D splitting.
...
### Phase WX.5 — microscopic camera zoom between layers (Reza: "the camera moves to the next layer and expands that bubble into the page… like a microscopic zoom")
- **Root cause of the cramped feel**: WX.4's expansion popped satellites around the bubble IN the same flat view — on mobile they collided with the other clusters (screenshots). No camera, no layers.
- **Focused scene (layout)**: a single expanded id now returns a SCENE — the tapped bubble re-centres at (50,42) and enlarges, its items form a clean full ring (two rings >8), and every other node leaves the stage (you are INSIDE the bubble; the breadcrumb is the way back). Works for entities, joint/shared groups, and type clusters. New `ringAround` helper.
- **Camera (canvas + mobile)**: scenes are keyed AnimatePresence layers. Zoom IN: the old layer magnifies toward the tapped bubble's exact position (transform-origin = tap point) and racks out of focus (blur 10px) while the inner layer sharpens in from 0.65×; zoom OUT reversed. 0.5s appleEase; `useReducedMotion` collapses it to a crossfade.
- Tests updated to the scene contract (21 layout tests green). Stitch artefact for the focused-scene composition rides the owed variants pass (noted in plan).

### WX.5.1 — premium navigation polish (Reza feedback ×4 on the camera)
- **No way back (real bug)**: mobile had NO exit from a scene (the toggle only existed on desktop). Now: floating "‹ Universe" glass pill on the canvas (scene mode), tapping the focused bubble again flies out, and desktop accepts the same toggle. All exits play the reverse camera.
- **Animation imperceptible on iPhone**: iOS Safari frame-drops animated `filter: blur` — the camera ran but invisibly. Mobile is now scale+opacity only with a deeper ride (0.5↔2.4×, 0.6s); desktop keeps a light 4px focus-rack. Widget deep-links also play the arrival zoom (origin = the focused bubble).
