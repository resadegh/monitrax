...
## Session: yhm8ug (continuation 26, 2026-07-27) — MON-103 (capture Stage 3): Div 152 concession facts — the THIRD and FINAL overlay made live-capable; the capture feature is complete

### Changes Made
- **Type**: Feature (schema + assembler + Stitch-first UI; `changesNumbers: yes-CONDITIONAL` — fires ONLY once an entity has a saved Div 152 assessment with all six facts; everyone else byte-identical, locked)
- **Reza standing GO (2026-07-27, "go with stage 3")**: built on the established Stage-1/2 pattern; the Stitch design presented BEFORE any React (screen `8ff4092f751544688de33f8574c1232b`, §18.8 **9.2/10** — v1 8.3: Stitch's emerald primary leaked into the election pills / Save CTA / result strip; v3: full indigo-family identity, neutral outcome strip, quiet info line). **Disclosed:** several of Stitch's reported DOM ops never persisted to the export endpoint — the exact approved-intent fixes (gradient Save button, indigo `.option.selected` rule, ivory outcome panel, slate reductions, indigo ladder dots, quiet info line) are hand-applied in the committed artefact `div152-concessions-card.html`; the committed PNG is a local Chromium render (offline substitution of fonts/icons — the build uses Inter + Lucide; `-stitch-v1.png` preserves Stitch's original typography render).
- **Schema (§12.12, Reza's migration click)**: `Div152Assessment` — 4 nullable numerics (gain/MNAV/turnover Decimal(19,4); monthsHeld Int) + 2 nullable eligibility Booleans + 3 nullable election Booleans + nullable `retirementExemptionUsedToDate`, `@@unique([entityId, financialYear])`; + `AuditAction.DIV152_ASSESSMENT_SAVED`. Migration `20260727120000_mon103_div152_assessment` (CREATE TABLE + ADD VALUE — additive only, no existing row touched).
- **The load-bearing doctrines**: (1) `buildDiv152Input` (the ONE producer, `entityTaxFactsAssembler.ts:264`) returns null unless the FOUR numerics AND the TWO eligibility booleans are all explicitly set — for `isActiveAsset`/`isRetirementOrIncapacity` NEITHER default is safe (false denies real concessions on display; true fabricates them), so "Not sure" keeps the overlay inert. (2) Elections spread only when answered (null → absent key → the ENGINE's own contract defaults: reduction ON s152-205, exemption OFF s152-305, rollover OFF s152-410). (3) Electing the retirement exemption REQUIRES `retirementExemptionUsedToDate` — a blank never assumes $0 of the $500k lifetime cap already used (that would OVERSTATE the exemption).
- **Flow**: card → PUT `/api/tax/entity/[id]/div152-assessment` (null-preserving; audit `DIV152_ASSESSMENT_SAVED`, §13.3-sanitized) → `assembleDiv152Input` gates → entity route GET feeds `div152ByEntity` (third Promise.all element) → the MON-099 overlay fires with the step ladder + citations.
- **UI (per the presented design)**: `Div152AssessmentCard` below the PSI card for the same PSI entity types. Three sections (The disposal / The size tests / Retirement & elections), tri-states for the two eligibility questions, two-state election pills with the engine defaults pre-selected, the conditional lifetime-used field ("we never assume zero"), the amber doctrine note, and the LIVE step-ladder result strip reading `crossCutting.div152ByEntity[entityId]` (never re-derived).
- **Ratchet**: `tests/tax/mon103Div152Capture.test.ts` (8 tests): the gate in all six directions; elected-exemption-requires-lifetime-used; unanswered elections → absent keys; answered false passes verbatim; §19.2 worked example **$120,000 → AAR −$60,000 → retirement exemption −$60,000 (cap remaining $100k) → assessable $0** end-to-end through BOTH twins with s152-205 + s152-305 cited; the 15-year direction (190 months → $0, s152-105); gate-closed byte-parity.
- **Neo-sync (§21.2.1)**: `buildDiv152Input` modelled (node + feeds edge to `engine.tax.div152.applyDiv152` with the route evidence); ALL THREE assembler anchors re-pinned +1 (the `Div152Input` import shift: div7a 81→82, buildFteIeeInput 119→120, buildPsiInput 184→185, prose refs :150/:228); div152-assessment route Layer-0-allowlisted (graphify offline — recorded limitation, self-prunes); `GENERATED_CORE.md` regenerated; `neomatrix:check` green.

### Files Modified
- `prisma/schema.prisma` + `prisma/migrations/20260727120000_mon103_div152_assessment/`
- `app/api/tax/entity/[entityId]/div152-assessment/route.ts` (NEW) — GET/PUT persistence
- `lib/services/entityTaxFactsAssembler.ts` — `buildDiv152Input` + `assembleDiv152Input` (the ONE producer)
- `app/api/tax/entity/[entityId]/route.ts` — GET feeds `div152ByEntity`
- `app/dashboard/entities/[id]/tax/page.tsx` — Div 152 card render + crossCutting parse
- `components/tax/Div152AssessmentCard.tsx` (NEW) — the presented card + JSDoc Stitch ref
- `tests/tax/mon103Div152Capture.test.ts` (NEW, 8) · `.stitch/designs/capture-stage3/*` · graph + coverage-allowlist + registry + docs

### Build Status
- [x] tsc clean · [x] mon103 8/8 + tax/tax-engine/golden/neomatrix/issues/intake 1,473 passed (97 files) · [x] `npm run build` passes · [x] `neomatrix:check` green · [x] both lints green · [x] `issues:check` 103 valid

### Gate (§20.6)
`Gate (§20.6): Document 10/10 (built to the presented §18.8 9.2/10 Stitch design + the capture brief's Stage-3 spec under Reza's standing "go with stage 3"; §12.12 migration in-PR for his click; the design shown BEFORE any React with the deviation-handling disclosed) · Requirements 10/10 (schema + API + assembler + route feed + UI, all through the ONE producer; the three doctrines stated and enforced) · Logic 10/10 (§19.2 hand-computed $120k→$0 ladder verified against the READ engine source and locked end-to-end both twins; the gate proven in all six directions + the lifetime-used rule; gate-closed = byte-identical, locked)`
Coverage: verifies the gate, the mapping, and the orchestrator flow from a built input; does NOT verify the Prisma read path of `assembleDiv152Input` (Ring-3 on a seeded entity owns that) and does NOT verify the rendered card (Reza's eyeball + Ring-3).
