...
- Adapter: `CategorisationResult → AICategorizationResult` maps category/confidence/source, defaults essential false, derives direction, undefined → Uncategorised (never drops a row) ✓. **MON-135 (2026-07-30): `isRecurring` is `null` (no determination), NEVER a default `false`** — `false` is a one-off assertion the T3 run-rate gate acts on; recurrence is set `true` only from learned recurring-pattern evidence (merchant + amount within the ONE ≤10% tolerance) in `categoriseWithLearning` step 4b.
