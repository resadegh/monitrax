...
4. 🟡 **Q-DEC PR 2 — engine adapter layer (IN FLIGHT — 10 sub-PRs landed, PR 2.D.3d open as #989, PR 2.E queued)** — Reza decision 2026-06-04: **split by directory** for review tractability + smaller blast-radius per merge. Engines return BOTH Float and Decimal during PR 2 (back-compat); PR 3 swaps the primary; PR 4 drops Float. Operational refinement mid-build: PR 2.D (`lib/tax-engine/*`, ~12.2 k LOC across 45 files) further split into D.1 / D.2 / D.2b / D.3a / D.3b / D.3c / D.3c2 / D.3d for review tractability.
   ...
   - 🟡 **PR 2.D.3d — composers + remaining (IN FLIGHT — PR #989)** — `calculateSmsfIncomeTax` (Div 295 / NALI / ECPI / refundable franking, ECPI honesty `tax: null` preserved) + `processSalary` (NET→GROSS reverse binary-search + PAYG NAT 1004 + Medicare + super). **`entityTaxRouter` + `masterTaxPosition` deferred to PR 3** — pure aggregating routers, no marginal benefit to composer-tier Decimal siblings when underlying engines are already Decimal-capable; PR 3 cuts the boundary over.
   - ⏳ **PR 2.E — `lib/cfo/*`** (next) — scenarios + intelligence engine. Unblocks Phase 45 PR 1 once landed.
