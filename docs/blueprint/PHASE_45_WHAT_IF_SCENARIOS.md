...
4. 🟡 **Q-DEC PR 2 — engine adapter layer (IN FLIGHT — 14 sub-PRs landed, PR 2.E.4 open as next final 2.E sub-PR)** — Reza decision 2026-06-04: **split by directory** for review tractability + smaller blast-radius per merge. Engines return BOTH Float and Decimal during PR 2 (back-compat); PR 3 swaps the primary; PR 4 drops Float. Operational refinement mid-build: PR 2.D (`lib/tax-engine/*`, ~12.2 k LOC across 45 files) further split into D.1 / D.2 / D.2b / D.3a / D.3b / D.3c / D.3c2 / D.3d for review tractability; PR 2.E (`lib/cfo/*`, ~6.6 k LOC across 19 files) further split into E.1 (scenarios) / E.2 (score+risk) / E.3 (decision support) / E.4 (actions+AI+intelligence).
   ...
   - ⏳ **PR 2.E.4 — actions + AI advisor + intelligence** (next, final 2.E sub-PR) — `actionEngine` + `aiAdvisor` (tool-call dispatcher, mostly presentation but composes scenarios) + `intelligenceEngine` (cross-engine composer). After this lands, **all of PR 2 is complete** and Phase 45 PR 1 is unblocked.
5. Q-DEC PR 3 — engine-by-engine cutover (route handlers consume Decimal; `entityTaxRouter` + `masterTaxPosition` get their Decimal siblings here once downstream consumers are Decimal too; components format via `formatCurrency()`).
