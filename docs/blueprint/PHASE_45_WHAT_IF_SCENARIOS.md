...
4. ✅ **Q-DEC PR 2 — engine adapter layer — COMPLETE (16 sub-PRs landed 2026-06-03 → 2026-06-06)** — Reza decision 2026-06-04: split by directory + by theme for review tractability + smaller blast-radius per merge. Engines returned BOTH Float and Decimal during PR 2 (back-compat); PR 3 (in flight) swaps the consumer primary; PR 4 drops Float. Operational refinement mid-build: PR 2.D (`lib/tax-engine/*`, ~12.2 k LOC across 45 files) split into D.1 / D.2 / D.2b / D.3a / D.3b / D.3c / D.3c2 / D.3d; PR 2.E (`lib/cfo/*`, ~6.6 k LOC across 19 files) split into E.1 (scenarios) / E.2 (score+risk) / E.3 (decision support) / E.4 (actions+AI+intelligence).
   - ✅ **PR 2.A — foundation + `netWorthCalculator` proof** (MERGED PR #978) — `lib/decimal/` (`toDecimal`, `fromDecimal`, `decimalDiff`, `isCloseEnough` with policy-driven rounding) + `lib/calc-audit/shadowComparison.ts` extending Phase 41I + `netWorthCalculator` Decimal sibling as the proof of pattern.
   - ✅ **PR 2.B — rest of `lib/calculations/*`** (MERGED PR #979) — `expenseAggregator` / `incomeAggregator` / `loanAggregator` / `cashflowOrchestrator` Decimal siblings + `toAnnualDecimal` / `toMonthlyDecimal` on `lib/utils/frequencies.ts`. Two latent bugs fixed (`Decimal(0).isPositive()` truthiness; cashflow ratios use `'currency'` tolerance not `'percentage'`).
   - ✅ **PR 2.C — `lib/cashflow/incomeNormalizer.ts`** (MERGED PR #980) — `calculateTakeHomePayDecimal` unblocks PR 2.B's Float-bridge; `normalizeIncomeStreamDecimal` + `normalizeAllIncomeDecimal` + `getEffectiveMonthlyIncomeDecimal`. Harness improvement: shadow comparison skips non-numeric leaves.
   - ✅ **PR 2.D.1 — `lib/tax-engine/core/*`** (MERGED PR #981) — `paygCalculator` / `medicareLevyCalculator` / `taxOffsets` / `incomeTaxCalculator` Decimal siblings (~970 LOC); unblocks PR 2.C's inner Float-bridge.
   - ✅ **PR 2.D.2 — `lib/tax-engine/super/* (3 files)`** (MERGED PR #982) — salary-sacrifice critical path: `capTracker` (H3 Phase 45 anchor — concessional-cap headroom with carry-forward + bring-forward), `contributionCalculator` (SG + sacrifice + Div 293 detection), `highIncomeSuperTax` (Div 293 + Div 296 gated + TBC).
   - ✅ **PR 2.D.2b — `income/taxabilityRules + position/taxPositionCalculator`** (MERGED PR #983) — `calculateFrankingCredits` / `determineTaxability` / `calculateTaxPosition` / `calculateQuickTaxPosition` Decimal siblings.
   - ✅ **PR 2.D.3a — state taxes (`landTax/` + `stampDuty/` + `gst/`)** (MERGED PR #984) — `calculateGst` + `calculateStampDuty` + `calculateLandTax` + `calculateCrossStateLandTax` Decimal siblings (self-contained, reform-agnostic).
   - ✅ **PR 2.D.3b — CGT division engines** (MERGED PR #985) — `cgtDiscount` (Div 115 entity-rate dispatch with §12.14 FW-1 Measure 2 reform guard preserved) + `capitalLossNetting` + `cgtIndexation` + `cgtMinimumRate` + `foreignResidentCgt` (FW-2 skeletons preserved).
   - ✅ **PR 2.D.3c — loss-treatment engines** (MERGED PR #987) — `negativeGearing` (real arithmetic + FW-1 Measure 1 regime) + `companyLossRules` + `trustLossRules` (test-outcome dispatch) + `trustMinimumTax` + `lossRefundability` (FW-2 skeletons).
   - ✅ **PR 2.D.3c2 — beneficiary allocation + business concessions** (MERGED PR #988) — `trustDistribution.allocateTrustDistribution` (Div 6 + Div 6E streaming + s99A) + `div7aLoanClassifier` (exact annuity via `Decimal.pow` + surplus-cap pro-rata) + `div152SmallBusinessConcessions` (15-yr + 50% + retirement + rollover stacking).
   - ✅ **PR 2.D.3d — composers + remaining** (MERGED PR #989) — `calculateSmsfIncomeTax` (Div 295 / NALI / ECPI / refundable franking, ECPI honesty `tax: null` preserved) + `processSalary` (NET→GROSS reverse binary-search + PAYG NAT 1004 + Medicare + super). **`entityTaxRouter` + `masterTaxPosition` deferred to PR 3** — pure aggregating routers, no marginal benefit to composer-tier Decimal siblings when underlying engines are already Decimal-capable; PR 3 cuts the boundary over.
   - ✅ **PR 2.E.1 — scenarios + supporting utils** (MERGED PR #991) — Decimal siblings for all 6 deterministic What-If scenarios (`sellProperty` / `payDownLoan` / `refinanceLoan` / `redirectToOffset` / `cutSpendCategory` / `addInvestment`) + 3 supporting primitive Decimal siblings on `lib/utils/calculations.ts` (`calculatePIRepaymentDecimal` / `calculateInterestForPeriodDecimal` / `calculateEffectivePrincipalDecimal`). New `ScenarioResultDecimal` + `ScenarioImpactDecimal` types — **Phase 45 What-If critical path**.
   - ✅ **PR 2.E.2 — health + risk** (MERGED PR #992) — Decimal siblings for `scoreCalculator` (6 component sub-scores + weighted-overall composer) + `riskRadar.calculateSummary` (counts unchanged; `totalImpact` in Decimal). New `CFOScoreComponentsDecimal` + `RiskSummaryDecimal` types.
   - ✅ **PR 2.E.3 — decision support** (MERGED PR #993) — Pure-math Decimal siblings across all 4 decision-support files (8 helpers): `propertyDecisionSupport.calculatePortfolioSummaryDecimal`; `loanDecisionSupport.{calculateMonthlyPayment, calculatePayoffMonths, calculateTotalInterest}Decimal`; `investmentDecisionSupport.{calculateDividendYield, calculateMaxConcentration}Decimal`; `taxIntegration.{calculateUnrealisedCGT, calculateNegativeGearingBenefit}Decimal`. Async `calculateCFO*Insights` wrappers + categorical alert/risk generators deferred to PR 3 cutover.
   - ✅ **PR 2.E.4 — actions + AI advisor + intelligence** (MERGED PR #995) — `intelligenceEngine` 2 pure-math Decimal helpers (projected month-end balance + monthly-progress net worth). `actionEngine` + `aiAdvisor` intentionally skipped (categorical / presentation-side; rationale in code header). **PR 2 is complete; Phase 45 PR 1 is unblocked once PR 3 cutover lands.**
5. 🟡 **Q-DEC PR 3 — engine-by-engine cutover (IN FLIGHT — 5 sub-PRs landed; 1 queued)** — Reza decision 2026-06-06: 5-way split for review tractability (3.A composers / 3.B tax routes / 3.B.2 tax salary+super / 3.C cfo / 3.D AI tools / 3.E UI). `serializeDecimalsForJson` is the canonical Decimal → JSON boundary walker (PR 3.B).
   - ✅ **PR 3.A — composer-tier Decimal siblings** (MERGED PR #996) — `calculateEntityTaxPositionDecimal` + `buildMasterTaxPositionDecimal` (deferred from PR 2.D.3d; shipped now as PR 3 foundation). Routes dispatch to `*Decimal` siblings already shipped by PR 2.D.
   - ✅ **PR 3.B — tax entity + position routes** (MERGED PR #997) — `/api/tax/entity/[entityId]` (GET + POST) + `/api/tax/position` (GET) swapped. New `lib/decimal/serialize.ts:serializeDecimalsForJson` boundary walker.
   - ✅ **PR 3.B.2 — tax salary + super routes** (MERGED PR #998) — `/api/tax/salary` (POST) + `/api/tax/super` (GET) swapped. Dual-call to Float sources presentational fields (`calculations[]` narration).
   - ✅ **PR 3.C — cfo scenarios route** (MERGED PR #999) — `/api/cfo/scenarios/run` (POST) swapped. New `runScenarioDecimal` dispatcher routes to PR 2.E.1's 6 Decimal scenario siblings.
   - ✅ **PR 3.D — AI advisor tools cutover** (MERGED PR #1001) — All 9 tax-engine-consuming AI tools swapped to `*Decimal` siblings. Per-field `.toNumber()` boundary preserves the AI-facing `number` shape; `raw` field wrapped in `serializeDecimalsForJson`. Bonus parity fix: `calculateLandTaxDecimal` was missing the `UC-MULTI-STATE-LAND-TAX` UNCOMPUTED flag the Float sibling emits unconditionally — now patched (PR 2.D.3a gap closed).
   - ⏳ **PR 3.E — UI consumers cutover** (last 3.x) — components + hooks consume Decimal at fetch boundary; format via `formatCurrency()` (type-abstract).
...
- **Phase 45.2.2 — SMSF detail hero** (this PR, 2026-06-09). Third asset-class application of the §18.7.5 Asset Spotlight template. New route `/dashboard/investments/super/[id]`. Sub-palette stays sky→indigo (same Stage I Invest as properties — SMSF shares the "long-term anchored asset" mood; codified explicitly in §18.7.5 mapping table). Photo: classical institutional lobby (`/public/decor/smsf-lobby.jpg`) generated with the same Architectural Digest-tier prompt that landed apartment + horizon. KPI mapping: hero member balance + concessional YTD with cap-utilization pill, mini-grid Concessional used / Non-Concessional used / 1Y return, 4-tile SG inflows / Salary sacrifice / Personal deductible / Carry-forward. v1 fetches `/api/tax/super` and filters client-side (no new GET endpoint). Stitch design generated via `generate_variants` from v5 properties source (REFINE creative range + TEXT_CONTENT + IMAGES aspects) — same pattern as PR #1027. List page CTA rewired to navigate to the new route.
- **Phase 45.2.1 — investments detail hero** (PR #1027, merged 2026-06-09). First non-property application of the §18.7.5 Asset Spotlight template — proves the template is portable across asset classes with only the per-asset swaps documented in the §18.7.5 mapping table. New route `/dashboard/investments/accounts/[id]`. Indigo→violet sub-palette (Stage I Invest), financial-district skyline photo at `/public/decor/investments-skyline.jpg`. KPI mapping: hero portfolio value + cost basis + gain %, mini-grid YTD return / Asset mix / Cash balance, 4-tile Dividends / Distributions / Franking / Capital gains. Full 4-variant Stitch matrix locked via `generate_variants` from the v5 properties source (REFINE creative range + TEXT_CONTENT + COLOR_SCHEME aspects) — composition, fonts, shadows, spacing inherit verbatim per Reza's "I want all designs to be similar" direction. v3→v4 was a photo swap only (apartment-adjacent → Sydney CBD skyline) per Reza's *"background photo of an apartment doesn't suit the investment page"* feedback. `isolate` baked into the page container from the start (lesson from Phase 45.2 properties prod incident codified into §18.7.5 composition rule 1). List page "View details" CTA rewired. Detail-dialog code in `/dashboard/investments/accounts/page.tsx` now unreachable — flagged in §12.1 follow-up for cleanup.
- **Phase 45.2 — properties detail hero** (PR #1024, merged 2026-06-08). Scaffolded `/dashboard/properties/[id]` detail route (parent to the existing `/strategy` and `/depreciation` sub-routes). First application of the new **Cremorne-Wide variant** (full-page photo canvas + 3-stop ivory/navy scrim, opacity-50 photo per Reza directive). 6 desktop iterations (v1 corner-bleed → v2 Cremorne-Wide → v3 polished tiles → v4 photo opacity -50% → v5 balanced layout via content design → v6 flex-1 structural alignment) — v5 locked. Full 4-variant matrix locked. Three NEW canonical design sub-patterns codified in this PR (Cremorne-Wide variant + Polished tile + Multi-column balance rule) plus the **"Asset Spotlight" template** (§18.7.5) — a reusable end-to-end composition for any single-asset detail page. Future investments / SMSF / loan detail pages apply Asset Spotlight verbatim with sub-palette + content swaps.

**§18.7.4 replicate queue remaining (queued, no commitment):** Income-page banner application is the only remaining candidate (lower priority — banner not protagonist). The three detail-page applications (properties, investments, SMSF) are all shipped.
