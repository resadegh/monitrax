...
| D6 | **What-If levers are single-taxpayer** — sellProperty CGT doesn't split across joint owners / apply entity rates. | 🟡 **HALF-SHIPPED 2026-06-13 (AD-2):** the `EntityTaxFacts` assembler now READS stakes — income/deductions on a co-owned asset split per legal interest (TR 93/32) and beneficial-override assets redirect to the beneficial owner. The What-If levers consuming the per-entity engine outputs remain the open half (levers still single-taxpayer until they call the per-entity path). |
