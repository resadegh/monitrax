...
| D6 | **What-If levers are single-taxpayer** — sellProperty CGT doesn't split across joint owners / apply entity rates. | ✅ **SHIPPED 2026-06-15 (AD-2 + D6):** AD-2 (2026-06-13) made the `EntityTaxFacts` assembler READ stakes; **D6 closes the open half** — the `sellProperty` What-If lever now computes CGT through the canonical per-entity path. New pure core `lib/cfo/scenarios/propertyDisposalCgt.ts` composes `calculateCgtDiscountDecimal` (Div 115 per-entity discount: individual/trust 50%, complying SMSF 33⅓%, company 0%) + the AD-2 `attributeAsset` weighting (TR 93/32: JT equal, TIC per share; beneficial override redirects to the beneficial owner), so the lever and the per-entity tax page agree on who owns what share. **Honesty boundary** (financial-adviser + compliance lenses): every owner's *taxable capital gain* after the correct entity discount is shown (certain from type + holding period); only the **user's own share** is estimated to a tax dollar (at their current marginal rate, clearly labelled — a large gain may push into a higher bracket); a co-owner's tax is never fabricated (shown as taxable-gain + "taxed in their own return"). §12.14 FW-2 preserved — a post-cut-over acquisition surfaces `UC-CGT-POST-REFORM` instead of a silent discounted number. The run route builds the per-property ownership/cost-base/marginal-rate context; absent it (AI-advisor path / no structure) the lever falls back to the prior CGT flag byte-for-byte. 16 D6 tests; full sweep green. |
...
**AD-2 shipped 2026-06-13 — stake & beneficial attribution.** The assembler no longer attributes flat by `ownerEntityId`. It reads `OwnershipStake` (TR 93/32: joint tenants split equally, tenants-in-common follow recorded percentages) and `BeneficialOwnershipOverride` (bare-trust / nominee / LRBA — income follows the BENEFICIAL owner, §4A P2/P3/P5/P8). Pure core: `lib/services/ownershipAttribution.ts`. Honest fall-back to flat + `UC-OWNERSHIP-SPLIT-UNKNOWN` when a TIC split can't be computed (shares missing / not summing to 100%); `UC-OWNERSHIP-SPLIT` / `UC-BENEFICIAL-OWNED` / `UC-BENEFICIAL-REDIRECTED` make every redirect visible. Additive — no group/override → byte-for-byte the prior mapping. See `PHASE_44_PART_2_MONEY_FLOW_TAX_REWIRE.md` §14 AD-2.

**D6 shipped 2026-06-15 — What-If lever per-entity CGT (the open AD-2 half).** The `sellProperty` lever no longer flags CGT as a single undifferentiated "may apply" caution. `lib/cfo/scenarios/propertyDisposalCgt.ts` (pure) composes the canonical `calculateCgtDiscountDecimal` (Div 115) per owner, weighting each by `attributeAsset` (the same AD-2 attributor the assembler uses — JT equal, TIC per share, beneficial override redirects), so the gain splits across joint owners and applies each entity's discount rate. The lever surfaces an "Estimated CGT (your share)" impact + an after-CGT net-worth line + a per-owner taxable-gain breakdown in the assumptions panel. **Honesty discipline:** only the user's own share is converted to a tax dollar (their current marginal rate, labelled an estimate); co-owners show taxable gain only ("taxed in their own return"); SMSF/company show the entity discount + a fund-/company-rate basis note, never a fabricated number; FW-2 post-reform disposals surface `UC-CGT-POST-REFORM`. Additive (no context → prior CGT flag verbatim). This completes Stage D's build items bar the registered-tax-agent sign-off scheduled at Basiq prep.
...
- **E2. Universe tax-flow overlay** — ✅ **SHIPPED 2026-06-13.** The Money Flow lens now carries the Stage-D tax treatments as a per-entity overlay: `wealthGraphService` computes a per-entity `taxStatus` (`{ computed, caveatCount }`) via the same `assembleEntityTaxFacts` + `calculateEntityTaxPositionDecimal` engine the reports + per-entity tax page use (so the canvas and those surfaces never disagree), at the current FY, in parallel across entities (§12.10). The status flows `WealthGraphEntity.taxStatus` → `WealthNode.taxStatus` (layout) → an honest pip on each entity tile when the Money Flow lens is active: **emerald ✓** = computed with no caveats, **amber N** = N items pending/assumed (reusing the canvas's existing reform-amber). A toolbar **legend pill** ("N computed · N to review") decodes the pips. Status ONLY — no tax dollar crosses the boundary (the engine's `result` stays server-side; §0.3 honesty). Per-flow regime (amber reform dots + "pending Royal Assent" pill) already shipped in Phase 2 — E2 adds the per-*entity* layer beside it. This completes the directive: structure + value + money flow + **tax flow**, one canvas. Built code-first; Stitch backfill per §18.2.1 — screen `8e2871ee0fc64e78a5361d86155c66eb`, artefact `.stitch/designs/phase47-e2/wealth-universe-tax-flow-overlay.{html,png}`. 2 layout pass-through tests. Additive — no schema/endpoint change (the `entity.read`-gated `/api/wealth-graph` now also carries the status; no tax figures, so no `tax_data.read` escalation needed).
