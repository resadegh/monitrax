...
**56.8b — desktop row cleaner refresh (this PR):** the MOBILE row was already redesigned (Phase 56, Apple-Wallet 9.4/10); the DESKTOP row was the unchanged Phase-55 design. 56.8b modernises the desktop body to the My-Wealth glass vocabulary — a **luminous 40px gradient gem + unreviewed dot** (was a flat tinted square), a **two-line "Account · Date"**, 15px merchant/amount, more breathing room — while **keeping** search, the confidence-hue category pill, the inline Confirm/Categorise/Neobrain actions, the swipe gestures, the bulk checkbox and the day-grouped glass cards. Visual-only; no handler/gesture/logic touched; mobile body unchanged. §18.8 review done (≈9.0, scores in the changelog); live-verified on the Vercel preview (§18.2.1 backfill — Stitch preview generator was flaking).

**Gem-colour decision (Reza 2026-06-30, 4-lens):** the outgoing gem is **neutral SLATE**, not indigo (the original) and not red (Reza's first instinct). Income = emerald (the one positive highlight); transfer = a lighter slate (⇄ icon differentiates). Rationale (reinforces §18.7.2, doesn't change it): red is reserved for *true loss / needs-attention* — painting every routine debit red is a wall of alarm (behaviour-psychology: calm-not-anxious; financial-adviser: normalise spending, save red for real signals; designer: red-fatigue kills premium; architect: one convention across mobile+desktop). Direction is already carried by the ↑/↓ arrow + the "−" sign, so the gem stays calm. Applied via the shared `gemGradient` (mobile + desktop).

*Phase 56 v1.7 — 56.8 hub layout + one-list Review + 56.8b desktop row refresh. 56.8c — mobile list amount-fit (min-w-0 truncation, the amount no longer pushed off the card) + deck swipe = notebook page-flip (slide-out → swap → slide-in). Governed by §12.2.1 (one list/source), §18.2.1 (cleaner-refresh live-verified), §20.5, §0 (architect: retire the duplicate; designer: Mercury-tier desktop row; psychology: clean hub + familiar list).*
