...
5. 🟡 CFO decision tables + report reconciliation locks (§3) — **in progress.**
   ...
   - **Decision-table queue (the tracked path to full §3 coverage):**
     ...
     - Report reconciliation locks (Σ report lines === canonical total) — the master service exposes no `expenseLines`/`loanLines` today; needs the report generators (`lib/reports/generators/*`) inspected for a line/total structure to lock. *(Next step-5 slice.)*
     - MON-030 (health/CFO/Safety score divergence, `changesNumbers:true`) — the number-changing dedup lands in this step as its own carefully-verified PR (§19.4 downstream sweep + §19.5 registry lifecycle + Reza's option B: keep the familiar 6 CFO bars sourced from canonical `generateHealthReport`, keep Safety Net distinct).
