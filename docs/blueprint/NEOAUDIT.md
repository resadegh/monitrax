...
5. 🟡 CFO decision tables + report reconciliation locks (§3) — **in progress.**
   ...
   - **Decision-table queue (the tracked path to full §3 coverage):**
     ...
     - 🟡 MON-030 (health/CFO/Safety score divergence, `changesNumbers:true`) — Reza's option B: keep the familiar 6 CFO bars sourced from canonical `generateHealthReport`, keep Safety Net distinct. Staged:
       ...
       - ⬜ **Stage 2b (remaining unwind):** re-ground `generateActions`/advisor on canonical categories, then DELETE `calculateCFOScore` + its calc-audit shadow engine (`decimal-cfo-score-risk.ts`) + the `scoreCalculator` Neomatrix nodes (`engine.scoreCalculator.calculateCFOScore`, `law.monitrax.cfoScoreWeights`, its input edges). Discovered (future MON, not this scope): `number.cfoScore` and `number.healthScore` are now the SAME number with two semanticKeys — a candidate to merge onto one key so A3 convergence + the parity matrix treat them as one.
