...
5. 🟡 CFO decision tables + report reconciliation locks (§3) — **in progress.**
   ...
   - **Decision-table queue (the tracked path to full §3 coverage):**
     - "negative cashflow ⇒ no positive-cashflow credit" (§3 example) — NOT yet located in source; research whether the guard exists (health/CFO/action engine) → fixture it, OR raise a MON if it's a genuine gap. *(Discovered this PR — do NOT fabricate the test; verify first §19.2.)*
     ...
     - 🟡 MON-030 (health/CFO/Safety score divergence, `changesNumbers:true`) — Reza's option B: keep the familiar 6 CFO bars sourced from canonical `generateHealthReport`, keep Safety Net distinct. Staged:
       ...
       - ⬜ **Stage 2 (the number-changing core):** re-source the CFO overall + grade + 6 bars from canonical `generateHealthReport`, remove `scoreCalculator`'s competing `overall`/`grade` formulas, unwind its calc-audit shadow engine (`decimal-cfo-score-risk.ts`) + Neomatrix nodes (`engine.scoreCalculator.calculateCFOScore`, `number.cfoScore`, `law.monitrax.cfoScoreWeights`, edges), verify the AI advisor + chat route + intelligenceEngine still ground correctly, add a Home==CFO cross-surface test. Full §19.4 downstream sweep + §19.5 registry lifecycle. **Changes a visible score → stays FIXING for Reza's live verification.**
