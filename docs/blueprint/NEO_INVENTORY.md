...
- **✅ NI-5b.1 SHIPPED** (this PR — fix) — the first cut rendered all 8,589 nodes with a synchronous `warmupTicks` pre-warm that **froze the tab on load** (Reza-reported). Replaced with **directory-cluster LOD**: the structural view now defaults to ~97 directory super-nodes (instant), **drills into a directory on click** (top-800 symbols by degree), and **searches across all 8,589 symbols**. This is the LOD/clustering that was flagged as a possible v2 — now the default, because the raw hairball both froze and wasn't navigable. Also: the 3 known-unwired engine islands (`div152`/`psi`/`fteIee`) are now **detected client-side and badged amber + "unwired (planned)"** so they read as intentional (honest, never faked). **Neo Inventory is feature-complete (NI-0→NI-5b).**
