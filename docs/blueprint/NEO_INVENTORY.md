...
- **✅ NI-5b SHIPPED** (this PR) — the **third "Structural (8,589)" view**: a new admin API `GET /api/admin/neomatrix/structural` serves the Layer-0 Graphify graph (compact tuples → `{nodes,edges}`); the explorer **lazy-loads** it on first switch (~2.7 MB, never on default page load), colours nodes by **top-level directory** (deterministic hue, no palette to maintain), and tunes the canvas for scale (directional particles OFF, thinner links, capped `cooldownTicks`, looser charge). Search + click-to-inspect (file:line + capped relations) make 8.6k nodes navigable. **v1 = the raw full graph** (scale + search + inspect); directory-clustering drill-down (LOD) remains a possible v2 if Reza wants cleaner navigation. **Neo Inventory is now feature-complete (NI-0→NI-5b).**
