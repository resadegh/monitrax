...
- **✅ NI-5a SHIPPED** (PR #1273) — semantic view: "Proven" toggle now shows the proven engines + their 1-hop lineage (60→115 nodes / 13→159 edges), and the "All" toggle relabelled "Semantic (231)" so it doesn't imply the structural census.
- **✅ NI-5b SHIPPED** (PR #1276) — the **third "Structural (8,589)" view**: a new admin API `GET /api/admin/neomatrix/structural` serves the Layer-0 Graphify graph (compact tuples → `{nodes,edges}`); the explorer **lazy-loads** it on first switch (~2.7 MB, never on default page load) and colours nodes by **top-level directory**.
- **✅ NI-5b.1 SHIPPED** (PR #1277) — directory-cluster LOD + amber "unwired (planned)" island badges (the 3 known-unwired engines `div152`/`psi`/`fteIee`, detected client-side).
- **✅ NI-5b.2 SHIPPED** (this PR — the real load fix) — NI-5b.1 still didn't load: the symptom was "Loading… 0/0 nodes", i.e. the **fetch never resolved** — the cluster render was downstream of a 2.68 MB `/structural` response that hung the browser. Fixed by **aggregating server-side**: `/api/admin/neomatrix/structural` now returns ~97 **directory clusters (~15 KB)** by default, and `?dir=` / `?q=` return ≤800 symbols (≤160 KB) on drill/search — the browser never downloads the full graph. Client is param-driven (debounced search, `AbortController`, **20 s timeout → visible error + Retry** so it can never silently hang again). **Neo Inventory is feature-complete (NI-0→NI-5b).**
