...
> - **41e.1 (Div 115 + Div 6 basic + capital loss netting)** — slice A in flight (PR #644 — Div 115 CGT discount module with entity-aware rate dispatch + 24 tests). Slice B in flight (PR #645 — capital loss netting + s100-50 / s115-100 / Div 102-A ordering + 18 tests including the critical s115-100-ordering pin). Slices C (Div 6 + `trustDistribution.ts` skeleton), D (router wiring) queued.
