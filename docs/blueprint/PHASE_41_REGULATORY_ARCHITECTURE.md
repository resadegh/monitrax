...
> - **41e.12 (State land tax — NSW + VIC) — SHIPPED.** First per-state regime. `lib/tax-engine/landTax/stateLandTax.ts` with `LandTaxConfig` per-state pattern. NSW CY2025 (general $1.075M threshold + s5A special trust 1.5% on first $1.075M + Sch 1A 4% foreign on residential) + VIC FY24-25 (general $50k threshold + s46IB trust 0.5% + s46IC absentee 4% on all taxable land). Order: general → trust → foreign. UC-MULTI-STATE-LAND-TAX + UC-LAND-TAX-PPOR-EXEMPTION + UC-LAND-TAX-TRUST-SURCHARGE-NUANCE. NSW Land Tax Act 1956 + VIC Land Tax Act 2005.
> - **41e.13 → 41e.17** — queued; sequence below remains the contract.
