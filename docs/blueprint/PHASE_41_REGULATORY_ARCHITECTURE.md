...
| **41h.5** | Tool registry expansion + SCENARIO_RUN — 4 new tools (3 FACT_LOOKUP: `getCgtExposure` / `getDiv7aRisk` / `getInHouseAssetRatio` + 1 SCENARIO_RUN: `runContributionScenario`). Registry size 3 → 7. SCENARIO_RUN pattern established (returns baseline + scenario + delta numerical fields; D-2 boundary preserved — fact, not recommendation). 34 new tests. **CLOSES PHASE 41h.** | **SHIPPED 2026-05-07** |
