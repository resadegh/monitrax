...
| **41i.5** *(SHIPPED 2026-05-07)* | L2 anomaly detection — `lib/calc-audit/anomalyDetection.ts` scans `CalcAuditFinding` history for two temporal patterns (HIGH_FREQUENCY: > N findings in lookback window; REGRESSION_AFTER_STABLE_PERIOD: > 30 days quiet then sudden findings). Both patterns persist as `L2_ANOMALY` findings flowing through the 41i.3 lifecycle + 41i.4 alerting. Endpoint `POST /api/admin/calc-audit/anomaly-scan` accepts admin session OR Cloud Scheduler shared secret (`CALC_AUDIT_SCHEDULER_SHARED_SECRET`). Cloud Scheduler runbook at `docs/operational/calc-audit/cloud-scheduler-setup.md`. 17 new tests; 649 total. **CLOSES PHASE 41i.** |
