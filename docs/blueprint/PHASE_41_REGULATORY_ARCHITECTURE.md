...
11. **HR-3 — User-visible calc errors are unacceptable.** *(Reza brief 2026-05-06; extended 2026-05-07.)* The Calculation Audit System (Phase 41i) is a silent admin-side safety net — never a user-facing surface. **Extended scope (Phase 41i.6, Reza brief 2026-05-07):** HR-3 covers BOTH **calc-engine drift** (engine output ≠ canonical authority value) AND **surface-rendering drift** (rendered tile ≠ canonical engine output). Every numeric tile / page / surface a user sees must trace to a registered canonical source via a `SurfaceDescriptor`; the surface audit harness (Phase 41i.6) re-derives the value from the canonical source and diffs against the expected render — any drift surfaces an `L4_SURFACE_AUDIT` finding to the admin queue. Errors flow: detect → admin alert → admin investigates → if real bug, fix code + backfill via existing data-update flow → user contacted via existing support tooling. No "we think there might be a calc error" toasts. No anxiety-inducing UI. Reviewers reject any PR that adds a user-facing surface for calc-correctness warnings, AND any PR that introduces a financial surface without a corresponding `SurfaceDescriptor` registration once Phase 41i.6 ships.
...
> - **41e.6 (Div 7A loan classifier) — SHIPPED PR #655.** New `lib/tax-engine/divisions/div7aLoanClassifier.ts` — s109N MRP annuity formula + COMPLIANT / MRP_SHORTFALL / NO_AGREEMENT / SUB_TRUST_UPE classification per ITAA 1936 Div 7A. Wired into router for COMPANY entities; result.div7aClassification populated when div7aLoans facts provided. UC-DIV7A-DISTRIBUTABLE-SURPLUS surfaces when deemed dividend > 0 (s109Y cap deferred). UC-DIV7A-SUBTRUST-UPE surfaces for sub-trust UPE arrangements (TR 2010/3 + PCG 2017/13).
> - **41e.7 (Div 152 small business CGT concessions) — SHIPPED PR #655.** Stacks 4 concessions (15-year exemption, 50% active asset reduction, retirement exemption $500k cap, small business rollover) on top of Div 115. Basic conditions test (s152-10) — MNAV ≤ $6M OR turnover ≤ $2M, plus active asset. UC-DIV152-AGGREGATION when MNAV/turnover near threshold. UC-DIV152-RETIREMENT-CAP when lifetime cap binds. UC-DIV152-ROLLOVER for replacement-asset tracking (deferred).
> - **41e.8 (negative gearing + per-entity loss treatment) — SHIPPED PR #656.** Loss treatment dispatched by entity type: PERSONAL/SOLE_TRADER/PARTNERSHIP/SMSF offset losses against own/partner/fund income same year (negative gearing); DISCRETIONARY_TRUST/UNIT_TRUST/COMPANY trap losses at entity level (carry forward, subject to loss-tests in 41e.15). UC-TAX-LOSS-CARRY-FORWARD when individual loss exceeds other income; UC-TRUST-LOSS-TESTS / UC-COMPANY-LOSS-TESTS for the trapped cases.
> - **41e.9 (Personal Services Income classifier) — SHIPPED PR #657.** Per Part 2-42 ITAA 1997 + TR 2022/3. Five tests dispatched: 80%-one-client, results test (s87-18), unrelated clients (s87-20), employment test (s87-25), business premises test (s87-30). PSB Determination (s87-60) short-circuits. When NOT a PSB → full PSI attributed to individual at marginal rates per s86-15. UC-PSI-DEDUCTION-RESTRICTIONS surfaces for the s86-60 deduction-limitation calc (deferred).
> - **41e.10 (FTE + IEE + 47% TFN withholding) — SHIPPED PR #658.** Classifies trust distributions per Sch 2F: INSIDE_FAMILY (normal) / INSIDE_VIA_IEE (s272-85) / OUTSIDE_FAMILY_FTDT (47% trustee tax) / TFN_WITHHOLDING (47% on non-quoting beneficiary). Family group test (s272-95) uses caller-asserted relationship. UC-FTDT-OUTSIDE-FAMILY when FTDT applies; UC-FTE-CONTROL-TEST when CONTROLLED_ENTITY claimed.
> - **41e.11 (SMSF triumvirate) — SHIPPED PR #659.** Sole purpose test (s62 SIS) + in-house asset 5% cap (Pt 8 SIS) + LRBA compliance (s67A + PCG 2016/5) + BRP exception (s71(1)(b)) + NALI at 45% (s295-550). Per-loan LRBA classification (BREACH_MULTI_ASSET / BREACH_BARE_TRUST / BREACH_RECOURSE / NALI_RISK / COMPLIANT). Non-complying SMSF → 45% rate (s295-160). 4 UNCOMPUTED flags surface for breach states. **All 5 SMSF rules in one classifier.**
> - **41e.12 (State land tax — NSW + VIC) — SHIPPED PR #660.** First per-state regime. `lib/tax-engine/landTax/stateLandTax.ts` with `LandTaxConfig` per-state pattern. NSW CY2025 (general $1.075M threshold + s5A special trust 1.5% on first $1.075M + Sch 1A 4% foreign on residential) + VIC FY24-25 (general $50k threshold + s46IB trust 0.5% + s46IC absentee 4% on all taxable land). Order: general → trust → foreign. UC-MULTI-STATE-LAND-TAX + UC-LAND-TAX-PPOR-EXEMPTION + UC-LAND-TAX-TRUST-SURCHARGE-NUANCE. NSW Land Tax Act 1956 + VIC Land Tax Act 2005.
> - **41e.13 (State land tax — rest-of-states + cross-state aggregator) — SHIPPED.** Adds 6 state configs: QLD_LAND_TAX_CY2025 ($600k threshold + 2% absentee surcharge on all land per Sch 3) + SA_LAND_TAX_FY2024_25 ($755k threshold + 0.5% trust + no foreign land tax surcharge) + WA_LAND_TAX_FY2024_25 ($300k threshold + no trust + no foreign surcharge) + TAS_LAND_TAX_FY2024_25 ($100k threshold + 2% foreign on residential) + ACT_LAND_TAX_FY2024_25 (Rates Act 2004 approximation + 0.75% foreign + UC-ACT-RATES-VS-LAND-TAX flag) + NT_LAND_TAX_FY2024_25 (structural zero + UC-NT-NO-LAND-TAX flag). New `lib/tax-engine/landTax/crossStateAggregator.ts` — `calculateCrossStateLandTax()` groups by state, aggregates within-state, sums across-state, returns per-state assessments (alphabetical order) + grand totals + de-duped citations/UC flags. UC-LAND-TAX-JOINT-OWNERSHIP for joint ownership / inter-trust grouping. `LandTaxConfig.foreignSurchargeResidentialOnly?: boolean` field added; foreign-surcharge dispatch now data-driven. **All 8 states/territories now supported.**
> - **41e.14 (Stamp duty + foreign purchaser surcharge) — SHIPPED.** New module `lib/tax-engine/stampDuty/stateStampDuty.ts`. 8 state configs (NSW/VIC/QLD/SA/WA/TAS/ACT/NT) with progressive duty brackets + FPAD/AFAD/FBI surcharges (NSW 8% / VIC 8% / QLD 8% / SA 7% / WA 7% / TAS 8% / ACT 0.75% / NT no regime). 4 UNCOMPUTED flags: UC-STAMP-DUTY-NT-NO-FPAD, UC-STAMP-DUTY-CONCESSION, UC-STAMP-DUTY-MULTI-PURCHASER, UC-STAMP-DUTY-NEW-BUILD.
> - **41e.15 (Trust + company loss rules) — SHIPPED.** Two new modules: `lib/tax-engine/divisions/trustLossRules.ts` (Sch 2F ITAA 1936 — IIT/50% Stake/Pattern of Distributions/Control/SBT across 4 trust types incl. FTE-protected family trusts and excepted trusts) + `lib/tax-engine/divisions/companyLossRules.ts` (Div 165 ITAA 1997 — COT primary + BCT fallback with both SBT s165-210 and Similar Business Test s165-211). v1 takes caller-asserted PASS/FAIL/NOT_ASSERTED; UC flags surface inconclusive paths.
> - **41e.16 (GST / BAS) — SHIPPED.** New module `lib/tax-engine/gst/gstCalculator.ts`. GST_RATE constant (10% per s9-70). 4 supply classifications (TAXABLE / GST_FREE s38 / INPUT_TAXED s40 / OUT_OF_SCOPE) + reverse charge (Div 84). Full BAS label aggregation (G1/G2/G3/G10/G11/1A/1B). 3 UC flags: UC-GST-INPUT-TAXED-DENIAL, UC-GST-REGISTRATION-REQUIRED, UC-GST-ADVANCED-DIVISIONS.
> - **41e.17 (MasterTaxPosition orchestrator) — SHIPPED. Closes Phase 41e.** New module `lib/tax-engine/orchestrator/masterTaxPosition.ts`. Pipeline: (1) per-entity dispatch via existing `calculateEntityTaxPosition`; (2) cross-cutting modules (cross-state land tax, per-transaction stamp duty, BAS GST); (3) per-entity loss-rule overlays (trust loss + company loss keyed by `entityId`); (4) de-duped citation + UNCOMPUTED aggregation; (5) AFSL boundary footer envelope via `renderBoundaryFootnote`. New `MasterTaxPositionV2` type extends existing `MasterTaxPosition` with `crossCutting?: CrossCuttingTaxResult` + `boundary: BoundaryFootnote` + `modulesInvoked: string[]`. Pure / deterministic. **Phase 41e is COMPLETE.**
...
| **41h.3** | Practice surface UI — `components/ai-advisor/` (`TaxAdvisorAnswer` / `TaxAdvisorBoundaryFooter` / `TaxAdvisorUncomputedFlag` / `TaxAdvisorAskForm`) + admin demo route at `/admin/ai-advisor` + `POST /api/admin/ai-advisor/ask` endpoint wired to `GeminiProvider` + `ProductionAuditSink`. Status routing (5 outcomes) renders error surfaces, recommendation routing, and Tier 1 segments with citations + boundary footer + UNCOMPUTED. 12 component tests via `react-dom/server.renderToString`. | **SHIPPED 2026-05-07** |
| **41h.4** | Ask-a-Pro router + user-facing surface graduation — `lib/ai/tax-advisor/askAProRouting.ts` (profession → marketplace `discipline` mapping: ADVISER→FINANCIAL_ADVISOR / ACCOUNTANT→TAX_AGENT / BROKER→MORTGAGE_BROKER) + `runAdvisorQuery` shared helper + `POST /api/ai-advisor/ask` user-facing endpoint (`report.read`) + `/dashboard/cfo/ask` consumer page + clickable Tier 2 CTA on `RouteToPro` card linking to `/marketplace?discipline=...&question=...&reason=...`. 17 new tests. | **SHIPPED 2026-05-07** |
| **41h.5** | Tool registry expansion + SCENARIO_RUN — 4 new tools (3 FACT_LOOKUP: `getCgtExposure` / `getDiv7aRisk` / `getInHouseAssetRatio` + 1 SCENARIO_RUN: `runContributionScenario`). Registry size 3 → 7. SCENARIO_RUN pattern established (returns baseline + scenario + delta numerical fields; D-2 boundary preserved — fact, not recommendation). 34 new tests. **CLOSES PHASE 41h CORE.** | **SHIPPED 2026-05-07** |
| **41h.6** | SCENARIO_RUN tools expansion — 3 new SCENARIO_RUN tools wrapping the highest-value tax engines: `runCgtScenario` (wraps `applyCapitalLossNetting`; "what if I sold property X?" → delta on Div 102-A net CG); `runLandTaxScenario` (wraps `calculateCrossStateLandTax`; "what if I bought a property in QLD?" → delta on cross-state aggregator + foreign surcharge); `runDiv7aRefinanceScenario` (wraps `classifyDiv7ALoans`; "what if I refinanced this loan to safe-harbour terms?" → delta on deemed dividend exposure). Each preserves the locked-in SCENARIO_RUN pattern from 41h.5: every numeric field path-addressed under `scenario.baseline.*` / `scenario.input.*` / `scenario.result.*` / `scenario.delta.*`; descriptions explicitly disclaim recommendation; HR-1/HR-2/D-2 boundaries preserved at every layer. Registry size 7 → 10 (FACT_LOOKUP × 6 + SCENARIO_RUN × 4). 36 new tests covering per-tool baseline + scenario + delta presence + zero-hypothetical idempotency invariant + foreign-surcharge delta + selective loan refinancing + per-tool HR-1/HR-2/D-2 contract. | **SHIPPED 2026-05-07** |
| **41h.7** | TRAIL-aligned IA — graduates the AI advisor from orphan deep-link to natural-IA placement under "My Guide" (TRAIL Stage 5 — Live, per CLAUDE.md §14). Two changes: (1) new "Ask the Advisor" child link under My Guide in the consumer sidebar (`components/DashboardLayout.tsx` `reachNavItems`), sibling of Actions / Health / Tax; (2) restrained Apple-glass CTA card on `/dashboard/cfo` (Actions tab) directly below `<AIAdviceSection />` linking to `/dashboard/cfo/ask`. Page-header on `/ask` rewarmed; URL stable; primitives untouched (provider-agnostic from 41h.3). Architect lens: routing + IA + visual hooks only — zero changes to `components/ai-advisor/` or `runAdvisorQuery`. Designer lens: chose sidebar-child + CTA over fold-inline-tab to keep the conversation surface focused (CFO Actions page already has 5+ sections; restraint over density). | **SHIPPED 2026-05-07** |

### 11.1.1 Phase 41f — Personal Bookkeeping Integration (added 2026-05-07)

**Spec doc:** `docs/blueprint/PHASE_41F_BOOKKEEPING_INTEGRATION.md` — full sub-PR sequence + schema migration plan + 5 strategic decisions (D-41F-1 → D-41F-5) gated on Reza sign-off.

**Strategic positioning (Reza brief):** *"Monitrax does NOT replace Xero — Monitrax CONSUMES Xero data and re-presents it through the wealth-strategy lens."*

**Why it matters for Phase 41:** closes the loop between the entity-aware tax engine (Phase 41e) and real-world entity bookkeeping. The `distributableSurplus` (s109Y simplified) feeds Phase 41e.6 Div 7A; the `trustNetIncome` feeds 41e.4 Div 6E streaming; trust-deed beneficiary + distribution rules feed 41e.4 + 41e.5 (s100A zone classifier).

| Sub-PR | Scope | Status |
|---|---|---|
| **41f.0** | Design doc + Reza sign-off block — `PHASE_41F_BOOKKEEPING_INTEGRATION.md`. 5 strategic decisions D-41F-1 (schema scope: extend vs new model) → D-41F-5 (distributable-surplus auto-feed). UNCOMPUTED v1 register (8 items). CDR posture (Xero data is non-CDR business data; standard Privacy Act 1988). | **DRAFT — sign-off pending (this PR)** |
| **41f.1** | Schema migration: extend `AccountingIntegration` with scope discriminator (`ORG | USER_ENTITY`) + nullable `userId` + `legalEntityId`; XOR check constraint; partial unique indexes per scope. New `EntityAccountingSnapshot` model (BS + P&L + tax-engine inputs `distributableSurplus` / `trustNetIncome` / `smsfMemberBalances` keyed by `(legalEntityId, fiscalPeriod)`). New `TrustDeedExtractedRules` model with `EXTRACTED → CONFIRMED → REJECTED` lifecycle. **§12.11 destructive-write checklist filled in advance** (`ALTER COLUMN organization_id DROP NOT NULL` is the only existing-constraint touch — pre-flight verified zero rows). | Queued — gated on 41f.0 sign-off |
| **41f.2** | Xero OAuth — new `lib/integrations/xero/` (oauth, token-refresh, tenant resolver). New `/dashboard/entities/[id]/connect-bookkeeping` page. New `/api/entities/[id]/accounting/oauth/[start\|callback]` routes. Persists OAuth tokens to `AccountingIntegration` (USER_ENTITY scope). | Queued — gated on 41f.1 |
| **41f.3** | Snapshot import — `lib/integrations/xero/snapshotPuller.ts` pulls BS + P&L + computes s109Y simplified `distributableSurplus`. Idempotent on `(legalEntityId, fiscalPeriod)`. Wires into Phase 41e.6 Div 7A classifier as canonical input (replaces user-entered estimate; per-loan override per D-41F-5). | Queued — gated on 41f.2 |
| **41f.4** | Trust-deed parser — `lib/integrations/trust-deed/` (Vision OCR + Gemini structurer). 4-step confirm-before-apply UI (upload → extract → review → apply per D-41F-3). `TrustDeedExtractedRules` writes; Phase 41e.4 (Div 6E streaming) + 41e.5 (s100A zone classifier) read CONFIRMED rules only. Can ship in parallel with 41f.2/3 (different file pathway). | Queued — gated on 41f.1 (schema), independent of 41f.2/3 thereafter |

**HR-1 preservation in 41f**: every imported number stamps `sourceProvider + sourceTenantId + pulledAt` on `EntityAccountingSnapshot`. Downstream tax engines consume from this typed snapshot row, never from raw Xero JSON. `rawProviderPayload` JSON kept for forensic re-derivation if our shape changes.

**HR-2 preservation in 41f**: trust-deed Gemini extractor returns confidence per rule; rules below 0.7 confidence default to UNCOMPUTED; user reviews each rule individually; CONFIRMED status is the only state any tax engine reads.

**D-2 preservation in 41f**: trust-deed parsing surfaces UNCOMPUTED notes verbatim ("Clause 5.2 references 'majority quorum' — review with adviser"); Phase 32C Ask-a-Pro deep-link surfaces directly from the review step. Never auto-applies legal interpretation.
...
| **41i.2** *(SHIPPED 2026-05-07)* | Engine adapter expansion — 22 new adapters across `tax-divisions.ts` (8: cgtNetting / div7a / psi / fteIee / div152 / smsfTriumvirate / highIncomeSuperTax / masterTaxPosition) + `tax-state.ts` (14: 7 land tax + 7 stamp duty for VIC/QLD/SA/WA/TAS/ACT/NT). Registry 14 → 36 engines / 45 fixtures all green. Health engine deferred (complex `FinancialHealthInput` shape). |
| **41i.3** *(SHIPPED 2026-05-07)* | L3 audit foundation — persistent `CalcAuditFinding` Prisma model + 3 enums (Source / Severity / Resolution) + `findingService.ts` (record/list/transition with locked-in lifecycle: OPEN → INVESTIGATING → FALSE_POSITIVE \| FIX_REQUIRED → FIXED) + auto-persist hook on `GET /api/admin/calc-audit` + new findings list/PATCH endpoints + admin UI section with lifecycle controls. Per-user "Audit this user" deferred to **41i.3b** (requires per-engine user-data adapters — substantial workstream; foundation is now in place). |
| **41i.4** *(SHIPPED 2026-05-07)* | Alerting + workflow — `lib/calc-audit/alertingService.ts` with Slack webhook (`SLACK_CALC_AUDIT_WEBHOOK_URL`) + email (`SENDGRID_API_KEY` + `MONITRAX_CALC_AUDIT_ALERT_EMAIL`) channels; severity threshold default HIGH (override via `CALC_AUDIT_ALERT_THRESHOLD`); fire-and-forget hooks on finding-CREATED + escalated-to-FIX_REQUIRED; backfill-affected-users runbook at `docs/operational/calc-audit/backfill-runbook.md` (3 remediation strategies + HR-3 notification policy + severity SLA matrix). 26 new tests; 632 total. |
| **41i.5** *(SHIPPED 2026-05-07)* | L2 anomaly detection — `lib/calc-audit/anomalyDetection.ts` scans `CalcAuditFinding` history for two temporal patterns (HIGH_FREQUENCY: > N findings in lookback window; REGRESSION_AFTER_STABLE_PERIOD: > 30 days quiet then sudden findings). Both patterns persist as `L2_ANOMALY` findings flowing through the 41i.3 lifecycle + 41i.4 alerting. Endpoint `POST /api/admin/calc-audit/anomaly-scan` accepts admin session OR Cloud Scheduler shared secret (`CALC_AUDIT_SCHEDULER_SHARED_SECRET`). Cloud Scheduler runbook at `docs/operational/calc-audit/cloud-scheduler-setup.md`. 17 new tests; 649 total. **CLOSES PHASE 41i CORE.** |
| **41i.6** *(QUEUED — design doc PR opening 2026-05-07)* | **Surface-level numerical audit** — extends HR-3 from "calc-engine drift" to "surface-rendering drift." Spec doc `PHASE_41I_6_SURFACE_AUDIT.md`. New `L4_SURFACE_AUDIT` enum value on `CalcAuditFindingSource`. New `lib/calc-audit/surfaces/` registry with one `SurfaceDescriptor` per user-facing financial tile (top 10 at v1: cashflow / expense / income / net-worth / health / tax / debt-freedom / emergency-fund / property-equity / investment-total). Three sub-PRs: 41i.6a (registry + first 10 descriptors + enum migration); 41i.6b (CI static-analysis pass — `npm run lint:financial-surfaces` rejects inline financial math at PR review time); 41i.6c (runtime audit harness + `[Full scan]` button on `/admin/calc-audit` with NDJSON-streamed progress + new `[L4_SURFACE_AUDIT]` filter chip). Reza brief 2026-05-07: *"continuously make sure all calculations and produced numbers in monitrax is accurate and trustworthy."* The trustworthiness commitment. ~6 days total. |
