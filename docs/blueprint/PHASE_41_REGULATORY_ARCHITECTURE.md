...
> - **41e.1 (Div 115 + Div 6 basic + capital loss netting)** — slice A (PR #644 — Div 115 CGT discount module + 24 tests), slice B (PR #645 — capital loss netting + s100-50 / s115-100 / Div 102-A + 18 tests), slice C (PR #647 — Div 6 basic via `trustDistribution.ts` + s99A 47% penalty + 20 tests), **slice D-1 (PR #649 — wire trustDistribution into entityTaxRouter; TRUST entities flip from UNCOMPUTED to computed when distribution data provided; new POST endpoint to test via curl; 6 new router tests).** Slice D-2 (CGT events into router) queued.
