...
| **50** | AI Document Router | 🔄 In Progress | Builds on 25/26: recognise → attach-or-create → file. Phase A ✅ (scan recognition fix + `ASSET` linkable type); Phase B ✅ (per-user storage quota + keyless GCS auth); Phase C ✅ (per-item Documents sections, AI-on-upload, auto-rename, thumbnails); **Phase D — engine intelligence ✅ COMPLETE (2026-06-19): D.1 dedup, D.1.1 scan-path dedup, D.2 reconcile, D.3 confidence SSOT, D.4 learned routing (suggest-only), D.5b retention clock, D.5a renewal→reminder, D.6 bulk-approve Smart Inbox — all suggest→confirm per the 2026-06-18 autonomy decision.** Remaining: Tax-pack export surfacing/unification (the core tax-pack export already exists from Phase 42). See `PHASE_50_AI_DOCUMENT_ROUTER.md`. |
| **51** | Loan Ledger, Repayment Matching & Tax Split | ✅ Complete (51.1, 2026-06-21) | Loan's own transaction ledger (quarantined from categorisation) + QIF/CSV import + actual-vs-actual offset↔loan repayment matching + confirm + interest/principal tax split, behind the loan **Repayments** tab. CDR-forward (QIF now → Basiq later, same store). See `PHASE_51_LOAN_LEDGER_AND_CATEGORISATION.md`. 51.2 (categorisation UI) = Phase 52 §52.5c. |
| **52** | Shared Categorisation Knowledge Base (RAG) | ✅ Engine live (2026-06-22), gated-on + seeded | De-identified, cross-user merchant→category KB the AI consults (lookup-first → Gemini-on-miss); k-anon graduation (k=5); PII-scrubber gate; write-back on confirmation; housekeeping; seeded ~275 AU merchants; fuzzy brand-prefix matching (52.5a). `KB_WRITE_ENABLED`/`KB_READ_ENABLED` ON. Remaining: 52.5b embeddings (pgvector), 52.5c review-exceptions UI. See `PHASE_52_SHARED_CATEGORISATION_KB.md` + runbook `operational/runbooks/14_…`. |
| **54** | Neobrain — AI Perception & Learning Layer | 🟢 Consolidation SSOT (2026-06-25) · modelled in Neomatrix | The umbrella for every AI engine that turns raw inputs (transactions, documents, receipts) into categorised, linked, trustworthy facts — and learns from each confirmation. Three pillars: (A) lookup-first categorisation cascade (MerchantMapping → AU rules → graduated k-anon shared KB → Gemini-on-miss RAG → Uncategorised) + per-user & cross-user learning; (B) transfer/loan-repayment detection + exclusion; (C) document/receipt intelligence (Vision OCR + Gemini → route/create). **Pure consolidation + modelling — no behaviour change.** Consolidates the design of Phases 13/18/25/26/29/42/50/51/52 into one SSOT (`PHASE_54_NEOBRAIN.md`) and adds a `neobrain` domain to the Neomatrix (39 nodes / 47 edges, all verified). Neobrain perceives; the Neomatrix calculates. See `PHASE_54_NEOBRAIN.md`. |
| **53** | Neomatrix — Financial Logic Knowledge Graph | 🟢 Modelled + 100% audited (all 6 domains) · **N2 3D explorer SHIPPED (admin-only)** · graph connected | Typed directed knowledge graph of every calc engine/number/formula/law (`docs/financial-logic/graph/financial-graph.json`, 102 nodes / 130 edges) + a generator that renders the markdown view + a CI audit. **Live:** N0 (Graphify code-only, adopted) · N1 (schema + generator) · N3 (the `vercel-build` + vitest gate — CLAUDE.md Part 21) · N4 full backfill (all 6 domains: core/tax/health/cfo/intelligence/reports, all `verified` file:line) · **A1 executable law-referenced audit — 27 engines, ZERO suspected-issues** (model refs the code; law-derived expected, real engines run, mismatch → `suspected-issue` raised not fixed; locks the income-tax bracket-boundary P0, SG-cap P1, loan-interest 100× classes) · **N2 — navigable 3D explorer at `/admin/neomatrix`** (admin-only developer tool, `react-force-graph-3d`; orbit/zoom/click→inspector; metadata-only API) · **cross-domain connectivity** (18 verified edges; 6 islands → 1 connected graph of 96/102 nodes, all 6 domains, core↔tax included). **Strategic decision (Reza 2026-06-24):** admin-only, not a user feature. **Pending (optional):** model the portfolio-snapshot SSOT to wire `linkageHealth`; bloom/post-fx; remaining niche-engine depth. See `PHASE_53_MONITRAX_NEOMATRIX.md` §0, §15 (explorer), §16 (connectivity). |
...
|-------|------|--------|-------|
| **12 (wizard)** | Onboarding Wizard Remediation | ✅ **§6A.1 COMPLETE 2026-05-19** (PR 3c.2e closed the last item — confidence indicators on derived metrics; full data-source hygiene story 6/6 ✅) | **PR 1 merged** (correctness sweep). **PR 2 merged** (draft persistence + resume banner). **PR 3a merged** (full wizard visual overhaul). **PR 3b merged** (3-tier Accounts data source picker). **PR 3c — 6/6 §6A.1 items shipped**: PR 3c.1 chip + nudge (#791); PR 3c.2a upgrade button (#793); PR 3c.2c write-site audit + `balanceWriteFields()` SSOT helper (#794); PR 3c.2d Settings > Data Health heat-map (#795); PR 3c.2b first-visit migration modal + new `UserPreference.dismissedBalanceUpgradeNudge` column + `/api/settings/balance-upgrade-nudge` endpoint (#796); PR 3c.2e confidence indicators — extended `getMasterFinancialSnapshot()` with `staleness` block + new `<ConfidenceIndicator>` primitive wired into `/dashboard/balances` Net Position hero (PR 3c.2e 2026-05-19). **Canonical BAU support runbook:** `docs/operational/runbooks/10_DATA_SOURCE_HYGIENE.md`. See `docs/blueprint/PHASE_12_ONBOARDING_TOUR.md` v2.4 and `docs/blueprint/PHASE_12_WIZARD_REDESIGN_PLAN.md` §6A. |
