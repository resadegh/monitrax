...
| **43.1** | Hidden Wealth Lens (Stark Naked follow-on #1) | 🟡 SHIPPING (May 2026) | Andrew's *"balance sheet is where all the cash is hiding"* applied to `/dashboard/balances`. Splits Total Assets by accessibility: **Liquid Today** (cash + offsets) → **Accessible** (investments, days) → **Locked Long-Term** (property equity + super + personal assets). Typography-led card; emerald → sky → slate palette, no red anywhere (loss-aversion-safe). New thin endpoint `/api/dashboard/hidden-wealth`; no `quickMetrics` changes. See `docs/blueprint/PHASE_43_1_HIDDEN_WEALTH.md`. Three follow-ons still queued: Spending Pareto, Margin Trend, ≥90-day `enoughHistory` gate. |
