...
| **50** | AI Document Router | 🔄 In Progress | Builds on 25/26: recognise → attach-or-create → file. Phase A ✅ (scan recognition fix + `ASSET` linkable type); Phase B ~ (per-user storage quota ✅; GCS cut-over blocked on operator provisioning; attach-to-existing `ATTACH_TO_*` + entity picker queued); Phase C queued (per-item Documents sections + Tax-pack). See `PHASE_50_AI_DOCUMENT_ROUTER.md`. |
