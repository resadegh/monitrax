...
| **37** | My Budget IA Simplification + Premium Redesign | ✅ Complete (2026-05-01) | PR #574. Sidebar collapsed 6 sub-tabs → 3 (Cashflow / My Plan / Debt Freedom). Default landing flipped from `/dashboard/budget-analysis` → `/cashflow`. Tax relocated to My Guide. New `/dashboard/plan` hub with Apple-style segmented control + AnimatePresence cross-fades. Cashflow + Debt Freedom heroes uplifted to Apple typography (font-light, tracking-[-0.04em], colour isolated to digits, muted supporting copy). Zero new calc engines, zero new APIs, zero data duplication. Standalone routes preserved as deep-link destinations. PR 7 telemetry + soft-retire deferred. See `docs/IMPLEMENTATION_PLAN.md`. |
| **38** | My Vault — Document Management Uplift | ✅ Complete (2026-05-01) | PRs #575 / #576 / #577 / #578. Documents elevated to top-level "My Vault" sidebar item (`Archive` icon, between My Guide and Reports). Apple-typography hero with FY counter. Smart Inbox surfacing AI-analysed-but-unverified docs with one-tap Confirm + Open per row. Generic Share Pass engine (`SharePackage` schema) with polymorphic `contentRefs` + `contentType` + `deliveryMethod` enums — extends to future content types (Reports, property summaries, net-worth snapshots) and future delivery methods (Xero / MYOB push) without migrations. `SendToAccountantDialog` modal. Public minimal-aesthetic `/share/[token]` page (no app shell, audit-logged, watermarked, time-limited 30-day default, soft-revocable). Manage Shares at `/dashboard/settings/shares`. Universal upload migration (hook-level translation closes legacy bypass without touching the 2,031-LOC ExpenseDialog). Tax-status lens on `FolderTree` (Deductible / Non-deductible / Untagged). Folder view + toolbar + FolderTree visual restyle (Apple typography, glass active-row, tabular-nums badges). 6 new API routes (3 owner-side auth-required, 3 PUBLIC token-bearer). Built on top of existing Phase 25 (DME) + Phase 26 (DIE) engines — zero new calc engines, zero data duplication. CDR §13.5 audit-logged on every state transition. Token = 256-bit entropy. See `docs/IMPLEMENTATION_PLAN.md`. |
