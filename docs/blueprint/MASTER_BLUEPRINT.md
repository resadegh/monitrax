...
| **53** | Neomatrix — Financial Logic Knowledge Graph | 🔄 In build (2026-06-23) — foundation + tax modelled & audited, self-enforcing | Typed directed knowledge graph of every calc engine/number/formula/law (`docs/financial-logic/graph/financial-graph.json`) + a generator that renders the markdown view + a CI audit. **Live:** N0 (Graphify code-only, adopted) · N1 (schema + 4-engine core slice + generator) · N3-seed (the `vercel-build` + vitest gate — CLAUDE.md Part 21) · N4 tax backfill (income/PAYG/CGT-reform/super/land-stamp-GST, all `verified` file:line) · **A1 executable law-referenced audit** (model refs the code — law-derived expected, real engines run, mismatch → `suspected-issue` raised not fixed; core + tax audited, no issue found). **Pending:** A1 SMSF/CGT + health/CFO/intelligence/reports; N2 2D explorer; N5 3D. See `PHASE_53_MONITRAX_NEOMATRIX.md` §0. |
