...
| **36** | My Accounts UX Simplification | 🔄 In Progress (Phase 2c remaining) | Phase 36 Phase 2a (PR #601) shipped — inline `LoanDetailDialog` on Balances. **Phase 2b/2d/2e SHIPPED in this PR** — `?action=` handler on Balances, `/dashboard/accounts` and `/dashboard/loans` bare list pages retired (sub-routes `/dashboard/loans/[id]` + `/[id]/strategy` preserved), `routeMap.ts` flipped (account + loan basePaths now `/dashboard/balances`), `?id=` cross-module-nav handler added on Balances to auto-open `<AccountDetailDialog>` / `<LoanDetailDialog>`, 7 source-side hrefs flipped. Phase 2c (Import Transactions UI migration) remains. See `docs/blueprint/PHASE_36_MY_ACCOUNTS_SIMPLIFICATION.md` §8. |
