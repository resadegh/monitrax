...
| **43.3** | Margin Trend Lens (Stark Naked follow-on #3) | 🟡 SHIPPING (May 2026) | Andrew's Principle 2 — *"the direction of your GP margin matters more than the absolute"* — applied to `/dashboard/budget-analysis`. 6-month savings-rate sparkline (pure SVG, no chart library) + delta-from-last-month + sliding-window trend direction (3-vs-3 ±2pp). Trend palette: emerald (up — Bandura victory tone) / amber (down — **never red**, loss-aversion-safe) / slate (flat). Direction-over-absolute closing copy. New thin endpoint `/api/dashboard/margin-trend` queries `prisma.unifiedTransaction` directly (UnifiedTransaction is the only honest source for actual historicals; the master snapshot is point-in-time). Zero new calc engines, zero `quickMetrics` changes, zero chart-library imports. See `docs/blueprint/PHASE_43_3_MARGIN_TREND.md`. One follow-on still queued: Phase 43.4 — ≥90-day `enoughHistory` gate for the Money Story Hero. |
