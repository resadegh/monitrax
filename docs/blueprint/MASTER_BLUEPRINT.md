...
| **43.4** | Tighter `enoughHistory` Gate (Stark Naked follow-on #4 — final) | 🟡 SHIPPING (May 2026) | Closes the four Stark-Naked-Numbers follow-ons. Replaces the cheap `monthlyExpenses > 0` gate on the Money Story Hero's day-count display with a two-mode honest check: (a) ≥90-day UnifiedTransaction history (one quarterly cycle, annual bills should have appeared), OR (b) ≥3 recurring `Expense` rows with ≥1 essential flag (user has done meaningful classification). Recognises that monitrax has both bank-imported and manual-entry users. New helper `lib/dashboard/expenseDataMaturity.ts`; one `enoughHistory` source-swap in `/api/dashboard/insights`. Zero client-side change. See `docs/blueprint/PHASE_43_4_ENOUGH_HISTORY_GATE.md`. **Stark-Naked-Numbers translation stream complete.** |
