...
### §6.4 — `MoneyFlowSankey` upgrade (Part 2d) ✅ Built

`getMoneyFlow()` today approximates. Part 2d adds a **`Distributions`** outflow column fed by recorded flows: `DistributionResolution` (trust → beneficiary) and `DividendDistribution` (company → shareholder). Only `CONFIRMED` rows count — `DRAFT` resolutions/dividends are working notes, not declared entitlements.

**As built.** The pure mapper `buildDistributionDetail()` (exported from `moneyFlowService.ts`, unit-tested) turns CONFIRMED resolutions + dividends into per-recipient detail. The trust column total is `trustNetIncome` (s95); per-beneficiary amount is `presentlyEntitledShare × trustNetIncome` (Bamford proportionate). The company column total is the declared `totalAmount`; per-shareholder amounts are the recorded `DividendPayment.amount`. A trust/company that declared a distribution keeps its money-flow row even if its income is not captured as `Income` rows (warn-not-reject, CLAUDE.md §6.1). Surplus is recomputed as `max(0, income − tax − essential − discretionary − loans − distributions)`. The Sankey gains an indigo `Distributions` node + a per-recipient detail list below the chart; the consumer-level `ConsumerMoneyFlowSankey` collapses to a single synthetic entity so it carries `Distributions: 0` / `distributions: []`.

**`SuperContribution` (member → SMSF) deferred.** A super contribution is an entity → entity flow that risks double-counting against the member's income/surplus (a salary-sacrifice contribution never appears as `Income`; an after-tax contribution is already inside discretionary/surplus). It needs its own carefully-scoped design pass — a Part 2 follow-up — rather than being forced into the `Distributions` column.

**Law-precision caveat (external review §13-F22).** These are **entitlement / declared** flows, not necessarily **cash** flows — a beneficiary can be presently entitled with no cash paid (an unpaid present entitlement / UPE); a dividend can be declared and paid in different years. The §11A money-flow lens upgrade therefore moves from "eligibility" to "**entitlements & declared distributions**" — it must **not** be labelled or read as "cash movements". The Sankey caveat states this explicitly when distributions are present. Where the model can distinguish (e.g. `DividendDistribution.declaredDate` vs `paymentDate`), the lens shows the entitlement and notes any unpaid portion rather than implying cash changed hands.
...
| **2b — Model services + routes** ✅ | The four service writers (§5) + thin API routes. Audited, §12.11-compliant. Tests. **Built** — `distributionResolutionService` / `dividendDistributionService` / `privateCompanyBenefitService` / `trustDeedRuleService` + 8 routes under `/api/tax/*`. | Standard PR review. |
| **2c — Assembler + repoint** | Split for reviewability into **2c-i** + **2c-ii**. | **Higher-risk** — careful review; the tax-route output changes from `UNCOMPUTED` to real numbers. |
| **2c-i — `smsfIncomeTax.ts`** ✅ | The new SMSF fund-earnings income-tax module (Div 295 — 15% concessional / ECPI / NALI top-rate; the §7.2 Q-SMSF module, approved by Reza) + wired into `entityTaxRouter`'s SMSF branch. **Built.** Isolated so the tax math gets focused review. | Q-SMSF approved → built. |
| **2c-ii — Assembler + repoint** ✅ | `entityTaxFactsAssembler` (the SSOT for building `EntityTaxFacts`) — feeds `trustDistribution` from `DistributionResolution` and `div7aLoans` from `PrivateCompanyBenefit` `LOAN` rows (Q-UPE — `UPE`/`SUB_TRUST_ARRANGEMENT` excluded). `GET /api/tax/entity` repointed to it → real numbers, not `UNCOMPUTED`. The dead `parentEntityId` read removed (route + `EntityTaxFacts` type). Q-PARTNERSHIP → partnerships not assembled (stay `UNCOMPUTED`). **Built.** **Note:** `trustMinimumTax` is a reform skeleton that does not yet read deed rules — the `TrustDeedRule` repoint there is a no-op until it leaves skeleton stage (post-Royal-Assent); the model + service are ready. `smsfIncomeTax` GET-feed is deferred — it needs pension-phase / ECPI facts not yet captured (a follow-up); the module is reachable via `POST` today. | Built. |
| **2d — Money-flow** ✅ | `MoneyFlowSankey` / `getMoneyFlow()` gain a `Distributions` outflow column fed by CONFIRMED `DistributionResolution` + `DividendDistribution`; the §11A money-flow lens shows recorded inter-entity distributions (declared entitlements, not cash — §6.4 / review §13-F22). **Built** — `buildDistributionDetail()` pure mapper + unit tests; `ConsumerMoneyFlowSankey` updated for the new field. `SuperContribution` member→SMSF limb deferred (§6.4 — double-count risk needs its own design pass). | Standard PR review. |
...
| **AD-2** | **Stake & beneficial attribution.** The assembler must read `OwnershipStake` percentages (TR 93/32 joint splits) and `BeneficialOwnershipOverride` (incl. BARE_TRUST / LRBA) — Phase 47 §4A P2/P3/P5/P8 + D6 bind Stage D to this. Until wired, jointly-held income is attributed flat by `ownerEntityId` — documented limitation, surfaced honestly. | ✅ **SHIPPED 2026-06-13.** New pure core `lib/services/ownershipAttribution.ts` (`attributeAsset` — JT equal split / TIC fractional / beneficial-override redirect / honest flat fall-back when a TIC split is uncomputable). `entityTaxFactsAssembler` now pulls income/expense on assets the entity CO-OWNS or BENEFICIALLY owns (not just `ownerEntityId === entity`) and scales every row by legal interest; weights sum to 1 across stakeholders (additivity preserved). No-false-silence flags `UC-OWNERSHIP-SPLIT` / `UC-OWNERSHIP-SPLIT-UNKNOWN` / `UC-BENEFICIAL-OWNED` / `UC-BENEFICIAL-REDIRECTED` ride `assemblerNotes`. Additive: an entity with no group/override receives byte-for-byte the prior flat mapping (all weights 1). 23 attribution tests + asset-key resolvers; 924-test suite green. Regime-neutral data-assembly (TR 93/32 splits apply identically pre/post reform — no §12.14 post-reform math introduced). |
