...
| **2c-ii — Assembler + repoint** | `entityTaxFactsAssembler`; repoint `GET /api/tax/entity` to real numbers; repoint `trustMinimumTax` to `TrustDeedRule`; remove the dead `parentEntityId` read. Q-UPE → engine returns `UNCOMPUTED` on contested sub-trust UPEs; Q-PARTNERSHIP → partnerships stay `UNCOMPUTED` (deferred follow-up). | The Q-* decisions are made — ready to build after 2c-i. |
