...
**Out of PR5, queued:**
- ✅ **PR5.5 SHIPPED 2026-05-18** — PDF summary + receipt bundle ZIP. Two new files: `lib/bookkeeping/taxPack/pdfExporter.ts` (`buildTaxPackPdf(summary)` — `pdfkit`, A4, 6 sections: cover + totals + ATO labels + per-property P&L + data sources + on-every-page footer disclaimer) and `lib/bookkeeping/taxPack/zipBundleBuilder.ts` (`buildReceiptZipBundle(userId, window)` — bundles every `Document` in the FY window whose `category` is RECEIPT / INVOICE / TAX, foldered by category, includes `MANIFEST.txt` with FY + counts + disclaimer). **§12.7 dep decision:** added `pdfkit` (single new dep, MIT, well-trodden Node PDF library — no GCP managed alternative exists for JSON-to-PDF); **dropped** the originally-planned `archiver` dep — reused the already-installed `jszip` instead, mirroring the canonical pattern at `/api/documents/export`. Tax Pack export endpoint extended to accept `format=pdf` and `format=zip`; the existing `<TaxPackExportButton />` (PR5.6) gets two new format options with PDF as the default. Schema unchanged. Privacy-clean (§13.3) — receipt bytes are user-uploaded artefacts (not CDR data); the manifest exposes only FY + counts.
