...
## 7. Surfaces instrumented

Labelled at the call site (`surface` on the completion options or the direct `recordAiUsage` call): `categorisation`, `cfo-advisor`, `debt-analysis`, `financial-advisor`, `budget-analysis`, `document-intelligence`, and (v1.1) `tax-advisor` + `cashflow-summary`. Every other Gemini call through the canonical chokepoints is recorded under `unknown` (counted, just unattributed).

**v1.1 (2026-06-29) — closed the two chokepoint-bypass gaps** so the cost baseline is complete *before* any tiering decision: the tax-advisor multi-turn provider (`lib/ai/tax-advisor/providers/geminiProvider.ts` — accumulates tokens across turns, now computes cost via `getModelPricing` + records `tax-advisor`) and the cashflow summary (`lib/cashflow-intelligence/geminiSummary.ts` — direct SDK call, now captures `usageMetadata` + records `cashflow-summary`). Both reuse the same `recordAiUsage` writer + pricing SSOT — no second cost formula.
