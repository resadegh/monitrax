...
- **1b — Centralised rules engine + service layer + migrate calculations off `parentEntityId`.** `lib/entity-graph/validityMatrix.ts` (the §6 grammar — pure functions, SSOT) + `lib/entity-graph/queries.ts` (graph traversal — pure functions, SSOT) + `lib/services/entityRelationshipService.ts` (the only writer of the graph; calls `validityMatrix` inside the write transaction; audited). See §8.4 for the SSOT architecture. The `OwnershipGroup`/`OwnershipStake` and `BeneficialOwnershipOverride` services. **All calculations reading `parentEntityId` repointed to the `TRUSTEE_OF` edges (via `queries.ts`) here, not 1c.**
  > **Build progress (2026-05-22).** 1b is shipped as two reviewable PRs. **1b-i** — the pure rules engine (`lib/entity-graph/{types,validityMatrix,queries}.ts`) + the §14.1 acceptance tests — *merged (PR #865)*. **1b-ii** — the DB-writing service layer (`entityRelationshipService.ts` + `ownershipService.ts` + `beneficialOwnershipService.ts`) — *shipped*. `parentEntityId` repoint: no `lib/calculations/` engine reads it; the sole reader is the tax-entity route, whose repoint belongs to Part 2's tax-engine rewire (Part 1 does not touch the tax engine). `getTrusteesOf()` is the canonical replacement query, ready for that rewire.
- **1c — Entity-section UI.** `EntityTree` → the entity-structure canvas (full spec in §11A) — a multi-edge, multi-lens graph; entity-detail lists all relationships; the accountant-review share-pass; the three-state badges.
  > **Build progress (2026-05-22).** 1c is shipped. The canvas (`components/entities/EntityCanvas.tsx`) is built on **React Flow (`@xyflow/react`) + `@dagrejs/dagre`** — library choice confirmed with Reza before build. It carries its own role-coloured Apple-glass node component (the Phase 39/41c design language is preserved; React Flow supplies only the interaction engine). The Control / Ownership / Money-flow / All lens toggle is live — the money-flow lens folds in the existing `MoneyFlowSankey` (the separate Money Flow tab on `/dashboard/entities` is removed). Click-through opens `EntityDetailDialog` (lists every relationship, with add / end / delete + a live §6.2 validity preview from `classifyEdge`) and `RelationshipDetailDialog`. API routes expose the Part 1b-ii services (`/api/entities/graph`, `/relationships*`, `/ownership-groups*`, `/beneficial-ownership*`, plus the `/verify` routes). Joint-ownership UI = `OwnershipGroupsDialog` (Q1). The accountant-review share-pass (Q4) is a print-friendly report page at `/dashboard/entities/accountant-review` with per-entity / per-relationship verify toggles — the `SharePackage` *pattern* (an owner-controlled export), deliberately **not** a public token link (§9). The canvas is presentational only (§8.4 / §11A) — it reads `queries.ts` + `validityMatrix` and renders; it computes nothing. `parentEntityId` stays frozen — its tax-route repoint is Part 2. `EntityTree.tsx` is retained (still used by the accountant portal client view).
- **1d — Onboarding wizard.** Extend `EntitiesStep` — a relationship sub-step capturing a working-graph skeleton (every entity + the load-bearing edges), progressive disclosure, finishable later in the entity section.
  > **Build progress (2026-05-22).** 1d is shipped. Realised as a new optional wizard step `entity-relationships` ("Who runs what", `components/onboarding/wizard/steps/RelationshipsStep.tsx`) inserted straight after the entity step — `getStepsForProfile` hides it unless the user has ≥1 non-personal entity, so a personal-name-only user never sees it (zero added onboarding friction). Per non-personal entity it captures the load-bearing edges for that type — directors + shareholders, trustee + beneficiaries/unitholders, members + trustee, partners, sole-trader operator — via collapsed cards + toggle-chip pickers (capture depth "control + ownership edges", confirmed with Reza). It pre-seeds `TRUSTEE_OF` from the trustee link already set in the entity step. `WizardData.relationships` carries the skeleton; `lib/onboarding/relationshipsSync.ts` persists it via the Part 1c `/api/entities/relationships` route on Continue — best-effort (never traps the user, §11 "finishable later"), idempotent on resume. **Part 1 of Phase 44 is now complete** (1a + 1b + 1c + 1d all merged); Part 2 (money-flow + tax-engine rewire) is the remaining body of work.
...
- **Then the rewire:** new `DistributionResolution`, `DividendDistribution`, `PrivateCompanyBenefit` (Div 7A), structured `TrustDeedRule` models; repoint `trustDistribution.ts` / `div6E.ts` / `div7A.ts` / `super/*` / `s100A.ts` to read graph + transactions; `MoneyFlowSankey` upgrade. The engine stays the one engine (§8.3) — only its *input shape* changes. Captured in its own design document.
  > **Design pass (2026-05-22).** The Part 2 design document is written: **`docs/blueprint/PHASE_44_PART_2_MONEY_FLOW_TAX_REWIRE.md`**. It opens with a full 45-module tax-engine audit. **Central finding:** the tax engine is already a mature, pure, citation-traced computation library — every rule module (trust Div 6/6E, Div 7A, SMSF caps, CGT, …) is built and correct for current law. The gap is *not* the engine — it is the **data layer** feeding it: trust-distribution / Div 7A / SMSF dispatch data currently reaches the engine only through a `curl` POST body; nothing persists it. So Part 2 is a *data-layer build*, not an engine rewrite — it adds the four "what actually happened" models + a canonical `entityTaxFactsAssembler`, and repoints `GET /api/tax/entity` to produce real numbers instead of `UNCOMPUTED`. The engine's 45 compute modules are not modified (one possible exception flagged for Reza: `super/smsfIncomeTax.ts`, filling a genuine gap — there is no SMSF fund-earnings income-tax module today). `parentEntityId` is confirmed already inert for tax (no engine module reads it). **The design doc is now v2** — an external Australian-tax-law conformance review (23 findings) has been incorporated as law-precision notes, new `UNCOMPUTED` gates and open questions (doc §13); no technical/design direction was taken from the review, only the law-alignment findings. **Status: 🟡 v2 awaiting Reza's approval before any Part 2 code.**
...
| **Phase 2 enhancement — FY slider** | ✅ Shipped this PR (2026-05-31) | Historical FY scrub via ghost-glass strip with tick marks for each FY with recorded distributions. Aggregator now fetches every CONFIRMED distribution/dividend (was current-FY only); `moneyFlowFyOptions` + per-flow `financialYear` drive client-side FY filtering. Reform-pending count auto-retracks selected FY (§12.14 FW-1 per-FY discipline). |
| **Phase 3 — click-to-zoom** | ✅ Shipped this PR (2026-05-31) | Level 2 ecosystem dim (selecting a tile recedes non-connected tiles + ribbons to 0.22, breadcrumb advances to "Level 2 · {entity}"). Level 3 EntityDetailPanel + mobile SelectedEntityCard gain a **Linked assets** list with per-asset click-through (property/loan use individual routes; account/investment/asset fall back to list pages). `useWealthExplorerData` exposes `snapshot` alongside `layout` (no re-fetch). Dead-end "Open full entity file →" CTA removed. |
