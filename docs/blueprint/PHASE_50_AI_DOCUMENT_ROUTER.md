...
3. **Confidence-gated autonomy.** 🟡 **D.3 SSOT SHIPPED (2026-06-18); auto-execution gated.** New canonical `lib/documents/intelligence/confidencePolicy.ts` — ONE place for the thresholds (AUTO ≥0.9, CONFIRM ≥0.7, ASK <0.7) + the autonomy band, replacing the 0.9/0.7 magic numbers duplicated across `GlobalScanReceipt`/`AnalysisPreviewCard`/`ExtractionReviewForm` (§12.3). Includes the **earned-autonomy** TRAIL-stage downgrade (Track/Reduce users never AUTO). 6 tests. **DEFERRED — auto-EXECUTION (writing a financial record without confirmation when band===AUTO) is NOT wired**: it's a §12.11 + financial-adviser decision + a Stitch UX pass, pending Reza sign-off on thresholds + the undo affordance. Today AUTO still routes through the one-tap confirm.
