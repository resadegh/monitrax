...
D.1 ✅, D.2 ✅, D.3 ✅ (SSOT; auto-exec rejected per autonomy decision), **D.4 ✅ (suggest-only learned routing)**. Remaining: **D.5 lifecycle** (renewal/expiry extraction → reminder engine; ATO 5yr retention clock) and the **bulk-approve Smart Inbox** convenience (the sanctioned autonomy shortcut from the 2026-06-18 decision — a Stitch-first UI follow-up where AUTO-band items are pre-checked but each confirm stays explicit + editable). Also D.1.1 (funnel the legacy scan path through `processUpload` so it shares the one dedup).
