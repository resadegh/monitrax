...
5. **Lifecycle intelligence.** 🟡 **D.5b SHIPPED (2026-06-18) — retention clock; D.5a deferred (Stitch-first).**
   ...
   - **D.5a — renewal/expiry extraction → reminder engine (DEFERRED, Stitch-first):** the reminder engine (`lib/reminders/reminderEngine.ts`) already projects renewals from dates ON the entity models (Asset.vehicleInsuranceExpiry, Property.buildingInsuranceExpiry, Loan.fixedExpiry, …). So D.5a's job is to write an extracted expiry date onto the linked entity so the existing engine lights up. **But that writes to a user's asset row — per the autonomy decision it must be suggest→confirm, which is a NEW UI surface (§18.2.1 Stitch-first).** Queued behind a Stitch pass: "we found an expiry of <date> on this <doc> — add it to your <entity>?" Never silent.
...
D.1 ✅, D.2 ✅, D.3 ✅ (SSOT; auto-exec rejected per autonomy decision), **D.4 ✅ (suggest-only learned routing)**, **D.5b ✅ (retention clock — advisory)**. Remaining: **D.5a** (renewal/expiry extraction → write entity date so the reminder engine lights up — Stitch-first, suggest→confirm per autonomy), the **bulk-approve Smart Inbox** convenience (the sanctioned autonomy shortcut from the 2026-06-18 decision — a Stitch-first UI follow-up where AUTO-band items are pre-checked but each confirm stays explicit + editable), and **D.1.1** (funnel the legacy scan path through `processUpload` so it shares the one dedup).
