...
// Phase 38 PR 2 — Smart Inbox helpers. The per-row summary / confidence-dot /
// action-label helpers were removed in Phase 50 D.6 when the inline inbox was
// replaced by the SmartInbox component (which owns its own presentation via the
// confidencePolicy SSOT). `formatDocumentTypeLabel` is kept — still used by the
// document-type badges elsewhere on the page.
...
export default function DocumentsLibraryPage() {
  ...
  // Phase 50 D.6 — the Smart Inbox (bulk-approve) is now a self-contained
  // component (components/documents/SmartInbox.tsx). It reuses
  // handleConfirmAnalysis (the SSOT confirm path) per approved item; the page
  // only supplies the data + the open-file handler below.
      ...
      <div className="flex h-[calc(100vh-4rem)]">
        ...
        <main className="flex-1 overflow-y-auto p-6">
          <div className="space-y-6 max-w-7xl mx-auto">
            ...
            {/* Phase 50 D.6 — bulk-approve Smart Inbox. AUTO-band (D.3) rows
                pre-selected; user always confirms + can edit any. Reuses the
                SSOT confirm path (handleConfirmAnalysis) per item — no new
