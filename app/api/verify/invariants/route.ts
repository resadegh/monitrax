 ...
 *
 * Computes the NeoAudit Ring-3 accounting invariants SERVER-SIDE on the
 * logged-in user's OWN real data and returns PASS/FAIL with exact deltas.
 * This is the "one click on real data" instrument from the NeoAudit blueprint
 * (docs/blueprint/NEOAUDIT.md §1.2 R3-self, §6 Release Scorecard) — it
 * replaces most of the manual Claude-in-Chrome Part-E relay.
 *
 * Sole responsibility (non-overlap contract §1.2): "do the invariants hold on
 * the user's REAL data right now?" — nothing else. It READS canonical sources
 * only (§12.2.1): getMasterFinancialSnapshot + the canonical accessors. It
 * computes NO financial figure of its own — every lhs/rhs below is either a
 * canonical field or the arithmetic identity between canonical fields that the
 * invariant asserts.
 *
  ...
  try {
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        invariants,
        // Informational (no assertion — canonical values + basis for the panel):
        info: {
