...
export const GET = withPermission<RouteContext>('holding.read', async (request, auth, context) => {
  try {
...
export const PUT = withPermission<RouteContext>('holding.write', async (request, auth, context) => {
  try {
    const { id } = await context!.params;
 ...
 */
export const PATCH = withPermission<RouteContext>('holding.write', async (request, auth, context) => {
  try {
    const { id } = await context!.params;
    const body = await request.json();
...
export const DELETE = withPermission<RouteContext>('holding.delete', async (request, auth, context) => {
  try {
    const { id } = await context!.params;
    // Verify ownership
    const existing = await prisma.investmentHolding.findUnique({
